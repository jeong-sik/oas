...
type t = {
  tbl: (string, Skill.t) Hashtbl.t;
}

let create () = { tbl = Hashtbl.create 16 }

let register reg (skill : Skill.t) =
  Hashtbl.replace reg.tbl skill.name skill

let find reg name =
  Hashtbl.find_opt reg.tbl name

let remove reg name =
  Hashtbl.remove reg.tbl name
...
let names reg =
  list reg |> List.map (fun (s : Skill.t) -> s.name)

let count reg =
  Hashtbl.length reg.tbl

let load_from_dir reg ?scope dir =
  if not (Sys.file_exists dir && Sys.is_directory dir) then
    Error (Error.Io (FileOpFailed {
      op = "load_from_dir";
      path = dir;
...
let to_json reg =
  `Assoc [
    ("skills", `List (list reg |> List.map skill_to_json));
    ("count", `Int (count reg));
  ]
...
let skill_of_json (json : Yojson.Safe.t) : (Skill.t, Error.sdk_error) result =
  ...
  try
    ...
    let path =
      ...
      | v -> Some (to_string v)
    in
    let scope = None in (* scope is informational; not round-tripped *)
    let allowed_tools =
    ...
    let supporting_files =
      ...
      | _ -> []
    in
    Ok { Skill.name; description; body; path; scope; allowed_tools;
         argument_hint; model; supporting_files; metadata = [] }
  with
  | Type_error (msg, _) ->
    Error (Error.Internal
      (Printf.sprintf "Skill_registry.skill_of_json: %s" msg))
...
let of_json (json : Yojson.Safe.t) : (t, Error.sdk_error) result =
    ...
    let rec load = function
      ...
      | j :: rest ->
        (match skill_of_json j with
         | Ok skill -> register reg skill; load rest
         | Error _ as err -> err)
    in
    load skills_json
  with
  | Type_error (msg, _) ->
    Error (Error.Internal
      (Printf.sprintf "Skill_registry.of_json: %s" msg))
