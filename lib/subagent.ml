...
let state_isolation_preamble = function
  ...
  | Isolated ->
    Some
      "[State Isolation: This agent runs in an isolated context without access to parent \
       conversation history.]"
  | Selective keys ->
    Some
      (Printf.sprintf
         "[State Isolation: This agent inherits only the following state keys from \
          parent: %s]"
         (String.concat ", " keys))
;;
...
let compose_prompt ?arguments spec =
  ...
  | None -> base
  | Some preamble -> if base = "" then preamble else preamble ^ "\n\n" ^ base
;;
