...
let lookup_replacement t id =
  Hashtbl.find_opt t.replacements id

(* ── Record decisions ───────────────────────────────────────── *)

let record_replacement t (r : replacement) =
  if Hashtbl.mem t.seen_ids r.tool_use_id then
    invalid_arg
      (Printf.sprintf
...
let record_kept t id =
  if Hashtbl.mem t.seen_ids id then
    invalid_arg
      (Printf.sprintf
...
let to_json t =
  let seen_list =
    Hashtbl.fold (fun id () acc -> `String id :: acc) t.seen_ids []
  in
  let replacements_list =
    Hashtbl.fold (fun _id r acc ->
      `Assoc [
...
let of_json json =
  try
    ...
    List.iter (fun j ->
      ...
      Hashtbl.replace t.replacements tool_use_id
...
let%test "record_kept: freezes without replacement" =
  ...
  record_kept t "t1";
  is_frozen t "t1"
  && lookup_replacement t "t1" = None

let%test "record_replacement on frozen raises" =
  let t = create () in
  record_kept t "t1";
  (try record_replacement t { tool_use_id = "t1"; preview = "p"; original_chars = 10 }; false
   with Invalid_argument _ -> true)

let%test "record_kept on frozen raises" =
  let t = create () in
  record_replacement t { tool_use_id = "t1"; preview = "p"; original_chars = 10 };
  (try record_kept t "t1"; false
   with Invalid_argument _ -> true)

let%test "lookup_replacement: returns recorded preview" =
  let t = create () in
  record_replacement t { tool_use_id = "t1"; preview = "preview text"; original_chars = 5000 };
...
let%test "apply_frozen: replaces frozen, collects fresh" =
  ...
  record_kept t "t2";
  let blocks = [
    ToolResult { tool_use_id = "t1"; content = "long content"; is_error = false; json = None };
    ToolResult { tool_use_id = "t2"; content = "kept content"; is_error = false; json = None };
    ToolResult { tool_use_id = "t3"; content = "new content"; is_error = false; json = None };
  ] in
  let modified, fresh = apply_frozen t blocks in
  (* t1: replaced with "short"; t2: kept as-is; t3: fresh *)
  let t1_ok = match List.nth modified 0 with
    | ToolResult { content; _ } -> content = "short"
    | _ -> false
  in
  let t2_ok = match List.nth modified 1 with
...
let%test "apply_frozen: idempotent" =
  ...
  let blocks = [
    ToolResult { tool_use_id = "t1"; content = "original"; is_error = false; json = None };
  ] in
