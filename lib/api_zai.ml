...
let auth_headers ?api_key content_type =
  ...
  if api_key = "" then base else ("Authorization", "Bearer " ^ api_key) :: base
;;
...
let parse_guard f =
  try f () with
  | Yojson.Safe.Util.Type_error (msg, _) | Yojson.Safe.Util.Undefined (msg, _) ->
...
let get_media_async_result
  ...
  | Ok (code, body) ->
    if code >= 200 && code < 300
    then parse_media_async_result body
    else Error (Http_client.HttpError { code; body })
;;
  ...
  try
    ...
    Fun.protect
      ...
      (fun () ->
         ...
         if len > max_multipart_file_size
         then
           Error
             (Http_client.HttpError
                ...
                ; body =
                    Printf.sprintf
                      "File %S is too large for in-memory multipart upload (%d bytes > \
                       %d bytes)"
                      path
                      len
...
let multipart_body ?prompt ?language ~model ~source () =
  ...
  (match prompt with
   | Some value -> add_field "prompt" value
   | None -> ());
  (match language with
...
let%test "status_of_string parses known values" = status_of_string "SUCCESS" = Success
...
let%test "parse_image_generation filters entries without urls" =
  match
    parse_image_generation
      {|{"created":42,"data":[{"url":"https://example.com/a.png"},{"x":1}]}|}
  with
...
let%test "parse_image_generation rejects malformed json" =
  ...
  | Error (Http_client.HttpError { code; body }) -> code = 400 && String.length body > 0
  | _ -> false
;;
...
let%test "parse_media_async_result defaults unknown status and filters bad items" =
  ...
  | Ok result ->
    ...
    && List.hd result.results
       = { url = "https://example.com/a.mp4"; cover_image_url = None }
  | Error _ -> false
;;

let%test "media_async_result_url encodes task id path segment" =
  let url =
    media_async_result_url ~base_url:"https://api.z.ai/api/paas/v4" ~id:"job/1 ?x#y"
  in
  String.contains url '%'
;;
...
let%test "multipart_body supports base64 source with optional fields" =
  ...
  | Error _ -> false
  | Ok (_boundary, body) ->
    String.contains body 'h' && String.contains body 'k' && String.contains body 'Z'
;;
