...
let generate ?baseline (runs : Eval.run_metrics list) : t =
  ...
  then
    ...
    let summary =
      ...
      | `Pass ->
        Printf.sprintf
          "PASS: %s — pass@%d=%.0f%%, %d regression(s), %d skipped"
          first.agent_name
          evaluated_runs
          (pak *. 100.0)
          (match comparison with
           | Some c -> c.regressions
           | None -> 0)
          skipped_runs
      | `Fail ->
        Printf.sprintf
          "FAIL: %s — pass@%d=%.0f%%, %d regression(s), %d skipped"
          first.agent_name
          evaluated_runs
          (pak *. 100.0)
          (match comparison with
      ...
      | `NoBaseline ->
        Printf.sprintf
          "NO BASELINE: %s — pass@%d=%.0f%%, %d skipped (run 'save-baseline' to create)"
          first.agent_name
          evaluated_runs
          (pak *. 100.0)
          skipped_runs
