...
let extract_with_coerce ~name ~typ ~required ~default ~unwrap json =
  ...
  | v ->
    let coerced =
      match Tool_input_validation.try_coerce typ v with
      | Some c -> c
      | None -> v
    in
...
let string_field name ~required ~desc ?(default = "") () =
  make_field
    ...
    ~desc
    ~extract:
      (extract_with_coerce ~name ~typ:Types.String ~required ~default ~unwrap:(function
         | `String s -> Some s
         | _ -> None))
;;
...
let int_field name ~required ~desc ?(default = 0) () =
  make_field
    ...
    ~desc
    ~extract:
      (extract_with_coerce ~name ~typ:Types.Integer ~required ~default ~unwrap:(function
         | `Int i -> Some i
         | _ -> None))
;;
...
let float_field name ~required ~desc ?(default = 0.0) () =
  make_field
    ...
    ~desc
    ~extract:
      (extract_with_coerce ~name ~typ:Types.Number ~required ~default ~unwrap:(function
         | `Float f -> Some f
...
let bool_field name ~required ~desc ?(default = false) () =
  make_field
    ...
    ~desc
    ~extract:
      (extract_with_coerce ~name ~typ:Types.Boolean ~required ~default ~unwrap:(function
         | `Bool b -> Some b
         | _ -> None))
;;
