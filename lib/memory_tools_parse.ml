...
let tool_error message = Error { Types.message; recoverable = true; error_class = None }
...
let parse_value_json json =
  ...
  | Yojson.Json_error _ -> Ok (`String raw)
;;
...
let parse_outcome json =
  ...
  | `String "success" -> Ok (Memory.Success (Option.value detail ~default:""))
