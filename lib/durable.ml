...
let run_from t ~start_idx ~input ~journal =
  ...
  let rec loop idx cur_input acc_journal =
    if idx >= total
    then Completed { journal = List.rev acc_journal; final_output = cur_input }
    else (
      ...
      let rec try_step attempt =
        ...
        | Error err ->
          ...
          then try_step (attempt + 1)
          else
            Failed
              { at_step = step.name; journal = List.rev (je :: acc_journal); error = err }
      in
      try_step 1)
  in
...
let execute t input = run_from t ~start_idx:0 ~input ~journal:[]
...
let resume t state =
  ...
  | Suspended { at_step; journal; _ } | Failed { at_step; journal; _ } ->
    ...
    let rec find_idx idx = function
      | [] -> None
      | (s : step) :: rest ->
        if s.name = at_step then Some idx else find_idx (idx + 1) rest
    in
    (match find_idx 0 steps with
     ...
     | Some idx ->
       ...
       let prior_journal = List.filter (fun e -> e.step_name <> at_step) journal in
...
let journal_entry_to_json je =
  `Assoc
    ...
    ; ( "completed_at"
      , match je.completed_at with
        | None -> `Null
        | Some f -> `Float f )
    ; "input_json", je.input_json
    ; ( "output_json"
      , match je.output_json with
        | None -> `Null
        | Some j -> j )
    ; ( "error"
      , match je.error with
...
let journal_entry_of_json json =
  ...
  try
    ...
    Ok { step_name; started_at; completed_at; input_json; output_json; error; attempt }
  with
  | Eio.Cancel.Cancelled _ as e -> raise e
  | Yojson.Safe.Util.Type_error (msg, _) -> Error msg
;;

let journal_list_to_json journal = `List (List.map journal_entry_to_json journal)
...
let execution_state_to_json = function
  | NotStarted -> `Assoc [ "state", `String "NotStarted" ]
