...
let create ?(config = default_config) agents = { agents; config }
let add_agent orch name agent = { orch with agents = (name, agent) :: orch.agents }
let find_agent orch name = List.assoc_opt name orch.agents
...
let run_agent_with_timeout ~sw ?clock ~task_id config agent prompt =
  ...
  | Some timeout, Some clk ->
    (try
     ...
     with
     | Eio.Time.Timeout -> Error (Error.Orchestration (TaskTimeout { task_id })))
   ...
   | Some bus ->
     ...
     Event_bus.publish
       ...
       (Event_bus.mk_event
          ~correlation_id
          ~run_id
          ?caused_by:started_run_id
          (AgentCompleted
             { agent_name = task.agent_name; task_id = task.id; result; elapsed }));
      ...
      | Error err ->
        Event_bus.publish
          ...
          (Event_bus.mk_event
             ...
             ?caused_by:started_run_id
             (AgentFailed
                { agent_name = task.agent_name; task_id = task.id; error = err; elapsed }))
      | Ok _ -> ())
   | None -> ());
...
let execute_sequential ~sw ?clock orch tasks = List.map (run_task ~sw ?clock orch) tasks

let execute_parallel ~sw ?clock orch tasks =
  Eio.Fiber.List.map ~max_fibers:orch.config.max_parallel (run_task ~sw ?clock orch) tasks
;;

let execute_fan_out ~sw ?clock orch ~prompt ~agents =
  let tasks =
    List.mapi
      (fun i agent_name -> { id = Printf.sprintf "fanout-%d" i; prompt; agent_name })
      agents
  in
...
let pipeline ~sw ?clock orch tasks = execute ~sw ?clock orch (Pipeline tasks)
  ...
  List.filter_map
    (fun tr ->
       ...
       | Error e ->
         Log.warn
           _log
           "task result discarded in collect_text"
...
and execute_conditional_inner ~sw ?clock orch last_result plan =
  ...
  | Branch { condition; if_true; if_false } ->
    if eval_condition last_result condition
    then execute_conditional_inner ~sw ?clock orch last_result if_true
    else execute_conditional_inner ~sw ?clock orch last_result if_false
  | Sequence plans ->
  ...
  | Loop { body; until; max_iterations } ->
    let rec iterate acc last iteration =
      ...
      then List.rev acc
        ...
        if eval_condition new_last until
        then List.rev all_acc
...
type selection_strategy =
  | FirstOk (** First non-error result in completion order. *)
  | BestBy of (task_result -> float) (** Highest score wins. Only considers Ok results. *)
  | MajorityText (** Most common text response wins. Ties broken by first occurrence. *)
      ...
      List.filter_map
        (fun tr ->
           ...
           | Error e ->
             Log.warn
               _log
               "task result excluded from BestBy scoring"
               [ Log.S ("task_id", tr.task_id)
               ; Log.S ("agent", tr.agent_name)
               ; Log.S ("error", Error.to_string e)
               ];
  ...
  | MajorityText ->
      ...
      List.filter_map
        (fun tr ->
           ...
           | Error e ->
             Log.warn
               _log
               "task result excluded from MajorityText voting"
               [ Log.S ("task_id", tr.task_id)
               ; Log.S ("agent", tr.agent_name)
               ; Log.S ("error", Error.to_string e)
               ];
    ...
    let counts =
      List.fold_left
        (fun acc (_tr, text) ->
           let prev =
             match List.assoc_opt text acc with
             | Some n -> n
             | None -> 0
           in
    ...
    (match best_text with
     | None -> None
     | Some text -> Some (fst (List.find (fun (_tr, t) -> t = text) texts)))
;;
...
let execute_consensus ~sw ?clock orch ~prompt ~agents ~strategy =
  let tasks =
    List.mapi
      (fun i agent_name -> { id = Printf.sprintf "consensus-%d" i; prompt; agent_name })
      agents
  in
...
let execute_hierarchical ~sw ?clock ?(max_parallel = 4) sub_plans =
  Eio.Fiber.List.map
    ~max_fibers:max_parallel
    (fun (label, sub_orch, sub_plan) ->
       let t0 = Unix.gettimeofday () in
       ...
       in
       { task_id = label; agent_name = label; result = Ok combined_response; elapsed })
    sub_plans
;;
...
let rec execute_step_plan ~sw plan ~input =
  ...
  | Step_parallel { plans; max_concurrency } ->
    ...
    let results =
      Eio.Fiber.List.map ~max_fibers (fun plan -> execute_step_plan ~sw plan ~input) plans
    in
    let texts =
      List.filter_map
        (function
  ...
  | Step_loop { body; until; max_iterations } ->
    let rec loop n current =
      ...
      then Ok current
        ...
        | Error e -> Error e
        | Ok output -> if until output then Ok output else loop (n + 1) output)
    in
    loop 0 input
;;
