...
let payload_to_json p =
  ...
  in
  let agent =
    match p.agent_name with
    | Some n -> [ "agent_name", `String n ]
    | None -> []
  in
  let caused_by =
    match p.caused_by with
...
let event_to_payload (event : Event_bus.event) : event_payload =
  ...
  let data =
    ...
    | Custom (name, data) -> `Assoc [ "name", `String name; "data", data ]
  in
  { event_type
...
let create ~targets ?(batch_size = 10) ?(flush_interval_s = 1.0) () =
  { targets
  ; batch_size
  ; flush_interval_s
...
let deliver_batch t payloads =
  List.iter
    (fun target ->
       match target with
       | File_append { path } -> deliver_to_file t path payloads
       | Custom_target { name; deliver } -> deliver_to_custom t name deliver payloads)
    t.targets
;;
...
let start ~sw ~(net : _ Eio.Net.t) ~bus t =
  let _ = net in
  if not (Atomic.compare_and_set t.running false true)
  then ()
  else (
    ...
    Eio.Fiber.fork ~sw (fun () ->
      Fun.protect
        ~finally:(fun () ->
          ...
          try Event_bus.unsubscribe bus sub with
          | exn ->
            Log.debug
              t.log
        ...
        (fun () ->
           try
             ...
             if !batch_len > 0 then deliver_batch t (List.rev !batch)
           with
           | Eio.Cancel.Cancelled _ as ex -> raise ex
           | exn ->
             Log.warn
               t.log
               "event forward loop crashed"
