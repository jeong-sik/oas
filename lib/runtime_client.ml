...
let start_session ?control_handler ?event_handler client request_data =
  let* response =
    request ?control_handler ?event_handler client (Runtime.Start_session request_data)
  in
  match response with
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf
            "Unexpected start_session response: %s"
...
let apply_command ?control_handler ?event_handler client ~session_id command =
  ...
  in
  match response with
  | Runtime.Command_applied session | Runtime.Finalized session -> Ok session
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf
            "Unexpected apply_command response: %s"
            (Runtime.show_response other)))
;;
...
let status client ~session_id =
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf "Unexpected status response: %s" (Runtime.show_response other)))
;;
...
let events client ~session_id ?after_seq () =
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf "Unexpected events response: %s" (Runtime.show_response other)))
;;
...
let finalize ?control_handler ?event_handler client ~session_id ?reason () =
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf "Unexpected finalize response: %s" (Runtime.show_response other)))
;;
...
let report client ~session_id =
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf "Unexpected report response: %s" (Runtime.show_response other)))
;;
...
let prove client ~session_id =
  ...
  | other ->
    Error
      (Error.Internal
         (Printf.sprintf "Unexpected prove response: %s" (Runtime.show_response other)))
;;
