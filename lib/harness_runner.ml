...
let case_failure ?detail ?response_text ?metrics ?raw_trace_path (case_ : Harness_case.t) =
...
let response_verdict response_text = function
  | Harness_case.Exact_text expected ->
    mk_verdict
      ...
      [ Printf.sprintf "expected=%s" expected; Printf.sprintf "actual=%s" response_text ]
  ...
  | Harness_case.Contains_text needle ->
    ...
    mk_verdict
      passed
      [ Printf.sprintf "needle=%s" needle; Printf.sprintf "response=%s" response_text ]
      ~detail:
        (if passed then None else Some "response text did not contain expected substring")
  | Harness_case.Structural_json expected_json ->
    (match
       try Ok (Yojson.Safe.from_string response_text) with
...
let trace_verdict
      ?trajectory
      (obs : Harness.Behavioral.observation)
...
let compare_numeric_threshold ~goal ~target ~tolerance_pct current =
  ...
  | Eval.Higher ->
    let floor =
      if Float.abs target < 1e-10 then target else target *. (1.0 -. tolerance)
    in
    current >= floor, floor
  | Eval.Lower ->
    let ceil =
  ...
  | Eval.Exact ->
    let allowed =
      if Float.abs target < 1e-10 then tolerance else Float.abs target *. tolerance
    in
...
let metric_verdict
      ...
      (assertion : Harness_case.metric_assertion)
  =
  let tolerance_pct = Option.value assertion.tolerance_pct ~default:0.0 in
  ...
  then
    ...
    | Some metric ->
      (match
         ( Eval.metric_value_to_float metric.value
         , Eval.metric_value_to_float assertion.target )
       with
       ...
       | _ ->
         ...
         mk_verdict
           ...
           ~detail:
             (if passed then None else Some "non-numeric metric requires exact match")))
;;
...
let collect_metrics
  ...
  Eval.record
    collector
    { name = "turn_count"; value = Eval.Int_val obs.turn_count; unit_ = None; tags = [] };
  Eval.record
    collector
    { name = "tool_calls"
...
let evaluate_assertion ?trajectory obs response response_text metrics = function
  | Harness_case.Response assertion -> response_verdict response_text assertion
  | Harness_case.Trace assertion -> trace_verdict ?trajectory obs response assertion
  | Harness_case.Metric assertion -> metric_verdict metrics assertion
;;
...
let grade_case
  ...
  let metrics =
    collect_metrics ~agent_name ~run_id:case_.id ~obs:observation ~response ~elapsed
  in
  let verdicts =
    List.map
      (evaluate_assertion ?trajectory observation response response_text metrics)
      case_.assertions
  in
  let collector =
    Eval.create_collector ~agent_name:metrics.agent_name ~run_id:metrics.run_id
  in
  ...
  let status, detail =
    ...
    | _ ->
      ...
      then Harness_report.Pass, None
        ...
        let first_failure =
          match
            List.find_opt (fun (verdict : Harness.verdict) -> not verdict.passed) verdicts
          with
  ...
  ; verdicts
  ; evidence =
      List.concat_map (fun (verdict : Harness.verdict) -> verdict.evidence) verdicts
  ; detail
  ...
  | _, None ->
    Error
      (Error.Io
         (Error.ValidationFailed
            { detail =
                Printf.sprintf "case '%s' does not define source_trace_path" case_.id
            }))
...
let run_case ~sw ~clock ~build_agent (case_ : Harness_case.t) =
  ...
  | Error err -> case_failure case_ ~detail:(Error.to_string err)
  | Ok agent ->
...
let run_dataset_mixed ?run_fixture cases =
  cases |> List.map (execute_case ?run_fixture) |> Harness_report.of_results
;;
