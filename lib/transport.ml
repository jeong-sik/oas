...
let is_runnable path = Sys.file_exists path && not (Sys.is_directory path)
    ...
    Error
      (Error.Io
         (Error.FileOpFailed
            ...
            ; path = "oas_runtime"
            ; detail =
                "No runtime binary found. Set OAS_RUNTIME_PATH or build \
                 bin/oas_runtime.exe."
            }))
;;
...
let default_control_response = function
  ...
  | Runtime.Hook_request _ -> Runtime.Hook_response { continue_ = true; message = None }
;;

let fail_all_pending transport err =
  Hashtbl.iter
    (fun _id resolver -> Eio.Promise.resolve resolver (Error err))
    transport.pending;
...
let dispatch_protocol_message transport = function
  ...
  | Runtime.Control_request_message payload ->
    let handler = Eio.Mutex.use_ro transport.mu (fun () -> transport.control_handler) in
    let response =
      match handler with
  ...
  | Runtime.Event_message payload ->
    let handler = Eio.Mutex.use_ro transport.mu (fun () -> transport.event_handler) in
...
let reader_loop transport =
  let rec loop () =
    ...
    if String.trim line = ""
    then (if not (Atomic.get transport.closed) then loop ())
    else (
      ...
      | Error detail ->
        set_reader_failed transport (Error.Serialization (JsonParseError { detail })))
  in
  loop ()
;;
...
let start_reader_fiber ~sw transport =
  Eio.Fiber.fork_daemon ~sw (fun () ->
    (try reader_loop transport with
     | End_of_file | Eio.Io _ | Unix.Unix_error _ | Sys_error _ ->
       if not (Atomic.get transport.closed)
       then
         set_reader_failed
           transport
           (Error.Io
  ...
  try
    ...
    let reader =
      Eio.Buf_read.of_flow
        (r_child_stdout :> _ Eio.Flow.source)
    ...
    | Runtime.Initialized init
      when String.equal init.protocol_version Runtime.protocol_version ->
    ...
    | Runtime.Initialized init ->
      Error
        (Internal
           (Printf.sprintf
              "Protocol version mismatch: runtime=%s sdk=%s"
    ...
    | other ->
      Error
        (Internal
           (Printf.sprintf "Unexpected init response: %s" (Runtime.show_response other)))
  with
  | Eio.Io _ as exn ->
    Error
      (Io
         (FileOpFailed
            { op = "spawn"; path = runtime_path; detail = Printexc.to_string exn }))
  | Unix.Unix_error (err, _, _) ->
    Error
      (Io
         (FileOpFailed
            { op = "spawn"; path = runtime_path; detail = Unix.error_message err }))
  | Failure msg ->
    Error (Io (FileOpFailed { op = "spawn"; path = runtime_path; detail = msg }))
;;

let request ?control_handler ?event_handler transport request =
  let open Error in
  if Atomic.get transport.closed
  then Error (Internal "Runtime transport is already closed")
  else (
    ...
    write_message transport (Runtime.Request_message { request_id; request });
...
let close transport =
  if Atomic.compare_and_set transport.closed false true
  then (
    (try ignore (request transport Runtime.Shutdown) with
...
let%test "is_runnable false for directory" = is_runnable "/tmp" = false
...
let%test "default_control_response Permission_request allows" =
  match
    default_control_response
      (Runtime.Permission_request { action = "test"; subject = "desc"; payload = `Null })
  with
...
let%test "default_control_response Hook_request continues" =
  match
    default_control_response
      (Runtime.Hook_request { hook_name = "before_turn"; payload = `Null })
  with
