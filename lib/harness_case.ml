...
type response_assertion =
  ...
  | Fuzzy_text of { expected: string; threshold: float }
...
let kind_of_string = function
  ...
  | other ->
    Error (Error.Serialization (UnknownVariant {
      type_name = "Harness_case.kind";
      value = other;
    }))
...
let goal_of_string = function
  ...
  | other ->
    Error (Error.Serialization (UnknownVariant {
      type_name = "Harness_case.metric_goal";
      value = other;
    }))
...
let trace_assertion_to_json = function
  ...
  | Max_turns value ->
    `Assoc [("type", `String "trace_max_turns"); ("value", `Int value)]
...
let metric_assertion_to_json (metric : metric_assertion) =
  `Assoc [
    ...
    ("tolerance_pct",
     match metric.tolerance_pct with Some value -> `Float value | None -> `Null);
  ]
...
let to_json (case_ : t) =
  `Assoc [
    ...
    ("source_trace_path",
     match case_.source_trace_path with Some path -> `String path | None -> `Null);
  ]
...
let response_assertion_of_json json =
  ...
  try
    ...
    | other ->
      Error (Error.Serialization (UnknownVariant {
        type_name = "Harness_case.response_assertion";
        value = other;
      }))
  with Yojson.Safe.Util.Type_error (msg, _) ->
    Error (Util.json_parse_error msg)
...
let trace_assertion_of_json json =
  ...
  try
    ...
    | other ->
      Error (Error.Serialization (UnknownVariant {
        type_name = "Harness_case.trace_assertion";
        value = other;
      }))
  with Yojson.Safe.Util.Type_error (msg, _) ->
    Error (Util.json_parse_error msg)
...
let assertion_of_json json =
  ...
  try
    ...
    | other ->
      Error (Error.Serialization (UnknownVariant {
        type_name = "Harness_case.assertion";
        value = other;
      }))
  with Yojson.Safe.Util.Type_error (msg, _) ->
    Error (Util.json_parse_error msg)
...
let of_json json =
  ...
  try
    ...
    | Error _ as err -> err
    | Ok kind ->
      let tags_json = match json |> member "tags" with `Null -> `List [] | other -> other in
      let artifacts_json =
        match json |> member "artifacts" with `Null -> `List [] | other -> other
      in
      let assertions_json =
        match json |> member "assertions" with `Null -> [] | other -> to_list other
      in
          ...
          | Ok artifacts ->
            ...
            | Ok assertions ->
              Ok {
                ...
                assertions;
                artifacts;
                source_trace_path = json |> member "source_trace_path" |> to_string_option;
              }))
  with Yojson.Safe.Util.Type_error (msg, _) ->
    Error (Util.json_parse_error msg)
...
let make_trace_replay ?(tags = []) ?(assertions = []) ?(artifacts = []) ~id ~prompt
    ~source_trace_path () =
  {
    id;
    kind = Trace_replay;
...
let trace_replay_of_records ~id ~source_trace_path records =
  ...
  | None ->
    Error (Error.Io (ValidationFailed {
      detail = "raw trace did not contain a Run_started prompt";
    }))
  | Some prompt ->
    ...
    let assertions =
      []
      |> (fun acc ->
           if trajectory.success then Trace Succeeds :: acc else acc)
      |> (fun acc ->
           match final_response_text trajectory with
           | Some text when String.trim text <> "" -> Response (Exact_text text) :: acc
           | _ -> acc)
      |> (fun acc ->
           if tool_sequence = [] then acc
           else Trace (Tool_sequence tool_sequence) :: Trace (Tool_call_count (List.length tool_sequence)) :: acc)
      |> List.rev
    in
