...
let detect_trend ~window data =
  ...
  if n < window then Insufficient_data
    ...
    if den < 1e-12 then Stable
      ...
      else if slope < -. threshold then Degrading
      else Stable
  ...
  | last :: prev :: rest ->
    ...
    let rec count_same acc prev = function
      ...
      | x :: xs ->
        ...
        if d = initial_dir then count_same (acc + 1) x xs
        else acc
    in
...
let%test "consecutive_direction empty" =
  consecutive_direction [] = (0, `Flat)

let%test "consecutive_direction single" =
  consecutive_direction [1.0] = (0, `Flat)
