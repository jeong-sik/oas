...
let tools_for_turn strategy turn ?context () =
  match strategy with
  | Phase_based { phases } ->
    let _ = context in  (* unused for phase-based *)
  ...
  | Gather_act_verify { gather_tools; act_tools; verify_tools } ->
    let _ = context in  (* unused for GAV *)
  ...
  | Retrieval_based { index; confidence_threshold; fallback_tools; always_include } ->
    ...
    | Some query ->
      if Tool_index.confident index query ~threshold:confidence_threshold then
        ...
        let add_unique acc name =
          ...
          else begin Hashtbl.replace seen name true; name :: acc end
        in
...
let extract_context_from_messages (messages : Types.message list) : string =
  let user_msgs = List.filter (fun (m : Types.message) ->
    m.role = Types.User
  ) messages in
