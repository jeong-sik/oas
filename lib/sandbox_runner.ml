...
let show_sandbox_config c =
  Printf.sprintf "sandbox_config(timeout=%.1fs max_turns=%d max_tool_calls=%d capture=%b)"
    c.timeout_s c.max_turns c.max_tool_calls c.capture_trajectory
...
let check_limits state =
  if state.violation <> None then ()
  else begin
    let elapsed = Unix.gettimeofday () -. state.start_time in
    if elapsed >= state.config.timeout_s then
      state.violation <- Some Timeout
    else if state.turn_count >= state.config.max_turns then
      state.violation <- Some MaxTurns
    else if state.tool_call_count >= state.config.max_tool_calls then
      state.violation <- Some MaxToolCalls
  end
...
let wrap_run_fn state
  ...
  fun prompt ->
    ...
    | None ->
      ...
      if state.config.capture_trajectory && prompt <> "" then
        state.steps <- Trajectory.Think { content = prompt; ts = ts_before }
          :: state.steps;
       ...
       | Ok resp ->
         ...
         List.iter (fun block ->
           ...
           | Types.ToolUse { id; name; input } ->
             ...
             if state.config.capture_trajectory then begin
               ...
               state.steps <- Trajectory.Act { tool_call = tc; ts = ts_before }
                 :: state.steps
             end
           | Types.Text text ->
             if state.config.capture_trajectory && text <> "" then
               state.steps <- Trajectory.Respond { content = text; ts = ts_after }
                 :: state.steps
           | Types.Thinking { content; _ } ->
             if state.config.capture_trajectory && content <> "" then
               state.steps <- Trajectory.Think { content; ts = ts_after }
                 :: state.steps
           | _ -> ()
         ) resp.content;
...
let run ~config ~agent_name ~model ~prompt ~run_fn =
  ...
  let error_msg = match result with
    ...
    | Ok _ -> None
  in
  let error_msg = match state.violation with
    | Some v ->
      Some (Printf.sprintf "%s (violation: %s)"
  ...
  Eval.record collector {
    name = "turn_count"; value = Int_val state.turn_count;
    unit_ = None; tags = [];
  };
  Eval.record collector {
    name = "tool_calls"; value = Int_val state.tool_call_count;
    unit_ = None; tags = [];
  };
  Eval.record collector {
    name = "elapsed_s"; value = Float_val elapsed;
    unit_ = Some "seconds"; tags = [];
  };
  Eval.record collector {
    name = "success"; value = Bool_val success;
    unit_ = None; tags = [];
  };
  ...
  let verdicts =
    ...
    [
      { Harness.passed = budget_ok;
        ...
        evidence = [Printf.sprintf "tool_calls=%d limit=%d"
          state.tool_call_count config.max_tool_calls];
        detail = if budget_ok then None
          else Some "tool call budget exceeded";
      };
      { Harness.passed = turns_ok;
        score = Some (if turns_ok then 1.0 else 0.0);
        evidence = [Printf.sprintf "turns=%d limit=%d"
          state.turn_count config.max_turns];
        detail = if turns_ok then None
          else Some "turn limit exceeded";
      };
      { Harness.passed = time_ok;
        score = Some (if time_ok then 1.0 else 0.0);
        evidence = [Printf.sprintf "elapsed=%.3fs limit=%.1fs"
          elapsed config.timeout_s];
        detail = if time_ok then None
          else Some "timeout exceeded";
      };
    ]
  in
