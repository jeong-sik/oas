...
let save ~path (baseline : baseline) =
  ...
  | Ok () -> Ok ()
  | Error err ->
    Error (Printf.sprintf "Failed to save baseline: %s" (Error.to_string err))
;;
...
let compare ?(tolerance_pct = 5.0) ~baseline ~(current : Eval.run_metrics) () : comparison
  ...
  List.iter
    (fun (m : Eval.metric) -> Hashtbl.replace baseline_map m.name m.value)
    baseline.run_metrics.metrics;
  List.iter
    (fun (m : Eval.metric) -> Hashtbl.replace current_map m.name m.value)
    current.metrics;
  ...
  let diffs =
    Hashtbl.fold
      (fun name bval acc ->
         ...
         | None -> Removed { name; value = bval } :: acc
         | Some cval ->
           (match Eval.metric_value_to_float bval, Eval.metric_value_to_float cval with
            | Some bf, Some cf ->
              if Float.abs bf < 1e-10
              then
                (* Baseline near zero: use absolute comparison *)
                if Float.abs (cf -. bf) < 1e-10
                then Unchanged :: acc
                else if cf > bf
                then
                  Improved
                    { name; baseline_val = bf; current_val = cf; delta_pct = 100.0 }
                  :: acc
                else
                  Regressed
                    { name; baseline_val = bf; current_val = cf; delta_pct = -100.0 }
                ...
                then
                  Regressed { name; baseline_val = bf; current_val = cf; delta_pct }
                  :: acc
                else if delta_pct > tolerance_pct
                then
                  Improved { name; baseline_val = bf; current_val = cf; delta_pct } :: acc
                else Unchanged :: acc)
            | _ ->
              ...
              then Unchanged :: acc
                ...
                Regressed { name; baseline_val = 0.0; current_val = 0.0; delta_pct = 0.0 }
                :: acc))
      baseline_map
      []
  in
  ...
  let diffs =
    Hashtbl.fold
      (fun name cval acc ->
         if not (Hashtbl.mem baseline_map name)
         then Added { name; value = cval } :: acc
         else acc)
      current_map
      diffs
  in
...
let show_diff = function
  ...
  | Regressed r ->
    Printf.sprintf
      ...
      r.current_val
      r.delta_pct
  | Added r -> Printf.sprintf "%s: NEW (%s)" r.name (Eval.show_metric_value r.value)
  | Removed r ->
