...
let summarize results =
  ...
  let pass_rate =
    if evaluated_runs = 0 then 0.0 else Float.of_int passed /. Float.of_int evaluated_runs
  in
  { total; passed; failed; skipped; evaluated_runs; skipped_runs; pass_rate }
;;

let of_results results = { results; summary = summarize results }
  ...
  `Assoc
    ...
    ; ( "detail"
      , match result.detail with
        ...
        | None -> `Null )
    ; ( "response_text"
      , match result.response_text with
        | Some value -> `String value
        | None -> `Null )
    ; ( "raw_trace_path"
      , match result.raw_trace_path with
        | Some value -> `String value
        | None -> `Null )
    ; ( "metrics"
...
let escape_xml text =
  let replace pairs value =
    List.fold_left
      (fun acc (needle, repl) -> Str.global_replace (Str.regexp_string needle) repl acc)
      value
      pairs
  in
  replace [ "&", "&amp;"; "<", "&lt;"; ">", "&gt;"; "\"", "&quot;"; "'", "&apos;" ] text
...
let to_junit_xml (report : t) =
  let testcases =
    ...
    |> List.map (fun (result : case_result) ->
      let base =
        Printf.sprintf
          "<testcase classname=\"harness\" name=\"%s\">"
          (escape_xml result.case_id)
      in
      let closing = "</testcase>" in
      match result.status with
      | Pass -> base ^ closing
      | Skip -> base ^ "<skipped message=\"skipped\" />" ^ closing
      | Fail ->
        let message = escape_xml (Option.value ~default:"failed" result.detail) in
        base
        ^ Printf.sprintf "<failure message=\"%s\">%s</failure>" message message
        ^ closing)
  in
  String.concat
    ""
