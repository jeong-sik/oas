...
let deterministic v =
  { value = v; confidence = 1.0;
    provenance = Deterministic; stress = stress_zero }

let from_user v =
  { value = v; confidence = 1.0;
    provenance = User; stress = stress_zero }
...
let is_deterministic u = match u.provenance with
  | Deterministic -> true
...
let with_confidence c u =
  { u with confidence = clamp_confidence c }
...
let to_result ~min_confidence u =
  if u.confidence >= min_confidence then Ok u.value
  else
    Error (Printf.sprintf
      "confidence %.2f < threshold %.2f (provenance: %s)"
      u.confidence min_confidence (provenance_label u.provenance))
...
let provenance_to_yojson = function
  | LLM { model; temperature } ->
    `Assoc [
      ("kind", `String "llm");
      ("model", `String model);
      ("temperature", match temperature with
        | Some t -> `Float t | None -> `Null);
    ]
  | Heuristic { name } ->
    `Assoc [("kind", `String "heuristic"); ("name", `String name)]
  | Deterministic ->
    `Assoc [("kind", `String "deterministic")]
  | User ->
    `Assoc [("kind", `String "user")]
...
let provenance_of_yojson json =
  ...
  try
    ...
    | "llm" ->
      ...
      let temperature = match json |> member "temperature" with
        | `Float f -> Some f | _ -> None in
      Ok (LLM { model; temperature })
    | "heuristic" ->
      Ok (Heuristic { name = json |> member "name" |> to_string })
...
let stress_to_yojson s =
  `Assoc [
    ("context_pressure", match s.context_pressure with
      | Some p -> `Float p | None -> `Null);
...
let stress_of_yojson json =
  ...
  try
    Ok {
      context_pressure = (match json |> member "context_pressure" with
    ...
    | Ok value ->
      ...
      (match provenance_of_yojson (json |> member "provenance") with
       ...
       | Ok provenance ->
         ...
         | Ok stress ->
           Ok { value; confidence = clamp_confidence confidence;
                provenance; stress })
  with exn -> Error (Printexc.to_string exn)
