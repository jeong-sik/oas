...
type export_config = {
  ...
  flush_interval_sec: float;
      (** Periodic flush interval. Default: 5.0. *)
  max_batch_size: int;
      (** Max spans per export batch. Default: 512. *)
  max_retries: int;
      (** Export retry attempts. Default: 3. *)
  timeout_sec: float;
      (** Per-request timeout. Default: 10.0. *)
}
...
val build_otlp_body :
  service_name:string -> Otel_tracer.span list -> string

(** Split a span list into batches of at most [max_size]. *)
val split_batches :
  int -> Otel_tracer.span list list -> Otel_tracer.span list ->
