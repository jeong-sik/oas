...
let string_of_signals intent signals =
  ...
  | _ ->
    Printf.sprintf "%s: matched %s" (intent_to_string intent) (String.concat ", " signals)
;;
...
let heuristic_classify query =
  ...
  let task_score =
    ...
    then 0.25
    else 0.0 +. if (not asks_question) && len > 0 then 0.05 else 0.0
  in
  let status_score =
  ...
  in
  let total_score = List.fold_left (fun acc (_, score, _) -> acc +. score) 0.0 scored in
  let intent, score, signals =
    if total_score <= 0.0
    then
      if asks_question
      then Knowledge_query, 0.55, [ "question_fallback" ]
      else if short
      then Conversational, 0.60, [ "short_message_fallback" ]
  ...
  in
  let confidence =
    if total_score <= 0.0 then score else clamp_unit (max 0.50 (score /. total_score))
  in
  { intent
