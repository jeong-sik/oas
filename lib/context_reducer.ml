...
let[@warning "-32"] estimate_next_turn_overhead ?system_prompt ?tools ?output_reserve () =
  Context_reducer_estimate.estimate_next_turn_overhead
    ?system_prompt ?tools ?output_reserve ()
...
let apply_repair_dangling_tool_calls = Context_reducer_apply.apply_repair_dangling_tool_calls
let apply_repair_orphaned_tool_results = Context_reducer_apply.apply_repair_orphaned_tool_results
