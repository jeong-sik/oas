    ...
    (match delta with
     | Types.TextDelta s | Types.ThinkingDelta s | Types.InputJsonDelta s ->
       (* [InputJsonDelta] is an incremental tool-argument fragment by
          ...
          JSON objects (deepseek-v4-flash emitting [task_items_list] as
...
let%test "finalize_stream_acc fails closed on multi-object tool args" =
  (* Reproduction of a provider stream where one tool call emitted two
     concatenated JSON objects. The tool block is index 1 (after thinking),
     matching malformed_tool_use_arguments:index:1. Asserts the turn fails
     closed with both objects preserved in [raw], never silently dispatching a
     partial argument object. *)
  let acc = create_stream_acc () in
  List.iter
    (accumulate_event acc)
    ...
    ; Types.ContentBlockStart
        ...
        ; content_type = "tool_use"
        ; tool_id = Some "call_1"
        ; tool_name = Some "task_items_list"
        }
    ; Types.ContentBlockDelta
