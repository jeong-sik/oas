    ...
    (match delta with
     | Types.TextDelta s | Types.ThinkingDelta s | Types.InputJsonDelta s ->
       (* [InputJsonDelta] is an incremental tool-argument fragment by
          ...
          JSON objects (deepseek-v4-flash emitting [task_list] as
...
let%test "finalize_stream_acc fails closed on live multi-object tool args" =
  (* Exact reproduction of the reported provider failure: deepseek-v4-flash
     (Ollama Cloud, OpenAI-compatible) opened a reasoning block (OAS block index 0)
     then emitted ONE task_list call whose arguments string was two concatenated
     ...
     objects preserved in [raw] -- never silently dispatching the tool with only
     the second object's params. *)
  let acc = create_stream_acc () in
  List.iter
    (accumulate_event acc)
