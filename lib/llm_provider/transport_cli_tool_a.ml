(** Agent_code CLI non-interactive transport.
    ...
    Uses [agent_code exec --json] which emits JSONL envelopes on stdout:
    ...
    response.  [command_execution] items are Agent_code's internal shell tool
    ...
    surface them as {!Types.api_usage}: Agent_code CLI is declared
...
type config =
  ...
  ; stdout_idle_timeout_s : float option
    (* When [Some s] and [clock] is [Some _], the agent_code subprocess
...
(* Agent_code CLI 0.125.0+ accepts [mcp_servers.<name>.bearer_token_env_var = "VAR"]
...
let bearer_env_var_name server_name =
  ...
  Buffer.add_string buf "OAS_CLI_TOOL_A_MCP_";
  String.iter
    (fun c ->
...
(** Lightweight structural check used as the [stdout_recovery] hook
    for [Cli_common_subprocess.run_stream_lines].  A successful agent_code
    ...
    the agent_code-cli 0.125.0+ [record_rollout_items] race that surfaces
    ...
    JSONL [type] field in agent_code output. *)
...
(** Threshold at which [build_args] stops passing the prompt as a
    ...
    argv entries.  Env override [OAS_CLI_TOOL_A_PROMPT_ARGV_THRESHOLD]
...
let prompt_argv_threshold () =
  Cli_common_env.int
    ...
    "OAS_CLI_TOOL_A_PROMPT_ARGV_THRESHOLD"
;;
...
(* Agent_code has no dedicated --no-mcp / --no-hooks flags; every runtime
   toggle goes through [-c key=value] TOML overrides (see
   `~/.agent_code/config.toml` schema).  Non-interactive OAS runs default to
   ...
   OAS_CLI_TOOL_A_CONFIG   "k=v,k2=v2"                → -c k=v -c k2=v2
   OAS_CLI_TOOL_A_SANDBOX  read-only|workspace-write  → -s <value>
                      |danger-full-access
   OAS_CLI_TOOL_A_PROFILE  <name>                     → -p <name>
   OAS_CLI_TOOL_A_SKIP_GIT 1                          → --skip-git-repo-check

   Unset → no flag added, preserving the pre-0.159 argv.

   NB: the [mcp_config]/[allowed_tools]/[max_turns]/[permission_mode]
   config fields have never had Agent_code equivalents and are still unused
   here.  Callers wanting those knobs should emit [-c ...] overrides
   via OAS_CLI_TOOL_A_CONFIG. *)
let legacy_env_extra_args () =
  let extras = ref [] in
  let add a = extras := !extras @ a in
  add [ "-c"; "mcp_servers={}" ];
  (match Cli_common_env.kv_pairs "OAS_CLI_TOOL_A_CONFIG" with
   | None | Some [] -> ()
   | Some pairs ->
     List.iter (fun (k, v) -> add [ "-c"; Printf.sprintf "%s=%s" k v ]) pairs);
  (match Cli_common_env.get "OAS_CLI_TOOL_A_SANDBOX" with
   | Some v -> add [ "-s"; v ]
   | None -> ());
  (match Cli_common_env.get "OAS_CLI_TOOL_A_PROFILE" with
   | Some v -> add [ "-p"; v ]
   | None -> ());
  if Cli_common_env.bool "OAS_CLI_TOOL_A_SKIP_GIT" then add [ "--skip-git-repo-check" ];
  !extras
;;
...
(* Returns [Ok (overrides, env_pairs)].  [env_pairs] are name=value pairs that
   the caller must inject into the spawned [agent_code] process's environment so
...
let build_args
  ...
  (* Order: exec-level flags come before the positional prompt.  When the
     prompt is too large for argv, pass "-" so Agent_code reads it from stdin. *)
  let model_args =
    match cli_model_override ~config ~req_config with
    | None -> []
    | Some model -> [ "--model"; model ]
  in
  (* [--ephemeral] suppresses Agent_code CLI session/rollout persistence.  Without
     it, agent_code-cli 0.125.0+ records each invocation under
     [$CODEX_HOME/sessions/], and an asynchronous record_rollout_items step
     can race process exit, surfacing
       [ERROR agent_code_core::session: failed to record rollout items:
     ...
     valid stdout JSONL.  We never call [agent_code resume]/[fork] from this
  ...
  in
  let argv =
    [ config.agent_code_path; "exec"; "--json"; "--ephemeral" ]
    @ mcp_argv
    @ model_args
    @ (match runtime_mcp_policy with
...
(** Extract raw usage from a [turn.completed] envelope. The transport keeps
    this private and does not put it on [api_response.usage] because Agent_code
...
(** Parse a single envelope into zero or more OAS sse_events.
    [command_execution] items do not map to an OAS concept — Agent_code runs
...
let events_of_line_with_state (state : stream_state) line =
  try
    ...
    | "thread.started" ->
      ...
      [ Types.MessageStart { id; model = "agent_code"; usage = None } ]
    | "item.started" ->
      let item = Yojson.Safe.Util.member "item" json in
      let item_type = Cli_common_json.member_str "type" item in
...
(** Aggregate JSONL envelopes into an [api_response].  Only
    ...
    because Agent_code CLI usage is not a stable per-response contract.
    [command_execution] items are counted as provider-native internal
    actions rather than surfaced as OAS tool calls. *)
let parse_jsonl_result ?(model_id = "agent_code") ?(prompt = "") lines =
  ...
  then
    Error
      (Http_client.NetworkError
         { message = "no events parsed from agent_code output"; kind = Unknown })
  else
    Ok
      { Types.id = !thread_id
      ; model = model_id
...
(* Fires once per transport instance when any Agent_llm_a-only config field
   is set.  Agent_code CLI has no flag for these yet, so we warn and drop. *)
...
(** Strip API-key env so [agent_code] uses its own session/auth rather than
    inherited [PROVIDER_D_API_KEY].  Mirrors the rationale in
    {!Transport_cli_tool_d.agent_llm_a_cli_scrub_env}. *)
let agent_code_cli_scrub_env = [ "PROVIDER_D_API_KEY" ]
        ...
        | None ->
          ...
          (match
             Cli_common_subprocess.run_stream_lines
               ...
               ~scrub_env:agent_code_cli_scrub_env
...
let agent_code_req ?(model_id = "auto") () =
  Provider_config.make ~kind:Provider_config.Cli_tool_a ~model_id ~base_url:"" ()
;;

let%test "default_config agent_code_path" = default_config.agent_code_path = "codex"
...
let%test "build_args includes --ephemeral right after --json" =
  (* Regression guard: --ephemeral must precede config overrides and the
     prompt so agent_code-cli 0.125.0+ skips session/rollout persistence even
...
let%test "build_args uses stdin sentinel when prompt is too large" =
  ...
  let args, _ =
    build_args ~config:default_config ~req_config:(agent_code_req ()) ~prompt:big ()
  in
...
let%test "bearer_env_var_name uppercases and sanitises" =
  bearer_env_var_name "context7" = "OAS_CLI_TOOL_A_MCP_CONTEXT7_BEARER"
  && bearer_env_var_name "my-server" = "OAS_CLI_TOOL_A_MCP_MY_SERVER_BEARER"
  && bearer_env_var_name "x.y.z" = "OAS_CLI_TOOL_A_MCP_X_Y_Z_BEARER"
;;
...
let%test "runtime_mcp_overrides redirects Bearer to env var indirection" =
  ...
  | Error _ -> false
  | Ok (overrides, env_pairs) ->
    let env_var = "OAS_CLI_TOOL_A_MCP_CONTEXT7_BEARER" in
...
let%test "events_of_line thread.started → MessageStart" =
  ...
  | [ Types.MessageStart { id = "abc"; model = "agent_code"; _ } ] -> true
...
let%test "default: argv disables MCP even with no env" =
  with_unset "OAS_CLI_TOOL_A_CONFIG" (fun () ->
    with_unset "OAS_CLI_TOOL_A_SANDBOX" (fun () ->
...
let%test "env: OAS_CLI_TOOL_A_CONFIG emits -c pairs before prompt" =
  with_env "OAS_CLI_TOOL_A_CONFIG" "mcp_servers={},model=o3" (fun () ->
    let args, _ =
...
let%test "env: OAS_CLI_TOOL_A_SANDBOX and OAS_CLI_TOOL_A_SKIP_GIT" =
  with_env "OAS_CLI_TOOL_A_SANDBOX" "read-only" (fun () ->
    with_env "OAS_CLI_TOOL_A_SKIP_GIT" "true" (fun () ->
      let args, _ =
        build_args ~config:default_config ~req_config:(agent_code_req ()) ~prompt:"hi" ()
      in
      List.mem "-s" args
...
let%test "prompt_exceeds_argv_budget: OAS_CLI_TOOL_A_PROMPT_ARGV_THRESHOLD override" =
  with_env "OAS_CLI_TOOL_A_PROMPT_ARGV_THRESHOLD" "100" (fun () ->
...
(* Regression guard: HTTP MCP servers carrying [Authorization: Bearer X] must
   redirect the token through Agent_code CLI's [bearer_token_env_var] so it never
...
let%test "build_args runtime MCP routes Bearer through env var indirection" =
  ...
  && List.mem
       "mcp_servers.agent_tools.bearer_token_env_var=\"OAS_CLI_TOOL_A_MCP_AGENT_TOOLS_BEARER\""
       args
  ...
  && env = [ "OAS_CLI_TOOL_A_MCP_AGENT_TOOLS_BEARER", token ]
;;
