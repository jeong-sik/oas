...
let strip_markdown_fence (s : string) : string =
  ...
  if len < 6 then s
    ...
    let starts_with_fence =
      len >= 3 && s.[0] = '`' && s.[1] = '`' && s.[2] = '`'
    in
    if not starts_with_fence then s
    else
      ...
      in
      if not ends_with_fence then s
      else
        let inner_start = first_newline + 1 in
        let inner_end = len - 3 in
        if inner_start >= inner_end then s
        else
          String.trim (String.sub s inner_start (inner_end - inner_start))
...
let unwrap_double_stringify (s : string) : string =
  ...
  if len < 4 then s
    ...
    | `String inner ->
      ...
      if String.length inner_trimmed > 0
         && (inner_trimmed.[0] = '{' || inner_trimmed.[0] = '[') then
...
let strip_trailing_commas (s : string) : string =
  ...
  for i = 0 to len - 1 do
    ...
    if !escape then begin
      ...
      escape := false
    end else if c = '\\' && !in_string then begin
      Buffer.add_char buf c;
      escape := true
    end else if c = '"' then begin
      in_string := not !in_string;
      Buffer.add_char buf c
    end else if c = ',' && not !in_string then begin
  ...
  if len < 2 then s
    ...
    List.iter (fun (prefix, full) ->
      ...
      if plen <= len
         ...
         && (len = plen || let c = trimmed.[len - plen - 1] in
             c = ':' || c = ' ' || c = '\t' || c = ',' || c = '[')
      then
        result := String.sub trimmed 0 (len - plen) ^ full
    ) completions;
    !result
...
let close_brackets (s : string) : string =
  ...
  String.iter (fun c ->
    if !escape then escape := false
      ...
      if c = '{' then Stack.push '}' stack
      else if c = '[' then Stack.push ']' stack
      else if (c = '}' || c = ']') && not (Stack.is_empty stack) then
        ignore (Stack.pop stack)
    end
  ) s;
  if Stack.is_empty stack then s
  else begin
...
let parse (s : string) : Yojson.Safe.t =
  ...
  let maybe_unwrap_string = function
    | `String inner ->
      ...
      if String.length t > 0 && (t.[0] = '{' || t.[0] = '[') then
