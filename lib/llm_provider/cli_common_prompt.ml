...
let prompt_of_messages ?(include_tool_blocks = false) (messages : Types.message list) =
  let render = if include_tool_blocks then render_block_with_tools else text_of_block in
...
let system_prompt_of ~(req_config : Provider_config.t) (messages : Types.message list) =
...
let prompt_with_system_prompt ~prompt ~system_prompt =
  ...
  | Some sp ->
    ...
    if prompt = ""
    then Printf.sprintf "System:\n%s" sp
    else Printf.sprintf "System:\n%s\n\n%s" sp prompt
;;
...
let%test "prompt_of_messages empty" = prompt_of_messages [] = ""
...
let%test "prompt_of_messages multi-turn has context" =
  ...
  String.length result > 0 && result |> String.split_on_char '\n' |> List.length > 1
;;

let%test "non_system_messages drops system" =
  let filtered =
    non_system_messages [ msg System [ Text "be helpful" ]; msg User [ Text "hi" ] ]
  in
...
let%test "system_prompt_of prefers explicit req_config" =
  let req =
    Provider_config.make
      ...
      ()
  in
  system_prompt_of ~req_config:req [ msg System [ Text "from msg" ] ] = Some "explicit"
;;

let%test "system_prompt_of falls back to system message" =
  let req = Provider_config.make ~kind:Anthropic ~model_id:"" ~base_url:"" () in
...
let%test "prompt_of_messages default drops tool blocks" =
  let msgs =
    [ msg
        Assistant
        [ ToolUse { id = "tu_1"; name = "calc"; input = `Assoc [ "x", `Int 1 ] } ]
    ; msg
        User
        [ ToolResult
            { tool_use_id = "tu_1"; content = "2"; is_error = false; json = None }
        ]
...
let%test "prompt_of_messages with include_tool_blocks renders tool_result error" =
  let msgs =
    [ msg
        User
        [ ToolResult
            { tool_use_id = "tu_1"; content = "boom"; is_error = true; json = None }
        ]
    ; msg User [ Text "retry" ]
    ]
  in
...
let%test "render_block_with_tools drops thinking" =
  match
    render_block_with_tools
      (Types.Thinking { thinking_type = "thinking"; content = "internal" })
  with
  | None -> true
  | Some _ -> false
;;
