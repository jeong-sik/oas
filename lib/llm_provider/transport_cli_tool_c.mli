(** Provider_c CLI non-interactive transport.
    ...
    Implements {!Llm_transport.t} by spawning [provider_c --print]
...
type config =
  ...
  ; config_file : string option
    (** Path to a Provider_c CLI config file passed as [--config-file].
  ...
  ; mcp_config_files : string list
    (** Additional [--mcp-config-file] paths. Empty preserves the CLI's
        default MCP discovery (including [~/.provider_c/mcp.json] when it
        exists). *)
  ...
  ; extra_env : (string * string) list
    (** Additional environment variables injected into the [provider_c]
        subprocess. Default [[]]. *)
  ; cancel : unit Eio.Promise.t option
    (** When [Some p] and [p] resolves mid-run, the [provider_c] subprocess
        receives [SIGINT] via [Eio.Process.signal].
        Default [None]. *)
  ; session_id : string option
    (** When [Some id], the transport passes [--session id] on each call.
        Provider_c CLI creates that session if absent and resumes it if present.
  ...
  ; stdout_idle_timeout_s : float option
    (** When [Some s] and [clock] is [Some _], the [provider_c]
        ...
        RFC-0022 attempt liveness) and oas PRs #1458 (cli_tool_a) /
        #1459 (cli_tool_d).
        Default [None].
...
(** Sensible defaults: [provider_c] in PATH, [provider_c-for-coding], no explicit
    MCP overrides. *)
val default_config : config

(** Create a Provider_c CLI transport.

    The returned {!Llm_transport.t} spawns a fresh [provider_c --print]
    ...
    prompt string; Provider_c's stream-JSON output is then re-expanded into
