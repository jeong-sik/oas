...
type config =
  ...
  ; yolo : bool
  ; cwd : string option
  ; (* Fields below are accepted for parity with the Agent_llm_a Code config
     so callers can target multiple CLI backends with the same
     structure.  As of Provider_f CLI 0.8+, real flags exist for
  ...
  ; stdout_idle_timeout_s : float option
    (* When [Some s] and [clock] is [Some _], the provider_f subprocess
...
(* Non-interactive Provider_f runs default to MCP OFF by passing a sentinel
   MCP whitelist that will never match a real server.  Explicit
   allow-lists can opt back in.

   OAS_CLI_TOOL_B_ALLOWED_MCP    "a,b" → --allowed-mcp-server-names a
   ...
   OAS_CLI_TOOL_B_EXTENSIONS     "a,b" → -e a -e b

   Provider_f CLI 0.38+ PolicyEngine rejects empty strings in
...
let env_extra_args () =
  ...
  if Cli_common_env.bool "OAS_CLI_TOOL_B_NO_MCP"
  then add [ "--allowed-mcp-server-names"; no_mcp_sentinel ]
  else (
    match Cli_common_env.list "OAS_CLI_TOOL_B_ALLOWED_MCP" with
    | None | Some [] -> add [ "--allowed-mcp-server-names"; no_mcp_sentinel ]
    | Some names -> List.iter (fun n -> add [ "--allowed-mcp-server-names"; n ]) names);
  (match Cli_common_env.list "OAS_CLI_TOOL_B_EXTENSIONS" with
   | None | Some [] -> ()
   | Some names -> List.iter (fun n -> add [ "-e"; n ]) names);
  (match Cli_common_env.list "OAS_CLI_TOOL_B_ADMIN_POLICY" with
   | None | Some [] -> ()
   | Some names -> List.iter (fun n -> add [ "--admin-policy"; n ]) names);
  (match Cli_common_env.list "OAS_CLI_TOOL_B_POLICY" with
   | None | Some [] -> ()
   | Some names -> List.iter (fun n -> add [ "--policy"; n ]) names);
  !extras
;;

(* Provider_f CLI (>=0.38) has no dedicated system-prompt flag — earlier
...
let build_args ~(config : config) ~(req_config : Provider_config.t) ~prompt ~system_prompt
  ...
  let args = ref [ config.provider_f_path; "--output-format"; "json"; "-p"; prompt ] in
  ...
  (match Cli_common_env.get "OAS_CLI_TOOL_B_APPROVAL_MODE" with
...
(** Parse and aggregate usage across [stats.models] entries in a Provider_f
    ...
    "stats": {
      "models": {
        "provider_f-2.5-flash-lite": {
          "tokens": { "input": N, "candidates": N, "cached": N }
        },
        "provider_f-3-flash-preview": {
          "tokens": { ... }
        }
      }
    }
...
(** Parse the Provider_f CLI [--output-format json] result into an
...
(** Strip API-key env so [provider_f] uses the user's Google OAuth session
    rather than a metered API key.  Mirrors the rationale in
    {!Transport_cli_tool_d.agent_llm_a_cli_scrub_env}. *)
let provider_f_cli_scrub_env =
  [ "PROVIDER_F_API_KEY"; "GOOGLE_API_KEY"; "GOOGLE_APPLICATION_CREDENTIALS" ]
;;

(* Stderr markers the provider_f CLI emits when it is looping internally on
     ...
     | Some (Http_client.Hard_quota _ as kind) ->
       Error
         (Http_client.ProviderFailure
            { kind
            ; message =
                "cli_tool_b reported terminal quota exhaustion; rejecting without \
...
let run ~sw ~mgr ~(config : config) ?model argv =
  let fail_fast_enabled =
    not (Cli_common_env.bool "OAS_CLI_TOOL_B_CLI_NO_FAIL_FAST_ON_CAPACITY")
  in
  ...
  let result =
    Cli_common_subprocess.run_collect
      ...
      ~cwd:config.cwd
      ~extra_env:[]
      ~scrub_env:provider_f_cli_scrub_env
      ~on_stderr_line
  ...
  | _, Some kind ->
    Error
      (Http_client.ProviderFailure
         { kind
         ; message =
             "cli_tool_b stderr reported a provider/runtime failure; aborted early so \
...
(* Fires once per transport instance when any Agent_llm_a-only config field
   is set.  Provider_f CLI has no flag for these yet, so we warn and drop. *)
          ...
          { Llm_transport.response =
              Error
                (Http_client.ProviderFailure
                   ...
                   ; message =
                       "cli_tool_b does not support request-scoped runtime MCP \
                        configuration"
                   })
          ; latency_ms = None
          }
            ...
            (Http_client.ProviderFailure
               { kind =
                   Http_client.Capability_mismatch
                     { capability = Some "request_scoped_runtime_mcp" }
               ; message =
                   "cli_tool_b does not support request-scoped runtime MCP configuration"
               })
        | None ->
          warn_unsupported_once config warned;
          let messages = Cli_common_prompt.non_system_messages req.messages in
          ...
          let argv = build_args ~config ~req_config:req.config ~prompt ~system_prompt in
          let model = selected_model ~config ~req_config:req.config in
          (* Provider_f CLI does not support native streaming; replay synthetic events
...
let%test "default_config provider_f_path" = default_config.provider_f_path = "gemini"
...
let%test "build_args basic with yolo" =
  let args =
    build_args
      ...
      ~req_config:(Provider_config.make ~kind:Cli_tool_d ~model_id:"" ~base_url:"" ())
      ~prompt:"hello"
      ~system_prompt:None
  in
...
let%test "build_args with model" =
  ...
  in
  List.mem "--model" args
  && List.mem "provider_f-2.5-pro" args
  (* Provider_f CLI has no --system-prompt flag; system text is folded into
...
let%test "parse_json_result success (single model stats)" =
  let json =
    {|{"session_id":"sid","response":"hello world","stats":{"models":{"provider_f-2.5-flash":{"tokens":{"input":10,"candidates":5,"cached":2}}}}}|}
  in
...
let%test "classify_cli_error reclassifies provider_f startup crash as AcceptRejected" =
  let err =
    Error
      (Http_client.NetworkError
         { message =
             "provider_f exited with code 13: Warning: Detected unsettled top-level await\n\
...
let%test "classify_cli_error keeps unrelated network failures retryable" =
  let err =
    Error
      (Http_client.NetworkError
         { message = "provider_f exited with code 1: connection refused"; kind = Unknown })
  in
  match classify_cli_error err with
  | Error (Http_client.NetworkError { message; _ }) ->
...
let provider_f_req = Provider_config.make ~kind:Cli_tool_b ~model_id:"" ~base_url:"" ()

let%test "env: approval-mode supersedes config.yolo" =
  with_env "OAS_CLI_TOOL_B_APPROVAL_MODE" "plan" (fun () ->
    let args =
      build_args
        ~config:default_config
        ~req_config:provider_f_req
        ~prompt:"hi"
        ~system_prompt:None
    in
...
let%test "env: OAS_CLI_TOOL_B_NO_MCP disables all MCP via sentinel whitelist" =
  with_env "OAS_CLI_TOOL_B_NO_MCP" "1" (fun () ->
    let args =
      build_args
        ~config:default_config
        ~req_config:provider_f_req
...
let%test "env: OAS_CLI_TOOL_B_ALLOWED_MCP whitelist" =
  with_env "OAS_CLI_TOOL_B_ALLOWED_MCP" "alpha,beta" (fun () ->
    let args =
      build_args
        ~config:default_config
        ~req_config:provider_f_req
...
let%test "env: OAS_CLI_TOOL_B_EXTENSIONS splits on comma" =
  with_env "OAS_CLI_TOOL_B_EXTENSIONS" "ext-a,ext-b" (fun () ->
    let args =
      build_args
        ~config:default_config
        ~req_config:provider_f_req
...
let%test "default: no vars still keeps MCP disabled via sentinel" =
  with_unset "OAS_CLI_TOOL_B_ALLOWED_MCP" (fun () ->
    with_unset "OAS_CLI_TOOL_B_APPROVAL_MODE" (fun () ->
      with_unset "OAS_CLI_TOOL_B_EXTENSIONS" (fun () ->
        with_unset "OAS_CLI_TOOL_B_NO_MCP" (fun () ->
          let args =
            build_args
              ~config:default_config
              ~req_config:provider_f_req
