(** Provider_c CLI non-interactive transport.
...
type config =
  ...
  ; stdout_idle_timeout_s : float option
    (* When [Some s] and [clock] is [Some _], the provider_c subprocess
...
let default_config =
  { provider_c_path = "kimi"
  ; model = Some "provider_c-for-coding"
  ; cwd = None
  ; config_file = None
  ; mcp_config_files = []
  ; mcp_config_json = []
...
(* Provider_c CLI imports setproctitle before it starts the request. On macOS,
...
let prompt_argv_threshold () =
  Cli_common_env.int
    ...
    "OAS_CLI_TOOL_C_PROMPT_ARGV_THRESHOLD"
;;

let prompt_exceeds_argv_budget prompt = String.length prompt >= prompt_argv_threshold ()
let sanitize_for_provider_c prompt = Utf8_sanitize.sanitize prompt
...
let cli_model_override ~(config : config) ~(req_config : Provider_config.t) =
  ...
  in
  (match override, Capabilities.provider_c_cli_capabilities.supported_models with
...
let response_id_of_lines lines =
  ...
  in
  List.find_map find_id lines |> Option.value ~default:"provider_c-print"
;;
  ...
  then
    Error
      (Http_client.NetworkError
         { message = "no messages parsed from provider_c output"; kind = Unknown })
  else (
    let response_text =
      List.filter_map
        (function
...
let exit_code_of_message message =
  let prefix = "provider_c exited with code " in
     ...
     | Some 1 ->
       Error
         (Http_client.AcceptRejected
            { reason =
                "cli_tool_c rejected the request (exit 1). "
...
let warn_external_tools_once warned tools =
  ...
  then ()
    ...
    warned := true;
    Diag.warn
      "transport_provider_c_cli"
      "cli_tool_c print mode ignores OAS req.tools. Provider-native built-in tools and \
...
let provider_c_req ?(model_id = "auto") ?enable_thinking () =
  Provider_config.make
    ~kind:Provider_config.Cli_tool_c
    ~model_id
    ~base_url:""
    ?enable_thinking
    ()
...
let%test "default_config uses provider_c-for-coding" =
  default_config.model = Some "provider_c-for-coding"
;;
...
let%test "build_args with work dir, mcp, and thinking" =
  ...
  in
  let args =
    build_args ~config ~req_config:(provider_c_req ~enable_thinking:true ()) ~prompt:"hi"
  in
...
let%test "build_args uses request model over default" =
  let args =
    build_args
      ...
      ~req_config:(provider_c_req ~model_id:"provider_c-k2.6" ())
      ~prompt:"hi"
  in
  List.mem "--model" args
  && List.mem "provider_c-k2.6" args
  && not (List.mem "provider_c-for-coding" args)
;;
...
let%test "build_args adds session id when configured" =
  ...
  let args = build_args ~config ~req_config:(provider_c_req ()) ~prompt:"next" in
  List.mem "--session" args && List.mem "sess-abc" args
;;

let%test "build_args uses config-file flag for config_file" =
  let config = { default_config with config_file = Some "/tmp/provider_c.toml" } in
  let args = build_args ~config ~req_config:(provider_c_req ()) ~prompt:"first" in
  List.mem "--config-file" args
  && List.mem "/tmp/provider_c.toml" args
...
let%test "parse_jsonl_result restores tool trace" =
  ...
  in
  match parse_jsonl_result ~model_id:"provider_c-for-coding" ~prompt:"" lines with
  | Ok resp ->
...
let%test "classify_cli_error exit 1 becomes AcceptRejected" =
  match
    classify_cli_error
      (Error
         (Http_client.NetworkError
            { message = "provider_c exited with code 1: auth failed"; kind = Unknown }))
  with
