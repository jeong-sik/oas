...
let create ~max_concurrent ~provider_name =
  ...
  then
    invalid_arg
      (Printf.sprintf
         "Provider_throttle.create: max_concurrent must be >= 1, got %d"
         max_concurrent);
...
let available t = Slot_scheduler.available t.scheduler
let in_use t = Slot_scheduler.in_use t.scheduler

let create_with_source
      ~max_concurrent
      ~provider_name
      ~source
      ?(yield_cap = Replay_yield)
...
let of_discovery_status (status : Discovery.endpoint_status) =
  ...
  | Some s when s.total > 0 ->
    Some
      (create_with_source
         ~max_concurrent:s.total
         ~provider_name:status.url
         ~source:Discovered
         ~yield_cap:Explicit_slot_yield
         ())
  | _ ->
...
let default_for_kind (kind : Provider_config.provider_kind) =
  ...
  | Provider_config.Claude_code -> create ~max_concurrent:2 ~provider_name:"claude_code"
...
let acquire_permit ~priority t = Slot_scheduler.acquire_permit ~priority t.scheduler
let yield_permit t permit = Slot_scheduler.yield_permit t.scheduler permit
let resume_permit t permit = Slot_scheduler.resume_permit t.scheduler permit
let release_permit t permit = Slot_scheduler.release_permit t.scheduler permit
...
let%test "with_permit_timeout releases on exception" =
  Eio_main.run (fun env ->
    let clock = Eio.Stdenv.clock env in
    let t = create ~max_concurrent:2 ~provider_name:"test" in
    (try with_permit_timeout clock ~timeout_sec:5.0 t (fun () -> failwith "boom") with
     | Failure _ -> ());
    available t = 2)
;;
