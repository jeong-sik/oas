(** Provider-neutral, exact structured-output Single Surface.
    ...
    The caller supplies only an OAS catalog selector, an optional exact model,
    messages, a raw domain JSON schema, and the minimum guarantee. Provider
...
(** Resolve one exact OAS-owned provider binding and model. Unknown selectors
    fail closed; no provider, endpoint, model, or capability is inferred. *)
val select_target
  :  selector:string
  -> ?model:string
  -> unit
