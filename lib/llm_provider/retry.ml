...
let safe_prefix (body : string) ~(max_len : int) : string =
  if max_len <= 0
  then ""
  else if String.length body <= max_len
  then body
...
let extract_error_message (body : string) : string =
  ...
  else safe_prefix body ~max_len:overflow_message_scan_limit
;;
...
let parse_context_overflow_limit (body : string) : int option =
  ...
  let r1 =
    ...
    | true ->
      ...
      let rec find_pos i =
        if i + String.length anchor1 > String.length msg
        then None
        else if String.sub msg i (String.length anchor1) = anchor1
        then int_run_from msg (i + String.length anchor1) |> Option.map fst
        else find_pos (i + 1)
      in
      find_pos 0
  in
  ...
  | None ->
    ...
    let rec find_pos i =
      if i + String.length anchor2 > String.length msg
      then None
      else if String.sub msg i (String.length anchor2) = anchor2
      then (
        ...
        | Some (_, used_end) ->
          ...
          if slash_idx >= String.length msg || msg.[slash_idx] <> '/'
          then None
          else (
...
let classify_error ~status ~body : api_error =
  ...
  | 429 ->
    ...
    let retry_after =
      if is_hard_quota_message message then None else parsed_retry_after
    in
...
let calculate_delay config attempt =
  let base_delay =
    config.initial_delay *. (config.backoff_factor ** Float.of_int attempt)
  in
  let capped = Float.min base_delay config.max_delay in
  let jitter =
    Constants.Structured_retry.jitter_min
    +. Random.float Constants.Structured_retry.jitter_range
  in
  capped *. jitter
;;
...
let%test "is_context_overflow_message detects raw message" =
  is_context_overflow_message
    "Invalid request: request (11447 tokens) exceeds the available context size (8192 \
...
let%test "parse_context_overflow_limit: json body" =
  parse_context_overflow_limit {|{"error":{"message":"available context size (32768)"}}|}
  = Some 32768
;;
...
let%test "parse_context_overflow_limit: non-overflow returns None" =
  parse_context_overflow_limit {|{"error":{"message":"bad tool schema"}}|} = None
;;
...
let%test "RateLimited transient 429 is retryable" =
  is_retryable
    (RateLimited
       { retry_after = Some 1.0; message = "Too many requests, please slow down" })
;;
  ...
  not
    (is_retryable
       (RateLimited
          { retry_after = None
          ; message =
              "{\"type\":\"insufficient_quota\",\"details\":\"monthly quota reached\"}"
          }))
;;
...
let%test "RateLimited CJK yuebu buzu is NOT retryable" =
  not (is_retryable (RateLimited { retry_after = None; message = "余额不足，请充值后重试" }))
;;

let%test "RateLimited CJK edu buzu is NOT retryable" =
  not (is_retryable (RateLimited { retry_after = None; message = "额度不足，本月配额已用完" }))
;;
...
let%test "RateLimited benign please recharge context is retryable" =
  ...
  is_retryable
    (RateLimited
       { retry_after = Some 2.0; message = "please recharge your OAuth token and retry" })
;;
...
let%test "is_hard_quota RateLimited positive" =
  is_hard_quota (RateLimited { retry_after = None; message = "Insufficient balance" })
;;

let%test "is_hard_quota RateLimited transient is false" =
  not
    (is_hard_quota
       (RateLimited { retry_after = Some 1.0; message = "rate limit, retry in 1s" }))
;;
