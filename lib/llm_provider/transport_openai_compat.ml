...
let merge_config ~(transport_cfg : config)
    (req_cfg : Provider_config.t) : Provider_config.t =
  let base_url =
    if req_cfg.base_url <> "" then req_cfg.base_url
    else transport_cfg.base_url
  in
  let api_key =
    if req_cfg.api_key <> "" then req_cfg.api_key
    else transport_cfg.api_key
  in
  let model_id =
    if req_cfg.model_id <> "" then req_cfg.model_id
    else transport_cfg.model_id
  in
  let request_path =
    if req_cfg.request_path <> "" then req_cfg.request_path
...
let%test "merge_config uses transport base_url when req empty" =
  ...
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"" ~base_url:"" () in
...
let%test "merge_config preserves req base_url when present" =
  let transport_cfg = { default_config with base_url = "http://myserver:9000" } in
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"" ~base_url:"http://override:8080" () in
  let merged = merge_config ~transport_cfg req_cfg in
...
let%test "merge_config preserves req model_id when present" =
  let transport_cfg = { default_config with model_id = "qwen3.5-35b" } in
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"gpt-4o" ~base_url:"" () in
  let merged = merge_config ~transport_cfg req_cfg in
...
let%test "merge_config uses transport api_key when req empty" =
  ...
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"m" ~base_url:"" () in
  let merged = merge_config ~transport_cfg req_cfg in
...
let%test "merge_config uses transport request_path when req explicitly empty" =
  let transport_cfg = { default_config with request_path = "/api/v2/chat" } in
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"m" ~base_url:""
    ~request_path:"" () in
  let merged = merge_config ~transport_cfg req_cfg in
...
let%test "merge_config sets kind to OpenAI_compat regardless" =
  ...
  let req_cfg = Provider_config.make
    ~kind:Anthropic ~model_id:"" ~base_url:"" () in
  let merged = merge_config ~transport_cfg req_cfg in
  merged.kind = Provider_config.OpenAI_compat

let%test "merge_config appends extra_headers to explicit req headers" =
  let transport_cfg = { default_config with
    extra_headers = [("X-Custom", "value")] } in
  let req_cfg = Provider_config.make
    ~kind:OpenAI_compat ~model_id:"m" ~base_url:""
    ~headers:[("Authorization", "Bearer sk")] () in
  let merged = merge_config ~transport_cfg req_cfg in
