...
let parse_judge_response text =
  ...
  if String.uppercase_ascii trimmed = "PASS" then
    Ok (true, "pass")
  else
    let prefix = "FAIL" in
    ...
    then
      let reason =
        if String.length trimmed > plen + 1 then
          String.trim (String.sub trimmed (plen + 1)
                         (String.length trimmed - plen - 1))
          |> (fun s ->
...
let%test "parse_judge_response PASS" =
