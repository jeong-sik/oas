...
let metric_schema ?(metric_name = "score") () : Metric_contract.metric Structured.schema =
  { name = "report_metric";
    description = Printf.sprintf
      ...
      metric_name;
    params = [
      { name = "name"; param_type = String; description = "Metric name"; required = true };
      { name = "value"; param_type = Number; description = "Metric value (finite decimal)"; required = true };
    ];
    parse = (fun json ->
...
let extract_first_float (text : string) : float option =
  ...
  let rec scan i =
    if i >= len then None
      ...
      let c = text.[i] in
      if (c >= '0' && c <= '9') || (c = '.' && i + 1 < len && text.[i+1] >= '0' && text.[i+1] <= '9') then
...
let parse_metric_from_text ?expected_name (text : string)
  ...
  | Error _ ->
    ...
    let cleaned = text
      |> Llm_provider.Lenient_json.strip_markdown_fence
      |> String.trim
    in
...
let extract_metric_from_response ?(metric_name = "score")
...
let%test "extract_first_float: plain float" =
  extract_first_float "0.75" = Some 0.75

let%test "extract_first_float: with prefix" =
  extract_first_float "Score: 0.85" = Some 0.85

let%test "extract_first_float: percentage" =
  extract_first_float "75%" = Some 0.75

let%test "extract_first_float: integer" =
  extract_first_float "1" = Some 1.0

let%test "extract_first_float: embedded in text" =
  extract_first_float "The result is 0.42 which is good" = Some 0.42

let%test "extract_first_float: no number" =
  extract_first_float "no numbers here" = None
