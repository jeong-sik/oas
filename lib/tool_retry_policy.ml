...
let summary_of_failures failures =
  failures
  |> List.map (fun failure -> Printf.sprintf "- %s: %s" failure.tool_name failure.detail)
...
let decide ~policy ~prior_retries failures =
  ...
  let retryable = failures |> List.filter (failure_enabled policy) in
  ...
  | _ ->
    ...
    then Exhausted { attempts = prior_retries; limit = max_retries; summary }
