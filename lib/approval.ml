...
let create stages = { stages; log = Log.create ~module_name:"approval" () }
...
let evaluate t ~tool_name ~input ~agent_name ~turn =
  ...
  let rec go ctx_acc = function
    | [] ->
      Log.debug t.log "all stages passed, auto-approving" [ Log.S ("tool", tool_name) ];
      Hooks.Approve
    | stage :: rest ->
      ...
      (match result with
       | Decided decision ->
         ...
         decision
       | Pass -> go ctx_acc rest
       | Pass_with_context updated_ctx -> go updated_ctx rest)
  in
  go ctx t.stages
;;
...
let as_callback t : Hooks.approval_callback =
  fun ~tool_name ~input -> evaluate t ~tool_name ~input ~agent_name:"" ~turn:0
;;
...
let auto_approve_known_tools (known : string list) : approval_stage =
  ...
  ; evaluate =
      (fun ctx -> if List.mem ctx.tool_name known then Decided Hooks.Approve else Pass)
  ; timeout_s = None
  }
;;
...
let reject_dangerous_patterns (patterns : (string * string) list) : approval_stage =
  ...
  ; evaluate =
      (fun ctx ->
        let matches =
          List.exists
            (fun (name_pat, input_pat) ->
               let name_match = Util.regex_match (Str.regexp name_pat) ctx.tool_name in
        ...
        then
          Decided
            (Hooks.Reject
               (Printf.sprintf "tool '%s' matched dangerous pattern" ctx.tool_name))
        else Pass)
  ; timeout_s = None
  }
;;
