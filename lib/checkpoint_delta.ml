...
let apply_delta base delta =
  if delta.delta_version <> delta_version then
  ...
  else if Checkpoint_codec.checkpoint_hash base <> delta.base_checkpoint_hash then
    Error
      (Error.Io
         (ValidationFailed
    ...
    Result.bind result (fun checkpoint ->
        if Checkpoint_codec.checkpoint_hash checkpoint <> delta.result_checkpoint_hash then
          Error
            (Error.Io
               (ValidationFailed
