...
let metadata_assoc key metadata =
...
let upsert_metadata key value metadata =
...
let record_failure mem (record : failure_record) =
  ...
  let episode_metadata =
    ...
    @ optional_metadata (Option.map (fun value -> ("stdout", Util.clip value 1000)) record.stdout)
    @ optional_metadata (Option.map (fun value -> ("stderr", Util.clip value 1000)) record.stderr)
    @ optional_metadata (Option.map (fun value -> ("diff_summary", value)) record.diff_summary)
    @ optional_metadata (Option.map (fun value -> ("trace_summary", value)) record.trace_summary)
    @ optional_metadata (Option.map (fun value -> ("metric_name", value)) record.metric_name)
    @ optional_metadata (Option.map (fun value -> ("metric_error", value)) record.metric_error)
    @ record.metadata
  in
  ...
  let action_text =
    Option.value ~default:record.summary record.action
  in
  let updated_proc =
...
let recent_failures_for_procedure mem proc_id =
  Memory.recall_episodes mem ~limit:2
    ~filter:(fun episode ->
  ...
  |> (fun (procedures : Memory.procedure list) ->
       let rec take n acc = function
         ...
         | proc :: rest ->
           take (n - 1)
             ({ procedure = proc;
                recent_failures = recent_failures_for_procedure mem proc.id; } :: acc)
             rest
       in
...
let render_prompt_context lessons =
  ...
  | _ ->
    let sections =
      List.mapi (fun idx (lesson : lesson) ->
        let failure_lines =
...
let%test "retrieve_lessons returns recent failures" =
  ...
  | [{ procedure; recent_failures }] ->
    procedure.pattern = "improve metric"
    && List.length recent_failures = 1
  | _ -> false

let%test "render_prompt_context empty" =
  render_prompt_context [] = None
