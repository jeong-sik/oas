...
let check_budget (config : Types.agent_config) (usage : Types.usage_stats) =
  ...
  | Some limit when usage.estimated_cost_usd > limit ->
    Some (Error.Agent (CostBudgetExceeded {
...
let report_to_string (r : cost_report) : string =
  Printf.sprintf
    "Cost: $%.6f (%d calls, avg $%.6f/call) | Tokens: %d in, %d out (cache: %d write, %d read)"
    r.total_usd r.api_calls r.avg_cost_per_call
