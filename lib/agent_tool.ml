...
let create config =
  let parameters =
    { name = "prompt"; description = "The prompt to send to the agent";
...
let create_simple ~name ~description runner =
  create { name; description; runner;
           output_summarizer = None; input_parameters = [] }
...
let%test "extra input_parameters included" =
  let tool = create {
    ...
    input_parameters = [
      { name = "context"; description = "extra"; param_type = String; required = false };
    ];
  } in
