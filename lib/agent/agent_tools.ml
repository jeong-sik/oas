...
let normalize_search_alias_input = function
  | `Assoc fields as input ->
...
let normalize_execute_alias_input = function
  | `Assoc fields as input ->
...
let legacy_tool_alias requested input =
  ...
  | "Grep" | "Search" | "Find" -> Some ("SearchFiles", normalize_search_alias_input input)
  | "Bash" | "Shell" | "execute_command" | "masc_code_shell" ->
    Some ("Execute", normalize_execute_alias_input input)
  | _ -> None
;;
