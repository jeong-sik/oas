...
let store ctx (ep : episode) =
  Context.set_scoped ctx scope ep.id (episode_to_json ep)
...
let recall ctx ?(now = Unix.gettimeofday ()) ?(decay_rate = 0.01)
...
let forget ctx id =
  Context.delete_scoped ctx scope id

let count ctx =
  List.length (Context.keys_in_scope ctx scope)
