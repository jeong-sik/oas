...
let empty_metrics = {
  total_turns = 0; total_tokens_used = 0; total_cost_usd = 0.0;
  tasks_completed = 0; errors_encountered = 0; elapsed_seconds = 0.0;
}
...
let build_progress_summary (msgs : message list) : string =
  let assistant_msgs = List.filter (fun (m : message) ->
    m.role = Assistant
  ) msgs in
  let recent = match List.length assistant_msgs with
    ...
    | _ -> assistant_msgs
  in
  let parts = List.map (fun (m : message) ->
    clip (msg_text m) 200
  ) recent in
  String.concat "\n" parts
...
let extract_pending_actions (msgs : message list) : string list =
  ...
  | [] -> []
  | (last : message) :: _ when last.role = User ->
    [clip (msg_text last) 200]
  | _ -> []

let extract_key_decisions (msgs : message list) : string list =
  let decision_markers = ["decided"; "chosen"; "selected"; "using";
                          "approach:"; "strategy:"; "will use"; "going with"] in
  List.filter_map (fun (m : message) ->
    if m.role <> Assistant then None
    else
      let text = msg_text m in
      let lower = String.lowercase_ascii text in
      let has_marker = List.exists (fun marker ->
        let rec find pos =
          if pos + String.length marker > String.length lower then false
          else if String.sub lower pos (String.length marker) = marker then true
          else find (pos + 1)
        in
        find 0
      ) decision_markers in
      if has_marker then Some (clip text 150)
      else None
  ) msgs
...
let extract_dna ~(messages : message list) ~(goal : string)
    ~(generation : int) ~(trace_id : string)
    ?(metrics = empty_metrics) ?(warnings = []) () : dna =
  ...
  let compressed_context =
    let recent = match List.length messages with
      | n when n > 10 ->
        List.filteri (fun i _ -> i >= n - 10) messages
      | _ -> messages
    in
    recent
    ...
    |> String.concat "\n"
  in
  { generation; trace_id; goal; progress_summary;
    compressed_context; pending_actions; key_decisions;
    memory_refs = []; warnings; metrics }
...
let build_successor_system_prompt (dna : dna) : string =
  ...
  Buffer.add_string buf (Printf.sprintf
    "You are continuing a task from generation %d.\n\n" dna.generation);
  Buffer.add_string buf (Printf.sprintf "Goal: %s\n\n" dna.goal);
  if dna.progress_summary <> "" then
    Buffer.add_string buf (Printf.sprintf
      "Progress so far:\n%s\n\n" dna.progress_summary);
  if dna.pending_actions <> [] then begin
  ...
  if dna.key_decisions <> [] then begin
...
let hydrate_messages (dna : dna) : message list =
  let context_msg =
    if dna.compressed_context <> "" then
      [text_message System
        (Printf.sprintf "Previous conversation context:\n%s"
          dna.compressed_context)]
    else []
  in
  let goal_msg =
    [text_message User
      (Printf.sprintf "Continue: %s" dna.goal)]
  in
...
let normalize_for_model (msgs : message list) ~(target_model : string) : message list =
  ...
  let repair_tool_calls msgs =
    let rec process acc = function
      | [] -> List.rev acc
      | (m : message) :: rest ->
        let dangling = List.filter_map (function
          | ToolUse { id; name; _ } ->
            if has_tool_result rest id then None
            else Some (id, name)
          | _ -> None
        ) m.content in
        let repairs = List.map (fun (id, name) ->
          { role = Tool;
