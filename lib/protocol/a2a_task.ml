...
let message_part_to_yojson = function
  | Text_part s -> `Assoc [ "type", `String "text"; "text", `String s ]
  | File_part { name; mime_type; data; location } ->
    let file_obj =
      `Assoc
        ([ "name", `String name; "mimeType", `String mime_type ]
         @
         match location with
  ...
  | Data_part json -> `Assoc [ "type", `String "data"; "data", json ]
;;
...
let show_message_part mp = Format.asprintf "%a" pp_message_part mp
...
let create (msg : task_message) : task =
  ...
  let id =
    Printf.sprintf
      ...
      (int_of_float (now *. 1000.0) mod 100_000)
  in
  { id
  ; state = Submitted
  ; messages = [ msg ]
...
let transition (t : task) (new_state : task_state) : (task, transition_error) result =
  ...
  then Error (TaskAlreadyTerminal { state = t.state })
  else if List.mem new_state (valid_transitions t.state)
  then Ok { t with state = new_state; updated_at = Unix.gettimeofday () }
  else Error (InvalidTransition { from_state = t.state; to_state = new_state })
;;
...
let evict_if_needed (s : store) : unit =
  if Hashtbl.length s.tasks < s.max_tasks
  then ()
  else (
    let terminals =
      Hashtbl.fold
        (fun id t acc -> if is_terminal t.state then (id, t.updated_at) :: acc else acc)
        s.tasks
        []
    in
    ...
    | _ ->
      let sorted = List.sort (fun (_, a) (_, b) -> Float.compare a b) terminals in
...
let get_task (s : store) (id : task_id) : task option = Hashtbl.find_opt s.tasks id
let list_tasks (s : store) : task list = Hashtbl.fold (fun _ t acc -> t :: acc) s.tasks []
