...
let connect ~sw ~net config =
  try
    Ok
      {
        client =
          Sdk_http_client.create ~endpoint:config.base_url ~headers:config.headers
            ~net ~sw ();
      }
  with
  | Eio.Io _ as exn -> Error (http_transport_error ~url:config.base_url exn)
  | Unix.Unix_error _ as exn ->
      Error (http_transport_error ~url:config.base_url exn)
  | Failure msg ->
      Error (Error.Mcp (HttpTransportFailed { url = config.base_url; detail = msg }))

let initialize t =
  match
    Sdk_http_client.initialize t.client ~client_name:"oas-sdk"
      ~client_version:"0.87.1"
  with
  | Ok _ -> Ok ()
...
let connect_and_load_managed ~sw ~net (spec : http_spec) :
  ...
  let* client =
    connect ~sw ~net { base_url = spec.base_url; headers = spec.headers }
  in
