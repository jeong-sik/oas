...
let http_get ~sw ~net url =
  ...
  | Error (Llm_provider.Http_client.CliTransportRequired { kind }) ->
    ...
    Error
      (Error.Orchestration
         (DiscoveryFailed
            { url; detail = Printf.sprintf "CLI transport required for %s" kind }))
...
let http_post ~sw ~net ~url ~body =
  ...
  | Ok (code, resp_body) ->
    Error
      (Error.A2a (ProtocolError { detail = Printf.sprintf "HTTP %d: %s" code resp_body }))
  | Error (Llm_provider.Http_client.HttpError { code; body = err_body }) ->
    Error
      (Error.A2a (ProtocolError { detail = Printf.sprintf "HTTP %d: %s" code err_body }))
  | Error (Llm_provider.Http_client.AcceptRejected { reason }) ->
  ...
  | Error (Llm_provider.Http_client.CliTransportRequired { kind }) ->
    Error
      (Error.A2a
         (ProtocolError { detail = Printf.sprintf "CLI transport required for %s" kind }))
  | Error (Llm_provider.Http_client.ProviderTerminal { message; _ }) ->
    ...
    (try
       ...
       | Error e ->
         Error
           (Error.Orchestration
              (DiscoveryFailed
                 { url; detail = "invalid agent card: " ^ Error.to_string e }))
     with
     | Yojson.Json_error e ->
       Error (Error.Orchestration (DiscoveryFailed { url; detail = "JSON parse: " ^ e })))
;;

let send_task ~sw ~net remote message =
  let params = `Assoc [ "message", A2a_task.task_message_to_yojson message ] in
