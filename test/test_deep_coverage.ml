...
let test_trace_capability_all_variants () =
  let variants = [ Sessions.Raw; Summary_only; No_trace ] in
  List.iter
    (fun v ->
...
let test_worker_status_all_variants () =
  let variants = [ Sessions.Planned; Accepted; Ready; Running; Completed; Failed ] in
  List.iter
    (fun v ->
...
let test_telemetry_event_count () =
  ...
  match
    Sessions.telemetry_event_count_of_yojson (Sessions.telemetry_event_count_to_yojson v)
  with
  | Ok v2 ->
...
let test_telemetry_step_some () =
  let v : Sessions.telemetry_step =
    ...
    ; detail = Some "generated code"
    }
  in
  match Sessions.telemetry_step_of_yojson (Sessions.telemetry_step_to_yojson v) with
  | Ok v2 ->
...
let test_telemetry_full () =
  let v : Sessions.telemetry =
    ...
    ; generated_at = 1000.0
    ; step_count = 2
    ; event_counts = [ { name = "e1"; count = 3 }; { name = "e2"; count = 7 } ]
    ; steps =
...
let test_structured_event_count () =
  ...
  match
    Sessions.structured_event_count_of_yojson
      (Sessions.structured_event_count_to_yojson v)
  with
...
let test_structured_telemetry_step_all_some () =
  let v : Sessions.structured_telemetry_step =
    ...
    ; artifact_name = Some "aname"
    ; artifact_kind = Some "code"
    ; checkpoint_label = Some "cp1"
  ...
  in
  match
    Sessions.structured_telemetry_step_of_yojson
      (Sessions.structured_telemetry_step_to_yojson v)
  with
  | Ok v2 ->
...
let test_structured_telemetry_full () =
  ...
  in
  match
    Sessions.structured_telemetry_of_yojson (Sessions.structured_telemetry_to_yojson v)
  with
...
let test_evidence_file_roundtrip () =
  ...
  in
  match Sessions.evidence_file_of_yojson (Sessions.evidence_file_to_yojson v) with
  | Ok v2 ->
...
let test_missing_file_roundtrip () =
  ...
  match Sessions.missing_file_of_yojson (Sessions.missing_file_to_yojson v) with
  | Ok v2 ->
    check string "label" v.label v2.label;
    check string "path" v.path v2.path
  | Error e -> fail e
...
let test_hook_summary_all_some () =
  ...
  in
  match Sessions.hook_summary_of_yojson (Sessions.hook_summary_to_yojson v) with
  | Ok v2 ->
...
let test_tool_contract_full () =
  ...
  in
  match Sessions.tool_contract_of_yojson (Sessions.tool_contract_to_yojson v) with
  | Ok v2 ->
    check string "name" v.name v2.name;
...
let test_evidence_capabilities_all_true () =
  ...
  in
  match
    Sessions.evidence_capabilities_of_yojson (Sessions.evidence_capabilities_to_yojson v)
  with
  | Ok v2 ->
...
let test_worker_run_minimal () =
  ...
  match Sessions.worker_run_of_yojson (Sessions.worker_run_to_yojson v) with
  | Ok v2 ->
...
let test_worker_run_fully_populated () =
  let v : Sessions.worker_run =
    ...
    ; error = None
    ; failure_reason = None
    ; accepted_at = Some 1000.0
    ; ready_at = Some 1001.0
    ; first_progress_at = Some 1002.0
    ; started_at = Some 1000.5
    ; finished_at = Some 1005.0
    ; last_progress_at = Some 1004.5
...
let test_worker_run_each_status () =
  let statuses = [ Sessions.Planned; Accepted; Ready; Running; Completed; Failed ] in
  List.iter
    (fun status ->
       ...
       | Error e ->
         fail
           (Printf.sprintf
              "worker_run status %s: %s"
              (Sessions.show_worker_status status)
              e))
    statuses
;;
  ...
  List.iter
    (fun trace_capability ->
       ...
       | Error e ->
         fail
           (Printf.sprintf
              "worker_run trace %s: %s"
              (Sessions.show_trace_capability trace_capability)
              e))
    caps
;;
...
let test_show_functions () =
  let check_nonempty label s = check bool label true (String.length s > 0) in
  check_nonempty "show_trace_capability" (Sessions.show_trace_capability Raw);
  check_nonempty "show_worker_status" (Sessions.show_worker_status Running);
  check_nonempty
    "show_session_info"
  ...
  check_nonempty
    ...
    (Sessions.show_telemetry_step
       { seq = 0; ts = 0.0; kind = "k"; participant = None; detail = None });
  check_nonempty
    "show_structured_event_count"
    (Sessions.show_structured_event_count { event_name = "e"; count = 0 });
  check_nonempty
    "show_evidence_file"
    (Sessions.show_evidence_file { label = "l"; path = "p"; size_bytes = 0; md5 = "" });
  check_nonempty
    "show_missing_file"
  ...
  check_nonempty
    "show_evidence_capabilities"
    (Sessions.show_evidence_capabilities
       { raw_trace = false; validated_summary = false; proof_bundle = false });
  check_nonempty "show_worker_run" (Sessions.show_worker_run (mk_minimal_worker_run ()))
;;
...
let test_durable_journal_entry_roundtrip () =
  ...
  in
  let state = Durable.Completed { journal = [ je ]; final_output = `Null } in
...
let test_durable_journal_entry_no_completed () =
  ...
  in
  let state =
    Durable.InProgress { current_step = "in-flight"; attempt = 1; journal = [ je ] }
  in
  let json = Durable.execution_state_to_json state in
  match Durable.execution_state_of_json json with
...
let test_durable_resume_unknown_step () =
  ...
  let sm =
    Durable.add_step sm { name = "real_step"; execute = (fun x -> Ok x); retry_limit = 1 }
  in
  let state =
    Durable.Failed { at_step = "nonexistent"; journal = []; error = "old error" }
  in
  ...
  | Failed { error; _ } ->
    check
      ...
      (let pat = "not found" in
       try
         ignore (Str.search_forward (Str.regexp_string pat) error 0);
         true
       with
       | Not_found -> false)
...
let test_durable_resume_in_progress_noop () =
  let sm = Durable.create ~name:"r" () in
  let state = Durable.InProgress { current_step = "x"; attempt = 1; journal = [] } in
  match Durable.resume sm state with
  | InProgress _ -> ()
...
let test_durable_suspend_suspended_noop () =
  let sm = Durable.create ~name:"s" () in
  let state = Durable.Suspended { at_step = "x"; journal = []; reason = "old" } in
...
let test_durable_resume_suspended () =
  ...
  let state = Durable.Suspended { at_step = "step1"; journal = []; reason = "paused" } in
  match Durable.resume sm state with
...
let test_durable_multi_journal_roundtrip () =
  ...
  let json = Durable.execution_state_to_json state in
  match Durable.execution_state_of_json json with
  | Ok (Completed { journal; _ }) -> check int "3 entries" 3 (List.length journal)
...
let test_durable_serialization_journal_with_output () =
  ...
  in
  let state = Durable.Failed { at_step = "s2"; journal = [ je ]; error = "crash" } in
  let json = Durable.execution_state_to_json state in
  match Durable.execution_state_of_json json with
...
let test_event_forward_all_event_types () =
  ...
  let events : Event_bus.event list =
    ...
    ; ev
        (AgentCompleted
           { agent_name = "a"; task_id = "t1"; result = ok_result; elapsed = 1.5 })
    ...
    ; ev
        (ElicitationCompleted
           { agent_name = "a"; question = "continue?"; response = Hooks.Declined })
...
let test_event_forward_agent_name_none_cases () =
  ...
  check (option string) "Custom agent" None (Event_forward.agent_name_of_event ev2)
;;
...
let test_event_forward_custom_target_inline () =
  ...
  let fwd =
    Event_forward.create
      ~targets:[ Custom_target { name = "counter"; deliver = (fun _ -> incr count) } ]
      ~batch_size:1
      ()
  in
...
let test_event_forward_file_append () =
  ...
  at_exit (fun () ->
    ...
    | _ -> ());
  let bus = Event_bus.create () in
  let fwd = Event_forward.create ~targets:[ File_append { path } ] ~batch_size:1 () in
  Event_forward.start ~sw ~net:(Eio.Stdenv.net env) ~bus fwd;
  Event_bus.publish bus (Event_bus.mk_event (TurnStarted { agent_name = "a"; turn = 1 }));
  Eio.Fiber.yield ();
  Eio.Fiber.yield ();
  Event_forward.stop fwd;
  Eio.Fiber.yield ();
...
let test_event_forward_batch_size () =
  let fwd = Event_forward.create ~targets:[] ~batch_size:50 ~flush_interval_s:5.0 () in
  check int "delivered" 0 (Event_forward.delivered_count fwd);
...
let test_builder_safe_max_turns_zero () =
  ...
  match
    ...
    |> Builder.build_safe
  with
  | Error (Config (InvalidConfig { field; _ })) -> check string "field" "max_turns" field
...
let test_builder_safe_max_tokens_zero () =
  ...
  match
    ...
    |> Builder.build_safe
  with
  | Error (Config (InvalidConfig { field; _ })) -> check string "field" "max_tokens" field
...
let test_builder_chaining () =
  ...
  | Error e -> fail (Printf.sprintf "build_safe failed: %s" (Error.to_string e))
;;
...
let test_builder_with_periodic_callback () =
  ...
  let cb2 : Agent.periodic_callback =
...
let test_builder_with_tools_list () =
  ...
  @@ fun net ->
  let t1 =
    Tool.create ~name:"t1" ~description:"d1" ~parameters:[] (fun _ ->
      Ok { Types.content = "ok" })
  in
  let t2 =
    Tool.create ~name:"t2" ~description:"d2" ~parameters:[] (fun _ ->
      Ok { Types.content = "ok" })
  in
  match
    Builder.create ~net ~model:"agent_llm_a-sonnet-4-6"
  ...
  run
    ...
    [ ( "sessions.trace_capability"
      , [ test_case "all variants roundtrip" `Quick test_trace_capability_all_variants
        ...
        ; test_case "wire values" `Quick test_trace_capability_json_values
        ] )
