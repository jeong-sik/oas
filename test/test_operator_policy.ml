...
let test_merge_operator_allowlist () =
  ...
  check bool "operator source" true (source = Guardrails.Operator)
;;
...
let test_merge_operator_denylist () =
  ...
  in
  (match merged.tool_filter with
   | Guardrails.DenyList l -> check (list string) "deny b" [ "b" ] l
...
let test_merge_preserves_max_calls () =
  ...
  let merged, _ =
    Guardrails.merge_operator_policy
      ...
      ~agent
  in
  check (option int) "limit preserved" (Some 5) merged.max_tool_calls_per_turn
;;
...
let test_turn_override_intersects_operator () =
  ...
  let turn_params =
    { Hooks.default_turn_params with
      tool_filter_override = Some (Guardrails.AllowList [ "b"; "c" ])
    }
  in
  let tools_json, _, _ =
    Agent_turn.prepare_tools
...
let test_turn_override_cannot_widen_operator () =
  ...
  let turn_params =
    { Hooks.default_turn_params with
      tool_filter_override = Some (Guardrails.AllowList [ "b" ])
    }
  in
  let tools_json, _, _ =
    Agent_turn.prepare_tools
  ...
  run
    ...
    ; ( "prepare_tools"
      , [ test_case "operator restricts AllowAll" `Quick test_operator_restricts_allow_all
        ...
        ; test_case
            "turn override intersects operator"
            `Quick
            test_turn_override_intersects_operator
        ; test_case
            "turn override cannot widen operator"
            `Quick
            test_turn_override_cannot_widen_operator
        ; test_case "full prepare_turn" `Quick test_full_prepare_turn_with_operator
        ] )
    ; "policy_source", [ test_case "show" `Quick test_show_policy_source ]
    ; ( "builder"
