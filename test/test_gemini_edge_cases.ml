...
let test_disable_parallel () =
  ...
  let config = Provider_config.make ~kind:Gemini
      ~model_id:"gemini-2.5-flash"
      ~base_url:"https://generativelanguage.googleapis.com/v1beta"
      ~disable_parallel_tool_use:true () in
  let tools = [`Assoc [("name", `String "t1"); ("description", `String "d")]] in
  let body = Backend_gemini.build_request ~config
      ~messages:[Types.user_msg "hi"] ~tools () in
...
let test_vertex_ai_url () =
  ...
  check "stream has :streamGenerateContent" (string_has stream_url ":streamGenerateContent");
...
let test_sse_function_call () =
  ...
  let chunk_data = {|{"candidates":[{"content":{"parts":[{"functionCall":{"name":"get_weather","args":{"city":"Seoul"}},"thoughtSignature":"abc123"}],"role":"model"},"finishReason":"STOP","index":0}],"usageMetadata":{"promptTokenCount":44,"candidatesTokenCount":15},"modelVersion":"gemini-2.5-flash"}|} in
  (match Streaming.parse_gemini_sse_chunk chunk_data with
   | Some chunk ->
       ...
       let has_tool_start = List.exists (function
         | Types.ContentBlockStart { content_type = "tool_use"; tool_name = Some "get_weather"; _ } -> true
         | _ -> false) events in
       check "tool_use BlockStart emitted" has_tool_start;
       let has_input_delta = List.exists (function
         | Types.ContentBlockDelta { delta = InputJsonDelta _; _ } -> true
         | _ -> false) events in
       check "InputJsonDelta emitted" has_input_delta;
       let has_end = List.exists (function
...
let test_tool_use_id_roundtrip () =
  ...
  let response_json = Yojson.Safe.from_string {|{
  ...
  let (tu_id, tu_name, tu_input) = match List.hd resp.content with
  ...
  let config = Provider_config.make ~kind:Gemini
      ~model_id:"gemini-2.5-flash"
      ~base_url:"https://generativelanguage.googleapis.com/v1beta" () in
  let messages = [
    Types.user_msg "What's the weather?";
    { Types.role = Assistant; content = [Types.ToolUse { id = tu_id; name = tu_name; input = tu_input }]; name = None; tool_call_id = None ; metadata = []};
