...
type fp = { fp_name: string; fp_input: string }
...
let test_b8_streaming_idle_detection_logic () =
  ...
  let tool1 = Types.ToolUse { id = "t1"; name = "calc"; input = `Assoc [("x", `Int 42)] } in
  let tool2 = Types.ToolUse { id = "t2"; name = "calc"; input = `Assoc [("x", `Int 42)] } in
  let tool3 = Types.ToolUse { id = "t3"; name = "calc"; input = `Assoc [("x", `Int 99)] } in
  ...
  check bool "first turn not idle" false
    (is_idle_check None fps1);

  (* Same tool+input: idle detected *)
  check bool "identical tools → idle" true
    (is_idle_check (Some fps1) fps2);

  (* Different input: not idle *)
  check bool "different input → not idle" false
    (is_idle_check (Some fps1) fps3);

  (* Empty vs empty: idle *)
  check bool "empty vs empty → idle" true
    (is_idle_check (Some []) [])
...
let test_b8_streaming_idle_loop_guard () =
  ...
  check bool "IdleDetected error string contains 'idle'" true
    (String.lowercase_ascii msg |> fun s ->
      try ignore (Str.search_forward (Str.regexp_string "idle") s 0); true
...
let test_b9_injector_logs_instead_of_discard () =
  ...
  (try
    ignore (injector ~tool_name:"t" ~input:`Null
      ~output:(Ok { Types.content = "ok" }))
  with Failure _ -> threw := true);
...
let test_b10_catch_all_removed () =
  ...
  (try raise (Unix.Unix_error (Unix.ENOENT, "test", "path"))
   with Unix.Unix_error _ -> caught_unix := true);
...
let () =
  run "v024_fixes" [
    "B8_streaming_idle_FIXED", [
      test_case "fingerprint comparison logic" `Quick
        test_b8_streaming_idle_detection_logic;
      test_case "IdleDetected in loop guard" `Quick
        test_b8_streaming_idle_loop_guard;
      test_case "consecutive idle counter" `Quick
        test_b8_consecutive_idle_counter;
    ];
    "B9_injector_logging_FIXED", [
      test_case "injector logs instead of discard" `Quick
        test_b9_injector_logs_instead_of_discard;
    ];
    "B10_catch_all_FIXED", [
      test_case "narrow exception types exist" `Quick
        test_b10_catch_all_removed;
    ];
    "B11_atomic_closed_FIXED", [
      test_case "Atomic.t for closed flag" `Quick
        test_b11_atomic_closed;
    ];
  ]
