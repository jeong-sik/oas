...
let test_well_formed_accepts_valid () =
  ...
  List.iter
    (fun ev ->
       ...
       | Ok () -> ()
       | Error msg ->
         Alcotest.failf "expected %s to be well-formed but got: %s" (name ev) msg)
    cases
;;
...
let test_well_formed_rejects_invalid () =
  ...
  check_rejects
    "confidence below 0"
    (is_well_formed (Intent_predicted { intent_label = "x"; confidence = -0.1 }));
  check_rejects
    "confidence above 1"
    (is_well_formed (Intent_predicted { intent_label = "x"; confidence = 1.01 }));
  check_rejects
    "confidence NaN"
    (is_well_formed (Intent_predicted { intent_label = "x"; confidence = Float.nan }));
  check_rejects
    "confidence +inf"
    (is_well_formed
  ...
  check_rejects "level below 0" (is_well_formed (Disclosure_level { level = -1 }));
  check_rejects "level above 3" (is_well_formed (Disclosure_level { level = 4 }))
;;

let test_yojson_roundtrip () =
  let cases =
    [ Gravity_ranked { ranked_count = 7; query_terms = 2 }
    ; Intent_predicted { intent_label = "knowledge_query"; confidence = 0.84 }
    ; Mode_transitioned { from_mode = "focus"; to_mode = "scan" }
    ...
    (fun ev ->
       ...
       | Ok ev' ->
         ...
         Alcotest.(check string)
           ...
           (show ev)
           (show ev')
       | Error msg -> Alcotest.failf "yojson roundtrip failed for %s: %s" (name ev) msg)
    cases
;;
...
let () =
  Alcotest.run
    "cognitive_event"
    [ "labels", [ Alcotest.test_case "name is stable" `Quick test_name_is_stable ]
    ; ( "well_formed"
      , [ Alcotest.test_case "accepts valid" `Quick test_well_formed_accepts_valid
        ; Alcotest.test_case "rejects invalid" `Quick test_well_formed_rejects_invalid
        ] )
    ; ( "yojson"
      , [ Alcotest.test_case "roundtrip" `Quick test_yojson_roundtrip
        ; Alcotest.test_case "rejects garbage" `Quick test_yojson_rejects_garbage
        ] )
    ]
;;
