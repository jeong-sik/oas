...
Body content here.|} in
  ...
  Alcotest.(check (option string)) "name"
    (Some "my-skill") (Skill.frontmatter_value fm "name");
  Alcotest.(check (option string)) "description"
    (Some "A test skill") (Skill.frontmatter_value fm "description");
  Alcotest.(check (option string)) "model"
    (Some "claude-3") (Skill.frontmatter_value fm "model")
...
let test_scope_of_string () =
  Alcotest.(check bool) "project" true
    (Skill.scope_of_string "project" = Skill.Project);
  Alcotest.(check bool) "user" true
    (Skill.scope_of_string "user" = Skill.User);
  Alcotest.(check bool) "local" true
    (Skill.scope_of_string "local" = Skill.Local);
...
---
Do the thing with $ARGUMENTS|} in
...
let test_supporting_no_path () =
  let skill = { (Skill.of_markdown "") with
    path = None; supporting_files = ["file.sh"] } in
...
let test_supporting_with_path () =
  let skill = { (Skill.of_markdown "") with
    ...
    supporting_files = ["helpers.sh"; "/abs/path.sh"] } in
  let paths = Skill.supporting_file_paths skill in
...
let () =
  Alcotest.run "Skill_coverage" [
    ...
    "replace_all", [
      ...
      Alcotest.test_case "longer replacement" `Quick test_replace_all_longer_replacement;
      Alcotest.test_case "shorter than pattern" `Quick test_replace_all_shorter_than_pattern;
    ];
    "frontmatter", [
    ...
    "load", [
      Alcotest.test_case "nonexistent" `Quick test_load_nonexistent;
    ];
    "show", [
      Alcotest.test_case "skill show" `Quick test_skill_show;
    ];
  ]
