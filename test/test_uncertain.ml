...
let () = Alcotest.run "Uncertain" [
  "constructors", [
    Alcotest.test_case "from_llm defaults" `Quick (fun () ->
      ...
      Alcotest.(check (float 0.01)) "default confidence" 0.5 (Uncertain.confidence u);
    ...
    Alcotest.test_case "from_llm with params" `Quick (fun () ->
      let u = Uncertain.from_llm ~model:"claude" ~temperature:0.7 ~confidence:0.9 42 in
    ...
    Alcotest.test_case "from_heuristic" `Quick (fun () ->
      ...
      Alcotest.(check (float 0.01)) "default confidence" 0.3 (Uncertain.confidence u);
      match Uncertain.provenance_of u with
      | Heuristic { name } ->
        Alcotest.(check string) "name" "keyword_match" name
  ...
  "transformations", [
    Alcotest.test_case "map preserves metadata" `Quick (fun () ->
      ...
      Alcotest.(check (float 0.01)) "confidence preserved" 0.8 (Uncertain.confidence u'));

    Alcotest.test_case "bind takes min confidence" `Quick (fun () ->
      let u = Uncertain.from_llm ~model:"m" ~confidence:0.8 "hello" in
      let u' = Uncertain.bind (fun s ->
  ...
  "unwrapping", [
    ...
    Alcotest.test_case "to_result error below threshold" `Quick (fun () ->
      ...
      | Error msg ->
        Alcotest.(check bool) "has confidence info"
  ...
  "stress", [
    Alcotest.test_case "default stress is zero" `Quick (fun () ->
      ...
      Alcotest.(check (option (float 0.01))) "no pressure"
        None s.context_pressure;
    ...
    Alcotest.test_case "custom stress" `Quick (fun () ->
      let stress = Uncertain.{
        context_pressure = Some 0.85;
        time_pressure = true;
        retry_count = 2;
      } in
      let u = Uncertain.from_llm ~model:"m" ~stress "x" in
      let s = Uncertain.stress_of u in
      Alcotest.(check (option (float 0.01))) "pressure"
        (Some 0.85) s.context_pressure;
  ...
  "json_roundtrip", [
    ...
    Alcotest.test_case "stress roundtrip" `Quick (fun () ->
      let s = Uncertain.{ context_pressure = Some 0.9;
                          time_pressure = true; retry_count = 3 } in
      let json = Uncertain.stress_to_yojson s in
      match Uncertain.stress_of_yojson json with
      | Ok s' ->
        Alcotest.(check (option (float 0.01))) "pressure"
          (Some 0.9) s'.context_pressure;
