...
let assert_transcript ~provider ~names =
  List.iter (fun required ->
    ...
    min_transcript;
  let index n = List.find_index ((=) n) names
                |> Option.value ~default:max_int in
  check bool
    (Printf.sprintf "[%s] agent.started before agent.completed" provider) true
    (index "agent.started" < index "agent.completed");
  check bool
    (Printf.sprintf "[%s] turn.started before turn.completed" provider) true
    (index "turn.started" < index "turn.completed")

let assert_envelope ~provider events =
  List.iter (fun (e : Event_bus.event) ->
    check bool
      (Printf.sprintf "[%s] correlation_id non-empty" provider) true
      (String.length e.meta.correlation_id > 0);
    check bool
      (Printf.sprintf "[%s] run_id non-empty" provider) true
      (String.length e.meta.run_id > 0);
    check bool
      (Printf.sprintf "[%s] ts populated" provider) true
      (e.meta.ts > 0.0))
    events

(* ── Minimal agent driver ─────────────────────────────────────── *)

let run_minimal_agent ~env ~sw ~provider_label
  ...
  let tr = Orchestrator.run_task ~sw orch {
  ...
  (match tr.result with
   ...
   | Error e ->
     ...
     Printf.printf "  [%s] task returned Error: %s — checking error-path transcript\n%!"
       provider_label (Error.to_string e);
     check bool
       (Printf.sprintf "[%s] agent.started on error" provider_label) true
       (List.mem "agent.started" names);
     check bool
       (Printf.sprintf "[%s] agent.completed on error" provider_label) true
       (List.mem "agent.completed" names);
     check bool
       (Printf.sprintf "[%s] agent.failed on error" provider_label) true
...
let test_anthropic () =
  ...
  | Some _ ->
    ...
    let provider : Provider.config = {
      ...
      api_key_env = "ANTHROPIC_API_KEY" } in
    run_minimal_agent ~env ~sw ~provider_label:"anthropic"
...
let test_openai () =
  ...
  | None | Some "" ->
    skip_note "openai" "OPENAI_API_KEY not set"
  | Some _ ->
    Eio_main.run @@ fun env ->
    Eio.Switch.run @@ fun sw ->
    ...
    let provider : Provider.config = {
      ...
      api_key_env = "OPENAI_API_KEY" } in
    run_minimal_agent ~env ~sw ~provider_label:"openai"
      ~provider ~base_url ~model:"gpt-4o-mini"
...
let test_gemini () =
  ...
  | None | Some "" ->
    skip_note "gemini" "GEMINI_API_KEY not set"
  | Some _ ->
    Eio_main.run @@ fun env ->
    Eio.Switch.run @@ fun sw ->
    ...
    let provider : Provider.config = {
      ...
      api_key_env = "GEMINI_API_KEY" } in
    run_minimal_agent ~env ~sw ~provider_label:"gemini"
      ~provider ~base_url ~model:"gemini-2.0-flash"
...
let test_local_compat () =
  ...
  let statuses = Llm_provider.Discovery.discover
      ~sw ~net:env#net ~endpoints in
  let healthy = List.filter (fun (s : Llm_provider.Discovery.endpoint_status)
                              -> s.healthy) statuses in
  if healthy = [] then
    skip_note "local-openai-compat"
      (Printf.sprintf "no healthy endpoint in [%s]"
    ...
    List.iter (fun (s : Llm_provider.Discovery.endpoint_status) ->
      ...
      | m :: _ ->
        ...
        let provider : Provider.config = {
          ...
          model_id = m.id;
          api_key_env = "" } in
        run_minimal_agent ~env ~sw ~provider_label:label
          ~provider ~base_url:s.url ~model:m.id)
      healthy
