...
let mk_event ?(seq = 1) ?(ts = 100.0) kind : Runtime.event = { seq; ts; kind }
...
let mk_participant ?(name = "alice") ?(state = Runtime.Planned) ?(started_at = None) ()
  : Runtime.participant
  =
  { name
  ; role = None
  ...
  let event =
    mk_event
      ...
      (Agent_failed
         ...
         ; failure_cause = Execution_error "timeout"
         })
  in
  match Runtime_projection.apply_event session event with
  | Ok s ->
...
let test_count_by_state_via_report () =
  ...
  let has needle =
    List.exists (fun line -> Util.string_contains ~needle line) report.summary
  in
...
let test_apply_event_sequence () =
  ...
  let events =
    [ mk_event
        ~seq:1
        ~ts:10.0
        (Session_started { goal = "full flow"; participants = [ "alice" ] })
    ...
    ; ( "apply_event"
      , [ Alcotest.test_case "Session_started" `Quick test_apply_session_started
        ...
        ; Alcotest.test_case
            "Turn terminal fails"
            `Quick
            test_apply_turn_recorded_terminal_fails
    ...
    ; ( "build_proof"
      , [ Alcotest.test_case "passing" `Quick test_build_proof_passing
        ...
        ; Alcotest.test_case
            ...
            `Quick
            test_build_proof_duplicate_artifact_ids
        ; Alcotest.test_case
            "non-contiguous seq"
            `Quick
            test_build_proof_non_contiguous_seq
        ] )
    ; ( "integration"
