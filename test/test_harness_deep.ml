...
let test_adversarial_graceful_error_ok_result () =
  ...
  Alcotest.(check bool) "detail says expected error" true
    (match v.detail with
...
let test_adversarial_no_tool_with_tools () =
  ...
  Alcotest.(check bool) "detail lists tools" true
    (match v.detail with
...
let test_performance_no_constraints () =
  ...
  in
  let v =
    Harness.Performance.evaluate obs Harness.Performance.default_expectation
  in
  Alcotest.(check bool) "no constraints -> pass" true v.passed;
  Alcotest.(check bool) "no evidence (no active checks)" true
    (v.evidence = [])
...
let test_performance_cost_exceeded () =
  ...
  in
  let exp =
    { Harness.Performance.default_expectation with max_cost_usd = Some 1.0 }
  in
  let v = Harness.Performance.evaluate obs exp in
...
let test_regression_structural_match () =
  ...
  in
  let v =
    Harness.Regression.evaluate ~mode:(StructuralMatch cmp) obs
      {|{"key":"val"}|}
  in
...
let test_regression_fuzzy_threshold_boundary () =
  ...
  let v_pass =
    Harness.Regression.evaluate
      ~mode:(FuzzyMatch { threshold = 0.6 })
      obs "abcXX"
  in
  Alcotest.(check bool) "fuzzy at threshold" true v_pass.passed;
  let v_fail =
    Harness.Regression.evaluate
      ~mode:(FuzzyMatch { threshold = 0.7 })
      obs "abcXX"
  in
...
let test_regression_fuzzy_empty_strings () =
  let obs : Harness.Regression.observation =
    { output_json = `Null; output_text = "" }
  in
  let v =
    Harness.Regression.evaluate ~mode:(FuzzyMatch { threshold = 0.5 }) obs ""
  in
...
let test_regression_fuzzy_different_lengths () =
  ...
  let v =
    Harness.Regression.evaluate
      ...
      obs "short and much longer text"
  in
...
let test_swiss_cheese_no_layers () =
  ...
  Alcotest.(check (float 0.01)) "coverage 1.0 for empty" 1.0
    (Option.get v.score)
...
let test_swiss_cheese_single_fail () =
  ...
  Alcotest.(check bool) "detail mentions failure" true
    (match v.detail with
...
let test_swiss_cheese_require_n_more_than_available () =
  ...
  Alcotest.(check bool) "detail present" true
    (match v.detail with
...
let test_composability_turn_count_below () =
  ...
  Alcotest.(check bool) "detail mentions limit" true
...
let test_composability_context_not_propagated () =
  ...
  Alcotest.(check bool) "detail mentions key" true
    (match v.detail with
...
let test_behavioral_all_score_aggregation () =
  ...
  let v =
    Harness.Behavioral.evaluate obs
      (All
         [
           ToolSelected [ "search" ];
           CompletesWithin 5;
...
let test_behavioral_completes_within_score () =
  ...
  Alcotest.(check bool) "detail mentions turns" true
    (match v.detail with
...
let test_behavioral_tool_selected_evidence () =
  ...
  Alcotest.(check bool) "detail mentions missing" true
    (match v.detail with
...
let test_behavioral_all_partial_failure_count () =
  ...
  let v =
    Harness.Behavioral.evaluate obs
      (All
         [
           ToolSelected [ "missing" ];
           CompletesWithin 1;
           ContainsText "nope";
         ])
  in
  Alcotest.(check bool) "all fail" false v.passed;
  Alcotest.(check bool) "detail says 3/3" true
    (match v.detail with
...
let () =
  Alcotest.run "harness-deep"
    [
      ( "adversarial-deep",
        [
          Alcotest.test_case "error_contains found" `Quick
            test_adversarial_error_contains_found;
          Alcotest.test_case "error_contains not found" `Quick
            test_adversarial_error_contains_not_found;
          Alcotest.test_case "error_contains none" `Quick
            test_adversarial_error_contains_none;
          Alcotest.test_case "graceful_error ok result" `Quick
            test_adversarial_graceful_error_ok_result;
          Alcotest.test_case "no_tool with tools" `Quick
      ...
      ( "performance-deep",
        [
          ...
          Alcotest.test_case "all constraints fail" `Quick
            test_performance_all_constraints;
          Alcotest.test_case "no constraints" `Quick
            test_performance_no_constraints;
          Alcotest.test_case "latency only" `Quick
            test_performance_latency_only;
          Alcotest.test_case "cost exceeded" `Quick
            test_performance_cost_exceeded;
        ] );
      ( "regression-deep",
        [
          Alcotest.test_case "structural match" `Quick
            test_regression_structural_match;
          Alcotest.test_case "structural mismatch" `Quick
            test_regression_structural_match_fail;
          Alcotest.test_case "structural invalid golden" `Quick
            test_regression_structural_invalid_golden_json;
          Alcotest.test_case "fuzzy threshold boundary" `Quick
            test_regression_fuzzy_threshold_boundary;
          Alcotest.test_case "fuzzy empty strings" `Quick
            test_regression_fuzzy_empty_strings;
          Alcotest.test_case "fuzzy different lengths" `Quick
      ...
      ( "swiss-cheese-deep",
        [
          ...
          Alcotest.test_case "evaluate layers" `Quick
            test_swiss_cheese_evaluate_layers;
          Alcotest.test_case "evaluate layers partial" `Quick
            test_swiss_cheese_evaluate_layers_partial;
          Alcotest.test_case "require_n zero" `Quick
            test_swiss_cheese_require_n_zero;
          Alcotest.test_case "require_n exceed" `Quick
            test_swiss_cheese_require_n_more_than_available;
        ] );
      ( "composability-deep",
        [
          Alcotest.test_case "all_completed true" `Quick
            test_composability_all_agents_completed_true;
          Alcotest.test_case "all_completed false" `Quick
            test_composability_all_agents_completed_false;
          Alcotest.test_case "turn_count_below" `Quick
            test_composability_turn_count_below;
          Alcotest.test_case "context not propagated" `Quick
            test_composability_context_not_propagated;
        ] );
      ( "behavioral-deep",
        [
          Alcotest.test_case "all score aggregation" `Quick
            test_behavioral_all_score_aggregation;
          Alcotest.test_case "all empty" `Quick test_behavioral_all_empty;
          Alcotest.test_case "completes_within score" `Quick
            test_behavioral_completes_within_score;
          Alcotest.test_case "tool_selected evidence" `Quick
            test_behavioral_tool_selected_evidence;
          Alcotest.test_case "all partial failure count" `Quick
