...
let test_options_custom () =
  ...
  Alcotest.(check (option string))
    "runtime_path"
    (Some "/usr/local/bin/oas_runtime")
    o.runtime_path;
  Alcotest.(check bool) "include_partial" true o.include_partial_messages;
  Alcotest.(check (list string)) "sources" [ "env"; "config.json" ] o.setting_sources
;;
...
let test_protocol_message_round_trip_response () =
  let msg =
    Runtime.Response_message { request_id = "req-002"; response = Runtime.Shutdown_ack }
  in
  let serialized = Runtime.protocol_message_to_string msg in
  let parsed = Runtime.protocol_message_of_string serialized in
  Alcotest.(check bool) "round-trip ok" true (Result.is_ok parsed)
  ...
  let msg =
    Runtime.Event_message
      ...
      ; event =
          { seq = 1
          ; ts = 1000.0
          ; kind = Session_started { goal = "test goal"; participants = [ "agent-a" ] }
          }
      }
  in
  let serialized = Runtime.protocol_message_to_string msg in
...
let test_protocol_message_round_trip_control_request () =
  let msg =
    Runtime.Control_request_message
      { control_id = "ctrl-1"
      ; request =
          Permission_request { action = "read"; subject = "/file"; payload = `Null }
      }
  in
  let serialized = Runtime.protocol_message_to_string msg in
  let parsed = Runtime.protocol_message_of_string serialized in
...
let test_protocol_message_round_trip_control_response () =
  let msg =
    Runtime.Control_response_message
      { control_id = "ctrl-2"
      ; response = Hook_response { continue_ = true; message = Some "ok" }
      }
  in
  let serialized = Runtime.protocol_message_to_string msg in
  let parsed = Runtime.protocol_message_of_string serialized in
...
let test_protocol_message_round_trip_system () =
  let msg = Runtime.System_message { level = "info"; message = "initialized" } in
  let serialized = Runtime.protocol_message_to_string msg in
  let parsed = Runtime.protocol_message_of_string serialized in
  Alcotest.(check bool) "round-trip ok" true (Result.is_ok parsed)
;;
...
let test_request_apply_command () =
  let req =
    Runtime.Apply_command
      { session_id = "sess-1"
      ; command = Record_turn { actor = Some "user"; message = "Hello agent." }
      }
  in
  let s = Runtime.request_to_string req in
  let parsed = Runtime.request_of_string s in
...
let test_request_finalize () =
  let req = Runtime.Finalize { session_id = "sess-1"; reason = Some "task complete" } in
  let s = Runtime.request_to_string req in
  let parsed = Runtime.request_of_string s in
  Alcotest.(check bool) "round-trip ok" true (Result.is_ok parsed)
;;
...
let test_request_events () =
  let req = Runtime.Events { session_id = "sess-1"; after_seq = Some 5 } in
  let s = Runtime.request_to_string req in
  let parsed = Runtime.request_of_string s in
  Alcotest.(check bool) "round-trip ok" true (Result.is_ok parsed)
;;
...
let () =
  Alcotest.run
    "transport-deep"
    [ "default_options", [ Alcotest.test_case "fields" `Quick test_default_options ]
    ...
    ; ( "protocol_round_trip"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case "event message" `Quick test_protocol_message_round_trip_event
        ; Alcotest.test_case "invalid json" `Quick test_protocol_message_invalid_json
        ; Alcotest.test_case
            "control request"
            `Quick
    ...
    ; ( "request_response_strings"
      , [ Alcotest.test_case "shutdown" `Quick test_request_round_trip_shutdown
        ; Alcotest.test_case "start_session" `Quick test_request_round_trip_start_session
        ; Alcotest.test_case "request invalid" `Quick test_request_invalid
        ; Alcotest.test_case "error response" `Quick test_response_round_trip_error
        ; Alcotest.test_case
    ...
    ; "protocol_version", [ Alcotest.test_case "format" `Quick test_protocol_version ]
    ; ( "complex_requests"
