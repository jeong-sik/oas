...
let test_token_budget_exceeds () =
  ...
  let msgs =
    ...
    ; (* turn2: ~200 tokens *)
      user_msg long
    ; asst_msg long (* turn3: ~200 tokens *)
    ]
  in
...
let test_token_budget_image_doc () =
  ...
  let large_img = String.make 1_200_000 'A' in
  (* ~1200 tokens for image *)
  let large_doc = String.make 2_000_000 'A' in
  (* ~3000 tokens for document *)
  let msgs =
    [ Types.
        { role = User
        ; content =
...
let test_estimate_text () =
  let msg = user_msg "hello world" in
  (* 11 chars -> (11+3)/4 = 3 *)
...
let test_estimate_tool_use () =
  let msg =
    Types.
      { role = Assistant
      ; content = [ ToolUse { id = "id1"; name = "calc"; input = `String "input" } ]
      ; name = None
      ; tool_call_id = None
      ; metadata = []
      }
...
let test_group_basic () =
  let msgs = [ user_msg "a"; asst_msg "b"; user_msg "c"; asst_msg "d" ] in
...
let test_group_tool_result_stays () =
  let msgs =
    [ user_msg "q"; tool_use_msg "t1" "calc"; tool_result_msg "t1" "42"; asst_msg "done" ]
  in
  let turns = Context_reducer.group_into_turns msgs in
...
let test_custom () =
  ...
  let strategy =
    Context_reducer.custom (fun msgs ->
      called := true;
      List.rev msgs)
  in
  let msgs = [ user_msg "a"; asst_msg "b" ] in
...
let test_importance_scored_drops_low_scores () =
  let msgs = [ user_msg "goal"; asst_msg "low-value"; asst_msg "keep-me" ] in
  let reducer =
...
let test_importance_scored_boost_preserves_message () =
  let msgs = [ user_msg "goal"; asst_msg "[KEEP] anchor"; asst_msg "low-value" ] in
  let reducer =
    Context_reducer.importance_scored
      ~threshold:0.5
      ~scorer:(fun ~index:_ ~total:_ _msg -> 0.2)
      ~boost:(fun msg ->
        if Util.string_contains ~needle:"[KEEP]" (Types.text_of_message msg)
        then Some 0.95
        else None)
      ()
  in
...
let test_cap_preserves_tool_use () =
  ...
  let msgs =
    ...
    ; tool_result_msg "call_A" "ra"
    ; (* user + result *)
      tool_result_msg "call_B" "rb"
    ; asst_msg "done"
    ; (* final assistant, forces keep_recent to skip turn0 *)
      user_msg "extra turn"
    ; asst_msg "padding"
    ]
  in
...
let test_cap_truncates_text_only () =
  ...
  Alcotest.(check bool)
    "some blocks dropped"
    true
    (List.length capped.content < List.length asst_content)
;;
...
let test_cap_mandatory_overflow_returns_as_is () =
  ...
  let heavy_tool_input = `Assoc [ "k", `String (String.make 2000 'y') ] in
  let asst_content =
...
let test_prune_short_outputs () =
  let msgs = [ user_msg "q"; tool_use_msg "t1" "calc"; tool_result_msg "t1" "short" ] in
  let result =
    Context_reducer.reduce (Context_reducer.prune_tool_outputs ~max_output_len:100) msgs
  in
...
let test_prune_long_outputs () =
  ...
  let msgs =
    [ user_msg "q"; tool_use_msg "t1" "calc"; tool_result_msg "t1" long_output ]
  in
  let result =
    Context_reducer.reduce (Context_reducer.prune_tool_outputs ~max_output_len:50) msgs
  in
  let tool_content =
    match List.nth result 2 with
    | { Types.content = [ Types.ToolResult { content; _ } ]; _ } -> content
...
let test_merge_same_role () =
  let msgs = [ user_msg "a"; user_msg "b"; asst_msg "c" ] in
  ...
  match result with
  | { Types.content; _ } :: _ -> Alcotest.(check int) "2 blocks" 2 (List.length content)
  | _ -> Alcotest.fail "unexpected result"
;;

let test_merge_preserves_tool_results () =
  let msgs = [ tool_result_msg "t1" "42"; tool_result_msg "t2" "43" ] in
  let result = Context_reducer.reduce Context_reducer.merge_contiguous msgs in
...
let test_keep_first_and_last_identity () =
  let msgs = [ user_msg "turn1"; asst_msg "r1"; user_msg "turn2"; asst_msg "r2" ] in
  (* first_n=1 + last_n=1 = 2, total turns = 2, so all preserved *)
  let result =
    Context_reducer.reduce (Context_reducer.keep_first_and_last ~first_n:1 ~last_n:1) msgs
  in
  Alcotest.(check int) "all preserved" (List.length msgs) (List.length result)
;;
...
let test_keep_first_and_last_drops_middle () =
  ...
  match result with
  | { Types.content = [ Types.Text t1 ]; _ }
    :: _
    :: { Types.content = [ Types.Text t3 ]; _ }
    :: _ ->
...
let test_prune_by_role_drops_user () =
  let msgs = [ user_msg "q1"; asst_msg "a1"; user_msg "q2"; asst_msg "a2" ] in
  let result =
    Context_reducer.reduce (Context_reducer.prune_by_role ~drop_roles:[ Types.User ]) msgs
  in
...
let test_summarize_old_basic () =
  ...
  in
  let summarizer msgs = Printf.sprintf "[Summary of %d messages]" (List.length msgs) in
  let result =
  ...
  match result with
  | { Types.content = [ Types.Text t ]; _ } :: _ ->
    Alcotest.(check bool) "has summary prefix" true (String.length t > 0 && t.[0] = '[')
...
let test_summarize_old_all_recent () =
  ...
  let result =
    Context_reducer.reduce (Context_reducer.summarize_old ~keep_recent:5 ~summarizer) msgs
  in
...
let test_clear_tool_results_clears_old () =
  ...
  let msgs =
    ...
    ; tool_result_msg "t2" (String.make 300 'y')
    ; (* long result *)
      asst_msg "r2"
    ; user_msg "turn3"
    ; asst_msg "r3"
  ...
  in
  let result =
    Context_reducer.reduce (Context_reducer.clear_tool_results ~keep_recent:1) msgs
  in
  (* Old turns' tool results should be cleared *)
  let old_tool_results =
    List.filter_map
      (fun (m : Types.message) ->
         List.find_map
           (function
...
let test_clear_tool_results_preserves_recent () =
  ...
  let result =
    Context_reducer.reduce (Context_reducer.clear_tool_results ~keep_recent:5) msgs
  in
  let tool_results =
    List.filter_map
      (fun (m : Types.message) ->
  ...
  | [ content ] ->
    Alcotest.(check int) "preserved full length" 200 (String.length content)
...
let test_clear_tool_results_error_marker () =
  ...
  let msgs =
    ...
    ; Types.
        { role = Assistant
        ; content = [ ToolUse { id = "t1"; name = "fail"; input = `Null } ]
        ; name = None
        ; tool_call_id = None
        ; metadata = []
        }
  ...
  let has_error_marker =
    List.exists
      (fun (m : Types.message) ->
         List.exists
           (function
             | Types.ToolResult { content; _ } -> content = "[tool error result cleared]"
             | _ -> false)
           m.content)
      result
  in
    ...
    (match List.assoc "content" pairs with
     | `String s ->
       ...
       let has_truncated_marker =
         try
           let _ = Str.search_forward (Str.regexp_string "(truncated") s 0 in
           true
         with
         | Not_found -> false
       in
...
let test_orphaned_results_single () =
  ...
  let msgs = [ user_msg "q"; tool_result_msg "t1" "42"; asst_msg "done" ] in
  let result = Context_reducer.reduce Context_reducer.repair_orphaned_tool_results msgs in
...
let test_orphaned_results_after_compaction () =
  ...
  let msgs =
    [ user_msg "q"
    ; asst_msg "I will use a tool"
    ; (* ToolUse was here but got dropped *)
  ...
  Alcotest.run
    ...
    [ ( "keep_last_n"
      , [ Alcotest.test_case "fewer turns than N" `Quick test_keep_last_n_identity
        ...
        ; Alcotest.test_case
            "tool pairing: last turn only"
            `Quick
            test_keep_last_n_tool_pairing
        ; Alcotest.test_case
            "tool pair kept in turn"
            `Quick
            test_keep_last_n_keeps_tool_pair_in_turn
        ] )
    ; ( "token_budget"
    ...
    ; ( "importance_scored"
      , [ Alcotest.test_case
            "drops low scores"
            `Quick
            test_importance_scored_drops_low_scores
        ; Alcotest.test_case
            "boost preserves message"
            `Quick
            test_importance_scored_boost_preserves_message
        ] )
    ; ( "prune_tool_outputs"
    ...
    ; ( "merge_contiguous"
      , [ Alcotest.test_case "same role merged" `Quick test_merge_same_role
        ; Alcotest.test_case
            "tool results not merged"
            `Quick
            test_merge_preserves_tool_results
        ] )
    ; ( "drop_thinking"
    ...
    ; ( "keep_first_and_last"
      , [ Alcotest.test_case
            "identity when under limit"
            `Quick
            test_keep_first_and_last_identity
        ; Alcotest.test_case
            "drops middle turns"
            `Quick
            test_keep_first_and_last_drops_middle
        ] )
    ; ( "prune_by_role"
      , [ Alcotest.test_case "drops user messages" `Quick test_prune_by_role_drops_user
        ; Alcotest.test_case
            ...
            `Quick
            test_prune_by_role_preserves_tool_result
        ] )
    ; ( "summarize_old"
    ...
    ; ( "clear_tool_results"
      , [ Alcotest.test_case
            "clears old results"
            `Quick
            test_clear_tool_results_clears_old
        ; Alcotest.test_case
            "preserves recent"
            `Quick
            test_clear_tool_results_preserves_recent
        ; Alcotest.test_case
            "short untouched"
            `Quick
            test_clear_tool_results_short_untouched
    ...
    ; ( "prune_tool_args"
      , [ Alcotest.test_case
            "short args unchanged"
            `Quick
            test_prune_tool_args_short_unchanged
        ; Alcotest.test_case
            "long args truncated"
            `Quick
            test_prune_tool_args_long_truncated
        ; Alcotest.test_case
            "preserves recent turns"
            `Quick
            test_prune_tool_args_preserves_recent
        ; Alcotest.test_case
            "nested JSON truncated"
            `Quick
            test_prune_tool_args_nested_json
        ; Alcotest.test_case
            "non-assistant untouched"
            `Quick
            test_prune_tool_args_non_assistant_untouched
        ] )
    ; ( "repair_dangling_tool_calls"
    ...
    ; ( "repair_orphaned_tool_results"
      , [ Alcotest.test_case
            "no orphans unchanged"
            `Quick
            test_orphaned_results_no_orphans
    ...
    ; ( "cap_message_tokens_pair_preserving"
      , [ Alcotest.test_case "tool_use never dropped" `Quick test_cap_preserves_tool_use
        ; Alcotest.test_case
            "tool_result never dropped"
            `Quick
            test_cap_preserves_tool_result
        ; Alcotest.test_case
            "text truncated, pairs kept"
            `Quick
            test_cap_truncates_text_only
        ; Alcotest.test_case
