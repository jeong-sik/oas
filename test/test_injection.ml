...
let test_injection_type () =
  let inj : Hooks.injection = {
    ...
    extra_messages = [Types.{ role = User; content = [Text "injected observation"]; name = None; tool_call_id = None ; metadata = []}];
  } in
...
let test_injector_returns_some () =
  ...
  in
  (match injector ~tool_name:"bash" ~input:`Null ~output:(Ok { Types.content = "ok" }) with
  ...
  let injector : Hooks.context_injector =
    fun ~tool_name:_ ~input:_ ~output:_ ->
      Some {
        ...
        extra_messages = [
          Types.{ role = User; content = [Text "[system] git status: clean"]; name = None; tool_call_id = None ; metadata = []};
        ];
      }
  in
...
let () =
  run "injection" [
    ...
    "context", [
      test_case "updates applied" `Quick test_context_updates_applied;
    ];
  ]
