...
let test_complete_transport_override_emits_status () =
  Eio_main.run
  @@ fun env ->
  try
    Eio.Switch.run
    ...
    match Complete.complete ~sw ~net:env#net ~transport ~config ~messages ~metrics () with
    | Ok _ ->
      check int "transport emits synthetic status" 1 !hits;
      Eio.Switch.fail sw Exit
    | Error _ -> fail "expected Ok"
  with
  | Exit -> ()
  ...
  run
    ...
    ; ( "metrics"
      , [ test_case "callbacks" `Quick test_complete_metrics
        ...
        ; test_case
            "transport override emits status"
            `Quick
            test_complete_transport_override_emits_status
