...
let test_metric_low_diversity () =
  let s =
    make_summary
      ~tool_names:[ "a" ]
      ~tool_execution_started_count:100
...
let test_divergence_superset () =
  ...
  Alcotest.(check (float 0.01)) "jaccard for superset" 0.667 d.jaccard_distance;
...
let test_single_worker_moderate_diversity () =
  ...
  Alcotest.(check classification_testable)
    "moderate single -> insufficient"
    Insufficient_data
    v.classification;
  Alcotest.(check bool)
    "evidence mentions single"
    true
    (List.exists (fun e -> Util.string_contains ~needle:"single_worker" e) v.evidence)
;;
...
let test_single_worker_autonomous () =
  ...
  let v = analyze [ s ] in
  Alcotest.(check classification_testable)
    "single diverse worker -> autonomous"
    Autonomous
    v.classification;
...
let test_single_worker_scripted () =
  let s =
    make_summary
      ~tool_names:[ "only_one" ]
      ~tool_execution_started_count:20
      ~assistant_block_count:10
  in
...
let test_multi_worker_moderate_insufficient () =
  ...
  Alcotest.(check classification_testable)
    "moderate everything -> insufficient"
    Insufficient_data
    v.classification
;;
...
let test_multi_worker_tools_insufficient () =
  let w1 =
    make_summary
      ~tool_names:[ "a"; "b" ]
      ~tool_execution_started_count:2
...
let test_verdict_json_has_fields () =
  ...
  Alcotest.(check bool) "has evidence" true (Util.string_contains ~needle:"evidence" s);
  Alcotest.(check bool) "has metrics" true (Util.string_contains ~needle:"metrics" s)
;;
...
let test_four_workers_pairwise_count () =
  let make_worker name =
    make_summary
      ~tool_names:[ name ]
      ~tool_execution_started_count:1
      ~assistant_block_count:1
  in
  let v =
    analyze [ make_worker "a"; make_worker "b"; make_worker "c"; make_worker "d" ]
  in
  (* 4 choose 2 = 6 pairs *)
  Alcotest.(check int) "6 pairwise divergences" 6 (List.length v.pairwise_divergences)
;;
...
let test_default_thresholds () =
  ...
  Alcotest.(check (float 0.001)) "diversity_autonomous" 0.3 t.diversity_autonomous;
  Alcotest.(check (float 0.001)) "diversity_scripted" 0.15 t.diversity_scripted;
  Alcotest.(check (float 0.001)) "divergence_autonomous" 0.15 t.divergence_autonomous;
  Alcotest.(check (float 0.001)) "divergence_scripted" 0.05 t.divergence_scripted;
  Alcotest.(check int) "min_unique_tools" 3 t.min_unique_tools;
  Alcotest.(check (float 0.001)) "random_divergence" 0.8 t.random_divergence
;;
...
let test_empty_summaries () =
  ...
  Alcotest.(check int) "no divergences" 0 (List.length v.pairwise_divergences)
;;
...
let () =
  Alcotest.run
    ...
    ; ( "divergence"
      , [ Alcotest.test_case "single shared" `Quick test_divergence_single_shared
    ...
    ; ( "classify-multi"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case "all pass score" `Quick test_multi_worker_all_pass_score
        ; Alcotest.test_case "random" `Quick test_multi_worker_random
        ; Alcotest.test_case "divergence low" `Quick test_multi_worker_divergence_low
        ; Alcotest.test_case
            "tools insufficient"
            `Quick
    ...
    ; ( "pairwise"
      , [ Alcotest.test_case "four workers count" `Quick test_four_workers_pairwise_count
        ] )
    ; "edge_cases", [ Alcotest.test_case "empty summaries" `Quick test_empty_summaries ]
    ]
;;
