...
let test_string_contains_present () =
  check bool "needle in haystack" true
    (Util.string_contains ~needle:"bar" "foobarbaz")

let test_string_contains_absent () =
  check bool "needle missing" false
    (Util.string_contains ~needle:"qux" "foobarbaz")

let test_string_contains_empty_needle () =
  check bool "empty needle → true" true
    (Util.string_contains ~needle:"" "abc")

let test_string_contains_at_start () =
  check bool "at start" true
    (Util.string_contains ~needle:"foo" "foobar")

let test_string_contains_at_end () =
  check bool "at end" true
    (Util.string_contains ~needle:"baz" "foobaz")

let test_string_contains_needle_longer () =
  check bool "needle longer than haystack" false
    (Util.string_contains ~needle:"abcdef" "abc")

(* ── snoc / snoc_list ─────────────────────────────────── *)

let test_snoc_empty () =
  check (list int) "snoc to empty" [1] (Util.snoc [] 1)
...
let test_snoc_list_concat () =
  check (list int) "snoc_list concatenates" [1; 2; 3; 4]
...
let test_clip_short () =
  check string "no change when shorter" "hi" (Util.clip "hi" 10)

let test_clip_exact () =
  check string "no change when equal" "hello" (Util.clip "hello" 5)
...
let test_ci_match_lower () =
  check bool "needle in haystack lower" true
...
let test_ci_needle_longer () =
  check bool "needle longer → false" false
