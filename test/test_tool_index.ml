...
let entry name desc = Tool_index.{ name; description = desc; group = None; aliases = [] }
...
let test_single_tool_match () =
  let idx = Tool_index.build [ entry "read_file" "Read contents of a file from disk" ] in
...
let test_irrelevant_query () =
  let idx = Tool_index.build [ entry "git_commit" "Create a git commit with message" ] in
...
let test_confident_true () =
  let idx = Tool_index.build [ entry "search" "Search for files and directories" ] in
  check
    bool
    "confident on match"
    true
...
let test_confident_false () =
  let idx = Tool_index.build [ entry "search" "Search for files" ] in
  check
    bool
    "not confident on unrelated"
    false
...
let test_min_score_filter () =
  ...
  let idx = Tool_index.build ~config [ entry "tool" "a simple tool" ] in
...
let test_duplicate_names () =
  let idx =
    Tool_index.build
      [ entry "tool" "first description"; entry "tool" "second description" ]
  in
...
let () =
  run
    ...
    ; "groups", [ test_case "group co-retrieval" `Quick test_group_retrieval ]
    ; ( "confidence"
    ...
    ; "min_score", [ test_case "filtering" `Quick test_min_score_filter ]
    ; ( "edge_cases"
