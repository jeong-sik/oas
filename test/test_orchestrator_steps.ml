...
let append_step suffix : Orchestrator.step =
  fun input -> Ok (input ^ suffix)

let upper_step : Orchestrator.step =
  fun input -> Ok (String.uppercase_ascii input)

let fail_step msg : Orchestrator.step =
  fun _input -> Error (Error.Internal msg)
...
let test_step_run () =
  ...
  match Orchestrator.execute_step_plan ~sw
    (Step_run (append_step "_done")) ~input:"hello" with
  | Ok "hello_done" -> ()
...
let test_sequence_threads_output () =
  Eio_main.run @@ fun _env ->
  Eio.Switch.run @@ fun sw ->
  let plan = Orchestrator.Step_sequence [
...
let test_sequence_short_circuits () =
  ...
  let plan = Orchestrator.Step_sequence [
    ...
    Step_run (fun input -> ran_after := true; Ok input);
  ] in
  (match Orchestrator.execute_step_plan ~sw plan ~input:"x" with
   | Error _ -> ()
...
let test_parallel_all_receive_same_input () =
  Eio_main.run @@ fun _env ->
  Eio.Switch.run @@ fun sw ->
  let plan = Orchestrator.Step_parallel {
    plans = [
  ...
  | Ok result ->
    ...
    let has_base = List.for_all (fun p ->
...
let test_parallel_partial_failure () =
  ...
  let plan = Orchestrator.Step_parallel {
    plans = [
      Step_run (append_step "_ok");
      Step_run (fail_step "partial");
    ];
    max_concurrency = 2;
  } in
  match Orchestrator.execute_step_plan ~sw plan ~input:"x" with
  | Ok result ->
    Alcotest.(check bool) "has ok result" true (String.length result > 0)
...
let test_loop_converges () =
  Eio_main.run @@ fun _env ->
  Eio.Switch.run @@ fun sw ->
  let plan = Orchestrator.Step_loop {
  ...
  match Orchestrator.execute_step_plan ~sw plan ~input:"x" with
  | Ok result ->
    Alcotest.(check bool) "reached 5+" true (String.length result >= 5)
  | Error e -> Alcotest.failf "error: %s" (Error.to_string e)
...
let test_loop_max_iterations () =
  ...
  let plan = Orchestrator.Step_loop {
    ...
    until = (fun _ -> false);  (* never converge *)
    max_iterations = 3;
  } in
...
let test_step_as_tool () =
  let tool = Orchestrator.step_as_tool
    ~name:"upper" ~description:"Uppercase" upper_step in
  Alcotest.(check string) "name" "upper" tool.schema.name;
  match Tool.execute tool (`Assoc [("input", `String "hello")]) with
  | Ok { content } ->
    Alcotest.(check string) "HELLO" "HELLO" content
  | Error { message; _ } -> Alcotest.failf "error: %s" message

(* ── Suite ───────────────────────────────────────────────────── *)

let () =
  Alcotest.run "orchestrator_steps" [
    ("step_run", [
      Alcotest.test_case "basic" `Quick test_step_run;
    ]);
    ("sequence", [
    ...
    ("tool", [
      Alcotest.test_case "step_as_tool" `Quick test_step_as_tool;
    ]);
  ]
