...
let parse json =
  ...
  try Ok (json |> member "name" |> to_string) with
  | _ -> Error "bad input"
;;

let handle name = if name = "" then Error "empty" else Ok ("Hello, " ^ name)
...
let test_write_denied () =
  ...
  | Error e ->
    ...
    Alcotest.(check bool) "mentions denied" true (String.length e.message > 0)
;;
