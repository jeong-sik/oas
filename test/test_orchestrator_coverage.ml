...
let test_create_empty () =
  ...
  Alcotest.(check bool) "no agents" true
    (Orchestrator.find_agent orch "missing" = None)
...
let test_create_with_agents () =
  ...
  Alcotest.(check bool) "found" true
    (Orchestrator.find_agent orch "a1" <> None);
  Alcotest.(check bool) "not found" true
    (Orchestrator.find_agent orch "a2" = None)
...
let test_add_agent () =
  ...
  Alcotest.(check bool) "found after add" true
...
let test_eval_text_contains () =
  ...
  Alcotest.(check bool) "contains no match" false
    (Orchestrator.eval_condition
       (Some (make_ok_result "hello"))
       (Orchestrator.TextContains "world"));
  Alcotest.(check bool) "contains with error" false
    (Orchestrator.eval_condition
       (Some (make_err_result ()))
       (Orchestrator.TextContains "anything"));
  Alcotest.(check bool) "contains with None" false
    (Orchestrator.eval_condition None (Orchestrator.TextContains "x"))

let test_eval_custom () =
  let cond = Orchestrator.Custom_cond (fun tr ->
    match tr.result with Ok _ -> true | Error _ -> false) in
  Alcotest.(check bool) "custom ok" true
    (Orchestrator.eval_condition (Some (make_ok_result "x")) cond);
  Alcotest.(check bool) "custom err" false
    (Orchestrator.eval_condition (Some (make_err_result ())) cond);
  Alcotest.(check bool) "custom none" true
    (Orchestrator.eval_condition None cond)

let test_eval_and () =
  Alcotest.(check bool) "And all true" true
    (Orchestrator.eval_condition (Some (make_ok_result "hello world"))
       (Orchestrator.And [Orchestrator.Always; Orchestrator.ResultOk]));
  Alcotest.(check bool) "And with false" false
    (Orchestrator.eval_condition (Some (make_err_result ()))
       (Orchestrator.And [Orchestrator.Always; Orchestrator.ResultOk]))

let test_eval_or () =
  Alcotest.(check bool) "Or with one true" true
    (Orchestrator.eval_condition (Some (make_err_result ()))
       (Orchestrator.Or [Orchestrator.Always; Orchestrator.ResultOk]));
  Alcotest.(check bool) "Or all false" false
    (Orchestrator.eval_condition (Some (make_err_result ()))
       (Orchestrator.Or [Orchestrator.ResultOk;
                         Orchestrator.TextContains "nope"]))

let test_eval_not () =
  Alcotest.(check bool) "Not Always" false
    (Orchestrator.eval_condition None (Orchestrator.Not Orchestrator.Always));
  Alcotest.(check bool) "Not ResultOk" true
    (Orchestrator.eval_condition (Some (make_err_result ()))
...
let test_select_first_ok () =
  let results = [make_err_result (); make_ok_result "winner"; make_ok_result "second"] in
...
let () =
  Alcotest.run "Orchestrator_coverage" [
    "config", [
