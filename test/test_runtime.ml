...
let test_default_local_first_options () =
  Alcotest.(check (option string))
    "default provider"
    (Some "local")
    Client.default_options.provider;
  Alcotest.(check (option string)) "default model" None Client.default_options.model
;;
...
let test_runtime_attach_artifact_and_read_back () =
  ...
  let session =
    ...
    | other -> Alcotest.fail (Runtime.show_response other)
  in
  Alcotest.(check int) "artifact count in status" 1 (List.length session.artifacts);
  let listed =
      ...
      unwrap_response
        (runtime_query
           ...
           (Runtime.Apply_command
              { session_id = session.session_id
              ; command =
                  Runtime.Attach_artifact { name = "summary"; kind = "markdown"; content }
              }))
    with
...
let test_runtime_finalize_generates_telemetry_and_evidence () =
  ...
  if Sys.getenv_opt "OAS_RUNTIME_TELEMETRY_TEST" <> Some "1"
  then ()
  else
    Eio_main.run
    @@ fun env ->
              ...
              (function
                | Client.Session_events events ->
                  List.exists
                    (function
                      | { Runtime.kind =
                            Runtime.Agent_completed _ | Runtime.Agent_failed _
                        ; _
                        } -> true
                      | _ -> false)
                    events
    ...
    let artifacts = unwrap (Sessions.list_artifacts ~session_root ~session_id ()) in
    ...
    Alcotest.(check bool) "has evidence json" true (List.mem "runtime-evidence" names);
    ...
    let worker_runs = unwrap (Sessions.get_worker_runs ~session_root ~session_id ()) in
    let latest_worker =
    ...
    Alcotest.(check bool)
      ...
      (List.exists
         ...
         telemetry.steps);
    Alcotest.(check int)
      "structured telemetry step_count matches steps"
      structured_telemetry.step_count
      (List.length structured_telemetry.steps);
    Alcotest.(check bool)
      "structured telemetry has session_started"
      true
      (List.exists
    ...
    Alcotest.(check bool)
      "structured telemetry has agent spawn metadata"
      true
      (List.exists
         (fun (step : Sessions.structured_telemetry_step) ->
         ...
         structured_telemetry.steps);
    Alcotest.(check bool)
      "structured telemetry count tracks session_started"
      true
      (List.exists
    ...
    Alcotest.(check bool)
      "structured telemetry count tracks turn_recorded"
      true
      (List.exists
         (fun (count : Sessions.structured_event_count) ->
    ...
    (match worker_runs with
     | worker :: _ ->
       ...
       Alcotest.(check (option string)) "primary alias none" None worker.primary_alias;
       Alcotest.(check (option string)) "worker provider" (Some "mock") worker.provider;
    ...
    Alcotest.(check bool)
      "evidence tracks persisted files"
      true
      (List.length evidence.files >= 6);
    Alcotest.(check bool)
      "evidence includes report json file"
      true
      (List.exists
         (fun (file : Sessions.evidence_file) -> String.equal file.label "report_json")
         evidence.files);
    Alcotest.(check bool)
      "evidence includes proof json file"
      true
      (List.exists
         (fun (file : Sessions.evidence_file) -> String.equal file.label "proof_json")
         evidence.files);
    Alcotest.(check int)
      "hook summary mirrors bundle"
      (List.length hook_summary)
      (List.length bundle.hook_summary);
    Alcotest.(check int)
      "tool catalog empty for runtime mock"
      0
      (List.length tool_catalog);
    Alcotest.(check string) "bundle session id" session_id bundle.session.session_id;
    Alcotest.(check string) "bundle report session id" session_id bundle.report.session_id;
    Alcotest.(check string) "bundle proof session id" session_id bundle.proof.session_id;
    Alcotest.(check bool) "bundle proof ok" true bundle.proof.ok;
    Alcotest.(check int)
      "bundle structured telemetry steps"
      structured_telemetry.step_count
      (List.length bundle.structured_telemetry.steps);
    Alcotest.(check int) "bundle raw trace runs one" 1 (List.length bundle.raw_trace_runs);
    Alcotest.(check bool)
      "bundle latest raw trace run exists"
      true
      (Option.is_some bundle.latest_raw_trace_run);
    Alcotest.(check int)
      "bundle raw trace summaries one"
      1
      (List.length bundle.raw_trace_summaries);
    Alcotest.(check int)
      "bundle raw trace validations one"
      1
      (List.length bundle.raw_trace_validations);
    Alcotest.(check int) "bundle worker runs one" 1 (List.length bundle.worker_runs);
    Alcotest.(check bool)
      "bundle latest accepted none"
      true
      (bundle.latest_accepted_worker_run = None);
    Alcotest.(check bool)
      "bundle latest ready none"
      true
      (bundle.latest_ready_worker_run = None);
    Alcotest.(check bool)
      "bundle latest running none"
      true
      (bundle.latest_running_worker_run = None);
    Alcotest.(check bool)
      "bundle latest worker exists"
      true
      (Option.is_some bundle.latest_worker_run);
    Alcotest.(check bool)
      "bundle latest completed exists"
      true
      (Option.is_some bundle.latest_completed_worker_run);
    Alcotest.(check bool)
      "bundle latest validated worker exists"
      true
      (Option.is_some bundle.latest_validated_worker_run);
    Alcotest.(check bool)
      "bundle latest failed worker none"
      true
      (bundle.latest_failed_worker_run = None);
    Alcotest.(check bool)
      "bundle hook summary accessible"
      true
      (List.length bundle.hook_summary >= 0);
    Alcotest.(check int) "bundle tool catalog empty" 0 (List.length bundle.tool_catalog);
    Alcotest.(check int)
      "bundle validated worker runs one"
      1
      (List.length bundle.validated_worker_runs);
    Alcotest.(check int) "bundle raw trace run count one" 1 bundle.raw_trace_run_count;
    Alcotest.(check int)
      "bundle validated worker run count one"
      1
      bundle.validated_worker_run_count;
    Alcotest.(check bool)
      "bundle trace capabilities raw present"
      true
      (List.exists (( = ) Sessions.Raw) bundle.trace_capabilities);
    Alcotest.(check bool)
      "bundle capabilities raw trace true"
      true
      bundle.capabilities.raw_trace;
    Alcotest.(check bool)
      "bundle capabilities validated summary true"
      true
      bundle.capabilities.validated_summary;
    Alcotest.(check bool)
      "bundle capabilities proof bundle true"
      true
...
let test_high_level_query_and_sessions () =
  ...
  Alcotest.(check (option string))
    "title updated"
    (Some "Renamed runtime session")
    session.title;
  Alcotest.(check (option string)) "tag updated" (Some "experiment") session.tag;
...
let test_control_roundtrip_callbacks () =
  ...
  Client.set_can_use_tool client (fun action payload _ctx ->
    ...
    let subject =
      payload
      |> Yojson.Safe.Util.member "participant_name"
  ...
  Alcotest.(check bool)
    "session start hook seen"
    true
    (List.mem "SessionStart" !hook_calls);
  ...
  | Some worker ->
    Alcotest.(check string) "failed worker name" "guarded-worker" worker.agent_name;
    Alcotest.(check bool) "failed worker status" true (worker.status = Sessions.Failed);
    Alcotest.(check (option string))
      "failed worker resolved provider"
      (Some "mock")
      worker.resolved_provider;
         ...
         List.exists
           (function
             | Client.Session_events events ->
               List.exists
                 (function
                   | { Runtime.kind = Runtime.Agent_completed _ | Runtime.Agent_failed _
                     ; _
                     } -> true
                   | _ -> false)
                 events
...
let test_resume_existing_session () =
  ...
  Alcotest.(check (option string))
    "resumed session id"
    (Some session_id)
    (Client.current_session_id client2);
  ...
  Alcotest.(check bool) "resumed batch has messages" true (List.length resumed_batch >= 3)
;;
  ...
  Alcotest.run
    ...
    ; ( "artifacts"
      , [ Alcotest.test_case
        ...
        ; (* Mock subprocess timing makes this non-deterministic; skip in CI *)
          Alcotest.test_case
            "finalize generates telemetry and evidence"
            `Slow
    ...
    ; ( "sdk"
      , [ Alcotest.test_case
        ...
        ; Alcotest.test_case
            "control roundtrip callbacks"
            `Quick
            test_control_roundtrip_callbacks
        ; Alcotest.test_case
            "receive messages progressively"
            `Quick
            test_receive_messages_streams_progressively
        ; Alcotest.test_case
            "long lived client multiple turns"
            `Quick
            test_long_lived_client_multiple_turns
        ; Alcotest.test_case "resume existing session" `Quick test_resume_existing_session
        ; Alcotest.test_case
            "session settings persist across resume"
            `Quick
