...
let make_agent ~net ?(max_turns = 3) ?(tools = []) ?hooks ?context_reducer
    ?guardrails ?provider base_url =
  let config = { Types.default_config with
    name = "cov-agent"; max_turns;
  } in
  let prov = match provider with
    | Some p -> Some p
    | None ->
      Some { Provider.provider = Provider.Local { base_url };
             model_id = "mock-model"; api_key_env = "" }
  in
...
let test_tool_call_loop () =
  ...
  try
    ...
    let tool = Tool.create ~name:"calc" ~description:"Calculate"
        ~parameters:[{ name = "x"; param_type = Types.Integer;
                       description = "number"; required = true }]
        (fun input ->
          let x = Yojson.Safe.Util.(input |> member "x" |> to_int) in
          Ok { Types.content = string_of_int x }) in
...
let test_multi_content_tool () =
  ...
  try
    ...
    let tool = Tool.create ~name:"greet" ~description:"Greet"
        ~parameters:[{ name = "name"; param_type = Types.String;
                       description = "who"; required = true }]
        (fun _input -> Ok { Types.content = "hello" }) in
    let agent = make_agent ~net:env#net ~tools:[tool] ~max_turns:5 url in
...
let test_hooks_turn () =
  ...
  try
    ...
    let hooks = { Hooks.empty with
      before_turn = Some (fun _e -> incr before_count; Hooks.Continue);
      after_turn = Some (fun _e -> incr after_count; Hooks.Continue);
    } in
...
let test_pre_tool_skip () =
  ...
  try
    ...
    let tool = Tool.create ~name:"skipped" ~description:"Skip me"
        ~parameters:[]
        (fun _input -> Ok { Types.content = "should not run" }) in
    let hooks = { Hooks.empty with
      pre_tool_use = Some (fun _e -> Hooks.Skip);
    } in
    let agent = make_agent ~net:env#net ~tools:[tool]
...
let test_openai_compat () =
  ...
  try
    Eio.Switch.run @@ fun sw ->
    let url = start_multi ~sw ~net:env#net ~port:21014
        [openai_response "openai hello"] in
...
let test_structured_extract () =
  ...
  try
    ...
    let schema : (string * int) Structured.schema = {
      ...
      params = [
        { name = "name"; param_type = Types.String;
          description = "name"; required = true };
        { name = "age"; param_type = Types.Integer;
          description = "age"; required = true };
      ];
      parse = (fun json ->
        let open Yojson.Safe.Util in
        Ok (json |> member "name" |> to_string,
            json |> member "age" |> to_int));
    } in
    let config = { Types.default_config with
...
let test_schema_to_json () =
  let schema : unit Structured.schema = {
    ...
    description = "A test";
    params = [
      { name = "x"; param_type = Types.Integer;
        description = "num"; required = true };
      { name = "y"; param_type = Types.String;
...
let test_context_tool () =
  ...
  try
    ...
    let tool = Tool.create_with_context
        ...
        ~parameters:[
          { name = "key"; param_type = Types.String;
            description = "k"; required = true }]
        (fun _ctx _input -> Ok { Types.content = "ctx_result" }) in
    let agent = make_agent ~net:env#net ~tools:[tool] ~max_turns:5 url in
...
let () =
  ...
  run "full_pipeline_cov" [
    ...
    "streaming", [
      test_case "sse streaming" `Quick test_streaming_sse;
    ];
    "structured", [
