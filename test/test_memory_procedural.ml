...
let () = Alcotest.run "Memory_Procedural" [
  "store_recall", [
    Alcotest.test_case "store and recall procedure" `Quick (fun () ->
      let mem = Memory.create () in
      let proc : Memory.procedure = {
        id = "pr1"; pattern = "deploy error";
        action = "rollback then retry"; success_count = 5; failure_count = 1;
        confidence = 5.0 /. 6.0; last_used = 1000.0; metadata = [];
      } in
  ...
  "matching", [
    Alcotest.test_case "pattern substring match" `Quick (fun () ->
      ...
      Memory.store_procedure mem {
        id = "pr2"; pattern = "test failure"; action = "rerun";
        success_count = 3; failure_count = 7; confidence = 0.3;
        last_used = 100.0; metadata = [];
      };
      Memory.store_procedure mem {
        id = "pr3"; pattern = "deploy timeout"; action = "increase timeout";
        success_count = 2; failure_count = 1; confidence = 0.667;
        last_used = 100.0; metadata = [];
      };
    ...
    Alcotest.test_case "min_confidence filter" `Quick (fun () ->
      let mem = Memory.create () in
      Memory.store_procedure mem {
        id = "pr1"; pattern = "task"; action = "do it";
        success_count = 1; failure_count = 9; confidence = 0.1;
        last_used = 100.0; metadata = [];
      };
      let matches = Memory.matching_procedures mem ~pattern:"task"
        ~min_confidence:0.5 () in
  ...
  "record_outcome", [
    Alcotest.test_case "record_success updates confidence" `Quick (fun () ->
      ...
      Memory.store_procedure mem {
        id = "pr1"; pattern = "task"; action = "do it";
        success_count = 1; failure_count = 1; confidence = 0.5;
        last_used = 100.0; metadata = [];
      };
