...
let test_options_full () =
  ...
  in
  Alcotest.(check (option string)) "session_root" (Some "/tmp/sessions")
    opts.session_root;
  Alcotest.(check (list string)) "aliases" [ "deployer"; "captain" ]
    opts.aliases;
...
let test_options_special_chars_in_goal () =
  ...
  in
  Alcotest.(check string) "goal with special chars"
    "fix bug #123 with path/to/file" opts.goal
...
let test_raw_details_populated () =
  ...
  Alcotest.(check (list string)) "tools"
    [ "file_read"; "shell_exec" ] d.tool_names;
  Alcotest.(check int) "paired count" 5 d.paired_tool_result_count;
  Alcotest.(check bool) "has file write" true d.has_file_write;
  Alcotest.(check bool) "verification pass" true
    d.verification_pass_after_file_write
...
let test_raw_details_with_error () =
  ...
  in
  Alcotest.(check (option string)) "error present"
    (Some "Connection timeout") d.error;
  Alcotest.(check (option string)) "failure reason"
    (Some "Connection timeout") d.failure_reason
...
let test_options_whitespace_session_id () =
  ...
  in
  Alcotest.(check string) "session_id preserved" "  spaced-id  "
    opts.session_id
...
let () =
  Alcotest.run "direct-evidence-unit"
    [
      ( "options",
        [
          ...
          Alcotest.test_case "special chars" `Quick
            test_options_special_chars_in_goal;
          ...
          Alcotest.test_case "whitespace strings" `Quick
