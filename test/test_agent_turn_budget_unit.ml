...
let test_multiple_extensions () =
  ...
  check_bool "3 history entries" true (List.length _history = 3)
;;
...
let () =
  Alcotest.run
    ...
    [ ( "create"
      , [ Alcotest.test_case "basic" `Quick test_create_basic
        ; Alcotest.test_case
            "ceiling >= initial"
            `Quick
            test_create_ceiling_at_least_initial
        ] )
    ; ( "try_extend"
    ...
    ; "accumulation", [ Alcotest.test_case "multiple" `Quick test_multiple_extensions ]
    ]
;;
