...
let test_state_yojson_roundtrip () =
  let states = [
    A2a_task.Submitted; A2a_task.Working; A2a_task.Input_required;
    A2a_task.Completed; A2a_task.Failed; A2a_task.Canceled
  ] in
  List.iter (fun state ->
...
let test_show_task_state () =
  Alcotest.(check string) "show" "completed"
    (A2a_task.show_task_state A2a_task.Completed)
...
let test_is_terminal () =
  ...
  Alcotest.(check bool) "input_required" false (A2a_task.is_terminal A2a_task.Input_required);
...
let test_transition_error_to_string () =
  let e1 = A2a_task.InvalidTransition {
    from_state = A2a_task.Submitted; to_state = A2a_task.Completed } in
...
let mk_msg ?(role = A2a_task.TaskUser) ?(parts = [A2a_task.Text_part "hello"])
    ?(metadata = []) () : A2a_task.task_message =
  { role; parts; metadata }
...
let test_create_task () =
  ...
  Alcotest.(check string) "initial state" "submitted"
...
let test_transition_terminal () =
  let task = A2a_task.create (mk_msg ()) in
  let task = match A2a_task.transition task A2a_task.Working with
    | Ok t -> t | Error _ -> Alcotest.fail "first transition" in
  let task = match A2a_task.transition task A2a_task.Completed with
    | Ok t -> t | Error _ -> Alcotest.fail "second transition" in
  match A2a_task.transition task A2a_task.Working with
...
let test_transition_chain () =
  let task = A2a_task.create (mk_msg ()) in
  let task = match A2a_task.transition task A2a_task.Working with
    | Ok t -> t | Error _ -> Alcotest.fail "to working" in
  let task = match A2a_task.transition task A2a_task.Input_required with
    | Ok t -> t | Error _ -> Alcotest.fail "to input_required" in
  let task = match A2a_task.transition task A2a_task.Working with
    | Ok t -> t | Error _ -> Alcotest.fail "back to working" in
  let _task = match A2a_task.transition task A2a_task.Failed with
    | Ok t -> t | Error _ -> Alcotest.fail "to failed" in
  ()
...
let test_add_message () =
  ...
  let msg2 = mk_msg ~role:A2a_task.TaskAgent ~parts:[A2a_task.Text_part "response"] () in
...
let test_task_json_roundtrip () =
  ...
  let task = A2a_task.add_message task
      (mk_msg ~role:A2a_task.TaskAgent ~parts:[A2a_task.Text_part "resp"] ()) in
  let task = A2a_task.add_artifact task
      { name = "out"; parts = [A2a_task.Data_part (`Int 1)]; metadata = [] } in
  let task = match A2a_task.transition task A2a_task.Working with
    | Ok t -> t | Error _ -> Alcotest.fail "transition" in
  ...
  | Ok task2 ->
    ...
    Alcotest.(check string) "state" "working"
      (A2a_task.task_state_to_string task2.state);
    Alcotest.(check int) "messages" (List.length task.messages) (List.length task2.messages);
...
let test_task_of_yojson_bad_message () =
  let json = `Assoc [
    ...
    ("messages", `List [
      `Assoc [("role", `String "bad_role"); ("parts", `List []); ("metadata", `Assoc [])]
    ]);
    ("artifacts", `List []);
    ("metadata", `Assoc []);
    ("created_at", `Float 1.0);
...
let test_task_of_yojson_bad_artifact () =
  let json = `Assoc [
    ...
    ("messages", `List []);
    ("artifacts", `List [
      `Assoc [("name", `Int 123); ("parts", `List []); ("metadata", `Assoc [])]
    ]);
    ("metadata", `Assoc []);
    ("created_at", `Float 1.0);
    ("updated_at", `Float 1.0);
...
let test_store_basic () =
  ...
  | Some t ->
    Alcotest.(check string) "id" task.id t.id
...
let test_store_update () =
  ...
  let task = A2a_task.create (mk_msg ()) in
  A2a_task.store_task store task;
  let task2 = match A2a_task.transition task A2a_task.Working with
    | Ok t -> t | Error _ -> Alcotest.fail "transition" in
  A2a_task.store_task store task2;
  match A2a_task.get_task store task.id with
  | None -> Alcotest.fail "not found"
  | Some t ->
    Alcotest.(check string) "updated state" "working"
