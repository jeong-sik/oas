...
let make_structured_telemetry ?(session_id = "sess-c2") () : Sessions.structured_telemetry
  =
  { session_id; generated_at = 1000.0; step_count = 0; event_counts = []; steps = [] }
;;
...
let make_run_ref ?(worker_run_id = "run-c2") ?(agent_name = "agent-c2") ()
  ...
  ; end_seq = 10
  ; agent_name
  ; session_id = Some "sess-c2"
  }
;;

let make_worker
      ?(worker_run_id = "run-c2")
      ?(worker_id = Some "w-c2")
      ?(agent_name = "agent-c2")
      ?(runtime_actor = Some "actor-c2")
      ?(role = None)
      ?(aliases = [ "alias-c2" ])
      ?(primary_alias = Some "alias-c2")
...
let make_validation ?(worker_run_id = "run-c2") ?(agent_name = "agent-c2") ?(ok = true) ()
  : Raw_trace.run_validation
  =
  { run_ref = make_run_ref ~worker_run_id ~agent_name ()
  ; ok
  ; checks = []
...
let test_multi_worker_statuses () =
  ...
  let bundle =
    make_bundle
      ...
      ~latest_completed_worker_run:(Some w_completed)
      ()
  in
...
let test_validated_summary_only () =
  let w = make_worker ~validated:true ~trace_capability:Sessions.Summary_only () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
...
let test_no_trace_passes_evidence () =
  let w = make_worker ~trace_capability:Sessions.No_trace ~validated:false () in
...
let test_multi_trace_capabilities () =
  let w1 = make_worker ~worker_run_id:"r1" ~trace_capability:Sessions.Raw () in
  let w2 =
    make_worker
      ~worker_run_id:"r2"
...
let () =
  run
    ...
    ; "serialization", [ test_case "report roundtrip" `Quick test_report_json_roundtrip ]
    ]
;;
