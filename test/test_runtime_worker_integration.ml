...
let with_temp_dir f =
  let dir =
    Filename.concat
      ...
      (Printf.sprintf "oas-worker-test-%d-%06x" (Unix.getpid ()) (Random.int 0xFFFFFF))
  in
...
let make_store dir =
  ...
  | Ok store ->
    (match Runtime_store.ensure_tree store session_id with
     | Ok () -> store
     | Error e ->
       Alcotest.fail (Printf.sprintf "ensure_tree failed: %s" (Error.to_string e)))
  | Error e ->
    Alcotest.fail (Printf.sprintf "store create failed: %s" (Error.to_string e))
;;
...
let save_initial_session store =
  ...
  (match Runtime_store.save_session store session with
   | Ok () -> ()
   | Error e ->
     Alcotest.fail (Printf.sprintf "save_session failed: %s" (Error.to_string e)));
  session
;;
...
let test_extract_text_multiple () =
  let resp : Types.api_response =
    ...
    ; model = "m"
    ; stop_reason = EndTurn
    ; content = [ Types.Text "line1"; Types.Text "line2" ]
    ; usage = None
    ; telemetry = None
    }
  in
...
let test_extract_text_only_non_text () =
  let resp : Types.api_response =
    ...
    ; model = "m"
    ; stop_reason = EndTurn
    ; content = [ Types.ToolUse { id = "t1"; name = "fn"; input = `Null } ]
    ; usage = None
    ; telemetry = None
    }
  in
...
let test_make_event_seq () =
  let session = Runtime_projection.initial_session (make_start_request ()) in
  let event =
...
let test_with_store_lock_exception_passthrough () =
  ...
  match
    try
      let _ = Runtime_server_worker.with_store_lock state (fun () -> failwith "boom") in
      None
    with
...
let test_with_store_lock_cancellation_does_not_poison_mutex () =
  ...
  (try
   ...
   | Exit -> ());
  let result =
    try Ok (Runtime_server_worker.with_store_lock state (fun () -> 42)) with
    | Eio.Mutex.Poisoned exn ->
      Error ("store mutex was poisoned by cancellation: " ^ Printexc.to_string exn)
    | exn -> Error ("unexpected exception after cancellation: " ^ Printexc.to_string exn)
  in
  match result with
  | Ok value -> Alcotest.(check int) "store mutex usable after cancellation" 42 value
...
let test_persist_event_locked_basic () =
  ...
  with_temp_dir (fun dir ->
    ...
    let kind : Runtime.event_kind =
      Session_started { goal = session.goal; participants = session.planned_participants }
    in
    ...
    | Ok (updated_session, event) ->
      ...
      (match Runtime_store.load_session store session_id with
       | Ok loaded -> Alcotest.(check int) "disk last_seq" 1 loaded.last_seq
       | Error e -> Alcotest.fail (Printf.sprintf "load failed: %s" (Error.to_string e)))
    | Error e ->
      Alcotest.fail (Printf.sprintf "persist_event_locked failed: %s" (Error.to_string e)))
;;
...
let test_persist_event_locked_multiple () =
  ...
  with_temp_dir (fun dir ->
    ...
    | Error e -> Alcotest.fail (Printf.sprintf "event 1 failed: %s" (Error.to_string e))
    | Ok (session2, _ev1) ->
      ...
      (match Runtime_server_worker.persist_event_locked store state session2 kind2 with
       ...
       | Ok (session3, ev2) ->
         ...
         (match Runtime_store.read_events store session_id () with
          | Ok events -> Alcotest.(check int) "2 events on disk" 2 (List.length events)
          | Error e ->
...
let test_persist_event_by_session_id () =
  ...
  with_temp_dir (fun dir ->
    ...
    | Error e ->
      Alcotest.fail (Printf.sprintf "persist_event failed: %s" (Error.to_string e)))
;;
...
let test_persist_event_nonexistent_session () =
  ...
  with_temp_dir (fun dir ->
    let state = make_state net dir in
    let store = make_store dir in
    let kind : Runtime.event_kind = Turn_recorded { actor = None; message = "x" } in
    match Runtime_server_worker.persist_event store state "nonexistent-id" kind with
...
let test_persist_event_turn_recorded () =
  ...
  with_temp_dir (fun dir ->
    ...
    let _session = save_initial_session store in
    (match
       Runtime_server_worker.persist_event
         store
         state
         session_id
         (Session_started { goal = "test"; participants = [ "alice"; "bob" ] })
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail (Printf.sprintf "start failed: %s" (Error.to_string e)));
    let kind2 : Runtime.event_kind =
    ...
    | Error e -> Alcotest.fail (Printf.sprintf "turn failed: %s" (Error.to_string e)))
;;
...
let test_persist_event_spawn () =
  ...
  with_temp_dir (fun dir ->
    ...
    in
    match Runtime_server_worker.persist_event store state session_id kind with
    | Ok (_sess, ev) -> Alcotest.(check int) "spawn seq" 2 ev.seq
    | Error e -> Alcotest.fail (Printf.sprintf "spawn failed: %s" (Error.to_string e)))
;;
  ...
  with_temp_dir (fun dir ->
    ...
    (match
       Runtime_server_worker.persist_event
         ...
         state
         session_id
         (Session_started { goal = "integration test"; participants = [ "alice"; "bob" ] })
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail (Error.to_string e));
    (match
     ...
     | Ok _ -> ()
     | Error e -> Alcotest.fail (Error.to_string e));
    match Runtime_server_worker.generate_report_and_proof store state session_id with
    | Ok (final_session, report, proof) ->
      Alcotest.(check string) "report session_id" session_id report.session_id;
      Alcotest.(check bool) "report has summary" true (List.length report.summary > 0);
      Alcotest.(check bool) "report has markdown" true (String.length report.markdown > 0);
      Alcotest.(check string) "proof session_id" session_id proof.session_id;
      Alcotest.(check bool) "proof has checks" true (List.length proof.checks > 0);
      let report_path = Runtime_store.report_json_path store session_id in
      Alcotest.(check bool) "report.json exists" true (Sys.file_exists report_path);
      let proof_path = Runtime_store.proof_json_path store session_id in
      Alcotest.(check bool) "proof.json exists" true (Sys.file_exists proof_path);
      Alcotest.(check bool)
        "session has artifacts"
        true
        (List.length final_session.artifacts > 0);
      Alcotest.(check bool)
        "at least 3 artifacts"
        true
        (List.length final_session.artifacts >= 3);
      let structured =
        match Sessions.get_telemetry_structured ~session_root:dir ~session_id () with
        | Ok telemetry -> telemetry
        | Error e ->
          Alcotest.fail
            (Printf.sprintf "structured telemetry failed: %s" (Error.to_string e))
      in
      Alcotest.(check bool)
        "telemetry tracks telemetry artifact attachment"
        true
        (List.exists
      ...
      let evidence =
        ...
        | Ok bundle -> bundle
        | Error e ->
          Alcotest.fail (Printf.sprintf "evidence read failed: %s" (Error.to_string e))
      in
      Alcotest.(check bool)
        "evidence includes telemetry_json"
        true
        (List.exists
           ...
           evidence.files);
      Alcotest.(check bool)
        "evidence includes telemetry_md"
        true
        (List.exists
           (fun (file : Sessions.evidence_file) -> String.equal file.label "telemetry_md")
           evidence.files);
      Alcotest.(check bool)
        "evidence includes raw_trace_json"
        true
        (List.exists
           (fun (file : Sessions.evidence_file) ->
      ...
      let raw_trace_manifest =
        ...
        | Error e ->
          Alcotest.fail
            (Printf.sprintf "raw trace manifest read failed: %s" (Error.to_string e))
      in
      Alcotest.(check bool)
        "raw trace manifest has run list"
        true
        (List.length raw_trace_manifest.raw_trace_runs >= 0)
    | Error e ->
      Alcotest.fail
        (Printf.sprintf "generate_report_and_proof failed: %s" (Error.to_string e)))
;;
...
let test_generate_report_and_proof_empty_events () =
  ...
  with_temp_dir (fun dir ->
    ...
    | Error e ->
      Alcotest.fail
        (Printf.sprintf "report with no events failed: %s" (Error.to_string e)))
;;
...
let test_generate_report_nonexistent_session () =
  ...
  with_temp_dir (fun dir ->
    ...
    let store = make_store dir in
    match
      Runtime_server_worker.generate_report_and_proof store state "no-such-session"
    with
    | Ok _ -> Alcotest.fail "expected error for missing session"
    | Error _ -> ())
;;
...
let test_emit_output_delta_empty () =
  ...
  with_temp_dir (fun dir ->
    ...
    let store = make_store dir in
    let _session = save_initial_session store in
    match Runtime_server_worker.emit_output_delta store state session_id "alice" "" with
    | Ok () -> ()
    | Error e ->
      Alcotest.fail (Printf.sprintf "empty delta failed: %s" (Error.to_string e)))
;;
...
let test_emit_output_delta_whitespace_only () =
  ...
  with_temp_dir (fun dir ->
    ...
    let _session = save_initial_session store in
    match
      Runtime_server_worker.emit_output_delta store state session_id "alice" "   \t\n  "
    with
    | Ok () -> ()
    | Error e ->
      Alcotest.fail (Printf.sprintf "whitespace delta failed: %s" (Error.to_string e)))
;;
...
let test_emit_output_delta_nonempty () =
  ...
  with_temp_dir (fun dir ->
    ...
    (match
     ...
     | Error e -> Alcotest.fail (Error.to_string e));
    match
      Runtime_server_worker.emit_output_delta store state session_id "alice" "hello world"
    with
    | Ok () ->
      (match Runtime_store.read_events store session_id () with
       | Ok events ->
       ...
       | Error e -> Alcotest.fail (Printf.sprintf "read_events: %s" (Error.to_string e)))
    | Error e ->
      Alcotest.fail (Printf.sprintf "non-empty delta failed: %s" (Error.to_string e)))
;;
...
let test_emit_output_delta_nonexistent_session () =
  ...
  with_temp_dir (fun dir ->
    ...
    let store = make_store dir in
    match
      Runtime_server_worker.emit_output_delta store state "no-such-session" "alice" "text"
    with
    | Ok _ -> Alcotest.fail "expected error for missing session"
    | Error _ -> ())
;;
...
let test_persist_agent_became_live () =
  ...
  with_temp_dir (fun dir ->
    ...
    | Ok (sess, ev) ->
      ...
      let alice_opt =
        List.find_opt
          (fun (p : Runtime.participant) -> p.name = "alice")
          sess.participants
      in
      (match alice_opt with
       | Some alice ->
         Alcotest.(check bool) "alice is Live" true (alice.state = Runtime.Live)
       | None -> Alcotest.fail "alice not found in participants")
    | Error e -> Alcotest.fail (Printf.sprintf "became_live: %s" (Error.to_string e)))
;;
...
let test_persist_agent_completed () =
  ...
  with_temp_dir (fun dir ->
    ...
    | Ok (sess, ev) ->
      ...
      (match alice_opt with
       | Some alice ->
         Alcotest.(check bool) "alice is Done" true (alice.state = Runtime.Done)
       | None -> Alcotest.fail "alice not found in participants")
    | Error e -> Alcotest.fail (Printf.sprintf "completed: %s" (Error.to_string e)))
;;
  ...
  with_temp_dir (fun dir ->
    ...
    (match
       Runtime_server_worker.persist_event
         ...
         state
         session_id
         (Session_started { goal = "full lifecycle"; participants = [ "alice" ] })
     with
     | Ok _ -> ()
     | Error e -> Alcotest.fail (Error.to_string e));
    (match
    ...
    | Ok (final, report, proof) ->
      ...
      Alcotest.(check bool) "completed phase" true (final.phase = Runtime.Completed);
      (match Runtime_store.read_events store session_id () with
       | Ok events -> Alcotest.(check bool) "events >= 5" true (List.length events >= 5)
       | Error e -> Alcotest.fail (Printf.sprintf "read_events: %s" (Error.to_string e)))
    | Error e -> Alcotest.fail (Printf.sprintf "report failed: %s" (Error.to_string e)))
;;
...
let test_store_of_state_with_root () =
  ...
  with_temp_dir (fun dir ->
    let state = make_state net dir in
    match Runtime_server_types.store_of_state state with
    | Ok store -> Alcotest.(check string) "store root" dir store.root
    | Error e ->
      Alcotest.fail (Printf.sprintf "store_of_state failed: %s" (Error.to_string e)))
;;
...
let () =
  Alcotest.run
    "Runtime_server_worker integration"
    [ ( "extract_text"
    ...
    ; ( "with_store_lock"
      , [ Alcotest.test_case "basic" `Quick test_with_store_lock_basic
        ; Alcotest.test_case
            ...
            `Quick
            test_with_store_lock_exception_passthrough
        ; Alcotest.test_case "sequential" `Quick test_with_store_lock_sequential
        ; Alcotest.test_case
            "cancellation does not poison mutex"
            `Quick
    ...
    ; ( "persist_event"
      , [ Alcotest.test_case "by session_id" `Quick test_persist_event_by_session_id
        ; Alcotest.test_case
            "nonexistent session"
            `Quick
            test_persist_event_nonexistent_session
    ...
    ; ( "generate_report_and_proof"
      , [ Alcotest.test_case "basic" `Quick test_generate_report_and_proof_basic
        ; Alcotest.test_case
            "empty events"
            `Quick
            test_generate_report_and_proof_empty_events
        ; Alcotest.test_case
            "nonexistent session"
            `Quick
            test_generate_report_nonexistent_session
        ] )
    ; ( "emit_output_delta"
      , [ Alcotest.test_case "empty delta" `Quick test_emit_output_delta_empty
        ; Alcotest.test_case
            "whitespace only"
            `Quick
            test_emit_output_delta_whitespace_only
        ; Alcotest.test_case "non-empty delta" `Quick test_emit_output_delta_nonempty
        ; Alcotest.test_case
            "nonexistent session"
            `Quick
            test_emit_output_delta_nonexistent_session
        ] )
    ; ( "run_participant"
