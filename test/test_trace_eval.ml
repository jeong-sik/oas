...
let test_classify_agent_run () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"my_agent" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
...
let test_classify_tool_exec () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Tool_exec ~name:"grep" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let summary = Trace_eval.summarize spans in
...
let test_classify_hook_invoke () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Hook_invoke ~name:"before_turn" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let summary = Trace_eval.summarize spans in
...
let test_summarize_failed_tool () =
  with_reset @@ fun () ->
  let tool = Otel_tracer.start_span (default_attrs ~kind:Tracing.Tool_exec ~name:"fail_tool" ()) in
  Otel_tracer.end_span tool ~ok:false;
  let spans = Otel_tracer.flush () in
  let summary = Trace_eval.summarize spans in
...
let test_summarize_avg_duration () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"r1" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let summary = Trace_eval.summarize spans in
  (match summary.average_duration_ms with
...
let test_summarize_longest_span () =
  with_reset @@ fun () ->
  let s1 = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"short" ()) in
  Otel_tracer.end_span s1 ~ok:true;
  let s2 = Otel_tracer.start_span (default_attrs ~kind:Tracing.Api_call ~name:"long" ()) in
  for _ = 0 to 100 do ignore (Unix.gettimeofday ()) done;
...
let test_summarize_multiple_events () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Tool_exec ~name:"multi" ()) in
...
let test_evaluate_max_span_duration_pass () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"fast" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let eval = Trace_eval.evaluate ~max_span_duration_ms:10000.0 spans in
  Alcotest.(check bool) "ok (fast span)" true eval.ok;
  Alcotest.(check bool) "has duration check" true
    (List.exists (fun (c : Trace_eval.check) ->
       c.name = "longest_span_within_budget" && c.passed) eval.checks)

let test_evaluate_max_span_duration_fail () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"slow" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let eval = Trace_eval.evaluate ~max_span_duration_ms:(-1.0) spans in
  Alcotest.(check bool) "not ok (budget exceeded)" false eval.ok;
  Alcotest.(check bool) "duration check failed" true
    (List.exists (fun (c : Trace_eval.check) ->
       c.name = "longest_span_within_budget" && not c.passed) eval.checks)
...
let test_evaluate_empty_spans () =
  ...
  Alcotest.(check bool) "has_spans failed" true
    (List.exists (fun (c : Trace_eval.check) ->
       c.name = "has_spans" && not c.passed) eval.checks)
...
let test_check_detail_present () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"det" ()) in
  Otel_tracer.end_span span ~ok:true;
  let spans = Otel_tracer.flush () in
  let eval = Trace_eval.evaluate spans in
  List.iter (fun (c : Trace_eval.check) ->
    Alcotest.(check bool) ("detail present: " ^ c.name) true
...
let test_evaluate_flushed_with_budgets () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Api_call ~name:"api" ()) in
...
let test_evaluate_flushed_with_duration_budget () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"r" ()) in
  Otel_tracer.end_span span ~ok:true;
...
let test_duration_unfinished_span () =
  with_reset @@ fun () ->
  let span = Otel_tracer.start_span (default_attrs ~kind:Tracing.Agent_run ~name:"open" ()) in
...
let test_summarize_mixed_types () =
  ...
  let c = Otel_tracer.start_span (default_attrs ~kind:Tracing.Tool_exec ~name:"tool" ()) in
  Otel_tracer.end_span c ~ok:false;
  let d = Otel_tracer.start_span (default_attrs ~kind:Tracing.Hook_invoke ~name:"hook" ()) in
    ...
    [
      ...
      ( "evaluate_budget",
        [
          ...
          test_case "tool failure exceeds" `Quick test_evaluate_tool_failure_exceeds_budget;
      ...
      ( "evaluate_flushed",
        [
          ...
          test_case "with duration budget" `Quick test_evaluate_flushed_with_duration_budget;
        ] );
      ( "duration",
        [
          test_case "unfinished span" `Quick test_duration_unfinished_span;
        ] );
    ]
