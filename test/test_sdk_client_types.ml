...
let test_permission_mode_strings () =
  ...
  check_string
    "bypass"
    "bypass_permissions"
    (string_of_permission_mode Bypass_permissions)
;;
...
let test_permission_mode_of_string () =
  ...
  check_bool
    "accept_edits"
    true
    (permission_mode_of_string "accept_edits" = Some Accept_edits);
  check_bool "plan" true (permission_mode_of_string "plan" = Some Plan);
  check_bool
    "bypass"
    true
    (permission_mode_of_string "bypass_permissions" = Some Bypass_permissions);
...
let test_show_agent_definition () =
  let def =
    { description = "test"; prompt = "hello"; tools = Some [ "a" ]; model = Some "m" }
  in
...
let test_agent_entries_custom () =
  let custom =
    { description = "custom"; prompt = "do stuff"; tools = None; model = None }
  in
...
let test_agent_entries_multiple () =
  ...
  let a2 =
    { description = "a2"; prompt = "p2"; tools = Some [ "t" ]; model = Some "m" }
  in
...
let () =
  ...
  run
    ...
    ; "setting_source", [ test_case "show" `Quick test_show_setting_source ]
    ; "agent_definition", [ test_case "show" `Quick test_show_agent_definition ]
    ; ( "options"
    ...
    ; "default_agent", [ test_case "values" `Quick test_default_agent ]
    ; ( "agent_entries"
