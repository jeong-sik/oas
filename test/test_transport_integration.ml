...
let with_temp_dir f =
  let root =
    Filename.concat
      ...
      (Printf.sprintf "oas-transport-test-%d-%06x" (Unix.getpid ()) (Random.int 0xFFFFFF))
  in
...
let test_transport_lifecycle () =
  ...
  (match info with
   | Some init ->
     check
       bool
       "protocol version non-empty"
       true
       (String.length init.Runtime.protocol_version > 0);
     check bool "sdk_name non-empty" true (String.length init.sdk_name > 0)
...
let test_runtime_client_session () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let session =
         unwrap "start_session" (Runtime_client.start_session client start_req)
       in
       check string "session_id" "test-session-1" session.session_id;
       check string "goal" "Integration test" session.goal;
       check bool "phase not failed" true (session.phase <> Runtime.Failed);
       (* Status check *)
       let status_session =
         unwrap "status" (Runtime_client.status client ~session_id:"test-session-1")
       in
       check string "status session_id" "test-session-1" status_session.session_id;
       (* Events (may be empty for mock) *)
       let events =
         unwrap
           "events"
           (Runtime_client.events client ~session_id:"test-session-1" ~after_seq:0 ())
       in
       ...
       check
         ...
         "finalized phase"
         true
         (final_session.phase = Runtime.Completed || final_session.phase = Runtime.Failed))
;;
...
let test_runtime_client_commands () =
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       in
       let _session = unwrap "start" (Runtime_client.start_session client start_req) in
       (* Spawn agent *)
       let after_spawn =
         unwrap
           "spawn"
       ...
       in
       check bool "has participants" true (List.length after_spawn.participants > 0);
       (* Record a user turn *)
       let after_turn =
         unwrap
           "record_turn"
           (Runtime_client.apply_command
              client
              ~session_id:"cmd-test"
              (Runtime.Record_turn { actor = None; message = "user message" }))
       in
       check bool "turn recorded" true (after_turn.turn_count >= 0);
       (* Finalize *)
       let _final =
         unwrap "finalize" (Runtime_client.finalize client ~session_id:"cmd-test" ())
       in
       ())
;;
