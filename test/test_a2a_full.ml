...
let mk_server
  ...
  let config : A2a_server.config =
    { port = 8080; agent_card = mk_agent_card (); on_task_send; on_task_cancel }
  in
...
let test_tasks_cancel_success () =
  ...
  in
  let _code, resp =
    A2a_server.process_request server ~meth:"POST" ~path:"/a2a" ~body:send
  in
  let task_id =
    Yojson.Safe.Util.(
      Yojson.Safe.from_string resp |> member "result" |> member "id" |> to_string)
  in
  (* Cancel it *)
  let cancel =
    rpc_body
      ...
      ~id:(`Int 2)
  in
  let code, cancel_resp =
    A2a_server.process_request server ~meth:"POST" ~path:"/a2a" ~body:cancel
  in
  ...
  Alcotest.(check bool) "callback called" true (List.mem task_id !canceled_ids)
;;
...
let test_tasks_cancel_callback_error () =
  let on_task_send msg =
    ...
    | Error _ -> Ok task
  in
  let on_task_cancel _id = Error (Error.Internal "cancel refused") in
  let server = mk_server ~on_task_send ~on_task_cancel () in
  (* Create a task *)
  let send =
    rpc_body
      ...
      ~id:(`Int 1)
  in
  let _, resp = A2a_server.process_request server ~meth:"POST" ~path:"/a2a" ~body:send in
  let task_id =
    Yojson.Safe.Util.(
      Yojson.Safe.from_string resp |> member "result" |> member "id" |> to_string)
  in
...
let test_tasks_send_callback_error () =
  let on_task_send _msg = Error (Error.Internal "refused") in
...
let test_bad_rpc_structure () =
  ...
  let code, _ = A2a_server.process_request server ~meth:"POST" ~path:"/a2a" ~body in
...
let test_post_unknown_path () =
  let server = mk_server () in
  let code, _ = A2a_server.process_request server ~meth:"POST" ~path:"/other" ~body:"" in
...
let test_get_unknown_path () =
  let server = mk_server () in
  let code, _ =
    A2a_server.process_request server ~meth:"GET" ~path:"/not-agent-card" ~body:""
  in
  Alcotest.(check int) "404" 404 code
;;
...
let test_text_of_task_file_parts_ignored () =
  ...
  in
  Alcotest.(check string) "no text from non-text parts" "" (A2a_client.text_of_task task)
;;

let test_text_of_task_multiple_messages () =
  let msg1 : A2a_task.task_message =
    { role = TaskUser; parts = [ A2a_task.Text_part "question" ]; metadata = [] }
  in
  let msg2 : A2a_task.task_message =
    { role = TaskAgent; parts = [ A2a_task.Text_part "answer" ]; metadata = [] }
  in
  let task : A2a_task.task =
    { id = "t2"
    ; state = Completed
...
let () =
  Alcotest.run
    ...
    ; "tasks_get", [ Alcotest.test_case "not found" `Quick test_tasks_get_not_found ]
    ; ( "tasks_send"
    ...
    ; ( "text_of_task"
      , [ Alcotest.test_case
            "file parts ignored"
            `Quick
            test_text_of_task_file_parts_ignored
        ; Alcotest.test_case
            "multiple messages"
            `Quick
            test_text_of_task_multiple_messages
    ...
    ; ( "lifecycle"
      , [ Alcotest.test_case
            ...
            `Quick
            test_server_not_running_initially
