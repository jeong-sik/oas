...
let test_agent_accessors () =
  Eio_main.run @@ fun env ->
  let tool = Tool.create ~name:"test_tool" ~description:"desc"
    ~parameters:[] (fun _input -> Ok { Types.content = "result" }) in
...
let test_agent_clone () =
  ...
  | Ok agent ->
    ...
    Alcotest.(check (option string)) "desc preserved" (Some "orig desc")
...
let test_agent_clone_copy_context () =
  ...
  | Ok agent ->
    ...
    Alcotest.(check bool) "context copied" true
