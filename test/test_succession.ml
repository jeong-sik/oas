...
let test_merge_metrics_identity () =
  let merged =
    Succession.merge_metrics Succession.empty_metrics Succession.empty_metrics
  in
...
let test_extract_dna_empty_messages () =
  let dna =
    Succession.extract_dna ~messages:[] ~goal:"empty" ~generation:0 ~trace_id:"t0" ()
  in
...
let test_extract_dna_captures_decisions () =
  ...
  in
  let dna =
    Succession.extract_dna ~messages:msgs ~goal:"test" ~generation:1 ~trace_id:"t1" ()
  in
...
let test_extract_dna_pending_from_last_user () =
  let msgs =
    [ mk_msg Types.Assistant "response"; mk_msg Types.User "do this next please" ]
  in
  let dna =
    Succession.extract_dna ~messages:msgs ~goal:"test" ~generation:1 ~trace_id:"t1" ()
  in
...
let test_successor_prompt () =
  let dna : Succession.dna =
    ...
    ; trace_id = "t1"
    ; goal = "deploy app"
    ; progress_summary = "built module"
    ; compressed_context = "ctx"
    ; pending_actions = [ "run tests" ]
    ; key_decisions = [ "using Eio" ]
    ; memory_refs = []
    ; warnings = [ "low memory" ]
...
let test_normalize_strips_redacted_thinking () =
  let msgs =
    [ { Types.role = Types.Assistant
      ; content = [ Types.RedactedThinking "secret"; Types.Text "visible" ]
      ; name = None
      ; tool_call_id = None
      ; metadata = []
      }
...
let test_normalize_repairs_dangling_tool_calls () =
  let msgs =
    [ { Types.role = Types.Assistant
      ; content = [ Types.ToolUse { id = "t1"; name = "search"; input = `Null } ]
      ; name = None
      ; tool_call_id = None
      ; metadata = []
      }
  ...
  Alcotest.run
    ...
    ; ( "extract_dna"
      , [ Alcotest.test_case "basic" `Quick test_extract_dna_basic
        ...
        ; Alcotest.test_case
            "captures decisions"
            `Quick
            test_extract_dna_captures_decisions
        ; Alcotest.test_case
            "pending from last user"
            `Quick
            test_extract_dna_pending_from_last_user
        ] )
    ; ( "successor_prompt"
    ...
    ; ( "normalize"
      , [ Alcotest.test_case "strips thinking" `Quick test_normalize_strips_thinking
        ; Alcotest.test_case
            "strips redacted thinking"
            `Quick
            test_normalize_strips_redacted_thinking
        ; Alcotest.test_case
            "repairs dangling tool calls"
            `Quick
            test_normalize_repairs_dangling_tool_calls
        ; Alcotest.test_case
            "preserves matched tool calls"
            `Quick
            test_normalize_preserves_matched_tool_calls
        ] )
    ; ( "serialization"
