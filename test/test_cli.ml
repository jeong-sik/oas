...
let cli_exe =
  ...
  let candidate =
    Filename.concat (Filename.concat (Filename.dirname dir) "bin") "oas_cli.exe"
  in
  ...
  then candidate
    ...
    let cwd_candidate =
      Printf.sprintf "%s/_build/default/bin/oas_cli.exe" (Sys.getcwd ())
    in
    if Sys.file_exists cwd_candidate
    then cwd_candidate
    else
...
let metric name value : Eval.metric = { name; value; unit_ = None; tags = [] }
...
let test_version_output () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp "[0-9]+\\.[0-9]+\\.[0-9]+") trimmed 0);
       true
     with
     | Not_found -> false)
;;
...
let test_init_creates_config () =
  ...
  (try ignore (Yojson.Safe.from_string data) with
   | _ -> fail "oas.json is not valid JSON");
  try
...
let test_card_with_config () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string "card-test") output 0);
       true
     with
     | Not_found -> false);
...
let test_help () =
  ...
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string_case_fold "command") output 0);
       true
     with
     | Not_found -> false);
  check
    ...
    true
    (try
       ignore (Str.search_forward (Str.regexp_string_case_fold "eval") output 0);
       true
     with
     | Not_found -> false)
;;
...
let test_run_config_no_prompt () =
  ...
  Out_channel.with_open_text config_path (fun oc -> output_string oc {|{"name":"test"}|});
  ...
  Fun.protect
    ~finally:(fun () ->
      (try Sys.remove trace_path with
       | _ -> ());
      try Sys.remove dataset_path with
      | _ -> ())
    (fun () ->
       ...
       let cmd =
         Printf.sprintf
           "%s eval record-trace --session %s --out %s"
           cli_exe
           trace_path
           dataset_path
       in
...
let test_eval_run_empty_dataset () =
  ...
  Fun.protect
    ~finally:(fun () ->
      ...
      (try Sys.remove config_path with
       | _ -> ());
      (try Sys.remove (Filename.concat out_dir "report.json") with
       | _ -> ());
      (try Sys.remove (Filename.concat out_dir "report.md") with
       | _ -> ());
      (try Sys.remove (Filename.concat out_dir "report.junit.xml") with
       | _ -> ());
      try Sys.rmdir out_dir with
      | _ -> ())
    (fun () ->
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       Out_channel.with_open_text dataset_path (fun oc ->
         ...
         output_char oc '\n');
       let cmd =
         Printf.sprintf "%s eval run --dataset %s --out %s" cli_exe dataset_path out_dir
       in
       let exit_code = Sys.command cmd in
       check int "exit 0" 0 exit_code;
       check
    ...
    (fun () ->
       ...
       try
         ...
         check
           ...
           (Sys.file_exists (Filename.concat out_dir "report.json"));
         let report_json =
           In_channel.with_open_text (Filename.concat out_dir "report.json") (fun ic ->
             Yojson.Safe.from_string (In_channel.input_all ic))
         in
         ...
         check
           ...
           "passed"
           2
           (report_json |> member "summary" |> member "passed" |> to_int);
...
let test_eval_save_baseline_trace_replay_dataset () =
  let trace_path =
    Printf.sprintf "/tmp/oas_cli_baseline_trace_%d.ndjson" (Unix.getpid ())
  in
  let dataset_path =
    Printf.sprintf "/tmp/oas_cli_baseline_dataset_%d.jsonl" (Unix.getpid ())
  in
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let cmd =
         Printf.sprintf
           "%s eval save-baseline --dataset %s --out %s"
           cli_exe
           dataset_path
           baseline_path
       in
       ...
       | Error e -> fail e
       | Ok baseline -> check string "run id" "replay" baseline.run_metrics.run_id)
;;

let test_eval_run_with_baseline_failure () =
  let trace_path = Printf.sprintf "/tmp/oas_cli_reg_trace_%d.ndjson" (Unix.getpid ()) in
  let dataset_path =
    Printf.sprintf "/tmp/oas_cli_reg_dataset_%d.jsonl" (Unix.getpid ())
  in
  let baseline_path =
    Printf.sprintf "/tmp/oas_cli_reg_baseline_%d.json" (Unix.getpid ())
  in
  ...
  Fun.protect
    ...
    (fun () ->
       ...
       let cmd =
         Printf.sprintf
           "%s eval run --baseline %s --dataset %s --out %s"
           cli_exe
           baseline_path
           dataset_path
           out_dir
       ...
       let eval_json =
         In_channel.with_open_text (Filename.concat out_dir "eval.json") (fun ic ->
           Yojson.Safe.from_string (In_channel.input_all ic))
       in
       let open Yojson.Safe.Util in
       ...
       check
         bool
         "report markdown includes regression metric"
         true
...
let test_eval_save_baseline_requires_case_for_multi_run () =
  ...
  let dataset_path =
    Printf.sprintf "/tmp/oas_cli_case_dataset_%d.jsonl" (Unix.getpid ())
  in
  let baseline_path =
    Printf.sprintf "/tmp/oas_cli_case_baseline_%d.json" (Unix.getpid ())
  in
  Fun.protect
    ~finally:(fun () ->
      remove_path trace_a;
    ...
    (fun () ->
       ...
       check bool "mentions --case" true (Util.string_contains ~needle:"--case" output))
;;
...
let () =
  run
    "CLI"
    [ "version", [ test_case "version output" `Quick test_version_output ]
    ; "init", [ test_case "creates config" `Quick test_init_creates_config ]
    ; ( "run"
    ...
    ; "help", [ test_case "help output" `Quick test_help ]
    ; ( "eval"
      , [ test_case "record trace" `Quick test_eval_record_trace
        ...
        ; test_case
            "save baseline trace replay dataset"
            `Quick
            test_eval_save_baseline_trace_replay_dataset
        ; test_case "run with baseline failure" `Quick test_eval_run_with_baseline_failure
        ; test_case
            "save baseline requires case for multi-run"
            `Quick
