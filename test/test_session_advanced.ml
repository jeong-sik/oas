...
let test_empty_metadata_roundtrip () =
  ...
  | Ok s2 ->
    ...
    check bool "no keys" true
      (Context.get s2.metadata "nonexistent" = None)
...
let () =
  run "session_advanced" [
    ...
    "checkpoint", [
      ...
      test_case "resume preserves turn_count" `Quick
        test_resume_preserves_turn_count;
    ];
    "json_edge", [
