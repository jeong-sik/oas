...
let make_tool name =
  Tool.create ~name ~description:("desc_" ^ name) ~parameters:[]
    (fun _ -> Ok { Types.content = "ok" })

let make_tool_v2 name =
  Tool.create ~name ~description:("v2_" ^ name) ~parameters:[]
    (fun _ -> Ok { Types.content = "v2" })
...
let test_of_list_dedup () =
  ...
  | Some found ->
    Alcotest.(check string) "last wins" "v2_alpha" found.schema.description
...
let test_merge_last_writer_wins () =
  ...
  | Some found ->
    Alcotest.(check string) "right wins" "v2_y" found.schema.description

let test_filter () =
  let s = Tool_set.of_list [make_tool "allow_me"; make_tool "deny_me"] in
  let g = Guardrails.{ tool_filter = AllowList ["allow_me"]; max_tool_calls_per_turn = None } in
...
let test_concat () =
  ...
  Alcotest.(check (list string)) "concat names" ["a"; "b"; "c"]
...
let prop_merge_assoc =
  QCheck.Test.make ~count:200 ~name:"merge_associativity"
...
let prop_merge_left_id =
  QCheck.Test.make ~count:200 ~name:"merge_left_identity"
    arb_tool_set
    (fun s ->
...
let prop_merge_right_id =
  QCheck.Test.make ~count:200 ~name:"merge_right_identity"
    arb_tool_set
    (fun s ->
...
let prop_merge_idempotent =
  QCheck.Test.make ~count:200 ~name:"merge_idempotent"
    arb_tool_set
    (fun s ->
...
let prop_validate_ok =
  QCheck.Test.make ~count:200 ~name:"validate_always_ok"
    arb_tool_set
    (fun s -> Tool_set.validate s = Ok ())

(* ── Runner ──────────────────────────────────────────────── *)

let () =
  Alcotest.run "Tool_set" [
    "unit", [
