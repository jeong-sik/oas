...
(* 4000+ chars to exceed minimum cache threshold (1024 tokens ~ 3500 chars) *)
let long_system_prompt =
  String.concat "" (List.init 200 (fun i ->
    Printf.sprintf "Rule %d: follow this guideline carefully. " i))

let test_cache_system_prompt () =
  let config = PC.make ~kind:Anthropic ~model_id:"claude-sonnet-4-6"
    ~base_url:"" ~system_prompt:long_system_prompt
...
let test_cache_short_prompt_skipped () =
  (* Short prompt should NOT get cache_control even with flag on *)
  let config = PC.make ~kind:Anthropic ~model_id:"m"
  ...
  Alcotest.(check string) "short prompt = plain string" "Short."
    (json |> member "system" |> to_string)
...
let () =
  ...
  run "provider_complete" [
    ...
    "prompt_caching", [
      ...
      test_case "short prompt skips cache" `Quick test_cache_short_prompt_skipped;
    ];
  ]
