...
let make_agent ~net base_url =
  let config = { Types.default_config with
    name = "evidence-agent"; max_turns = 1;
  } in
  ...
  let raw_trace = match Raw_trace.create ~path:trace_path () with
    | Ok t -> t
...
let default_options ?(session_root = None) session_id : Direct_evidence.options =
  { session_root; session_id; goal = "test-evidence";
    title = Some "Evidence Test"; tag = Some "cov";
    worker_id = None; runtime_actor = None; role = None;
    aliases = ["test-alias"]; requested_provider = None;
...
let test_get_worker_run_no_trace () =
  ...
  let config = { Types.default_config with
    name = "no-trace-agent"; max_turns = 1;
  } in
  let tmpdir_rt = Filename.temp_dir "oas-rt-tmp" "" in
  let raw_trace = match Raw_trace.create ~path:(Filename.concat tmpdir_rt "t.jsonl") () with
    | Ok t -> t | Error e -> failwith (Error.to_string e) in
  ...
  try
    ...
    (match Agent.run ~sw agent "test persist" with
     | Ok _ ->
       ...
       let options = { (default_options "sess-ev-4") with
         session_root = Some tmpdir } in
           ...
           | Ok () ->
             (match Direct_evidence.get_proof_bundle ~agent ~raw_trace ~options () with
              | Ok bundle ->
                ...
                check bool "has worker runs" true
                  (List.length bundle.worker_runs > 0);
                ignore (Sys.command (Printf.sprintf "rm -rf %s" tmpdir));
                Eio.Switch.fail sw Exit
              | Error e ->
                ignore (Sys.command (Printf.sprintf "rm -rf %s" tmpdir));
  ...
  try
    ...
    (match Agent.run ~sw agent "test conformance" with
     | Ok _ ->
       ...
       let options = { (default_options "sess-ev-5") with
         session_root = Some tmpdir } in
...
let () =
  ...
  run "evidence_pipeline_cov" [
    ...
    "conformance", [
      test_case "from agent" `Quick test_conformance_from_agent;
    ];
  ]
