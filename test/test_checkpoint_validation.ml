...
let test_valid_dna () =
  let dna = "goal: Deploy v2 to production\n\
             current task: Run integration tests\n\
             - Check API endpoints\n\
             - Verify database migrations\n\
             context: Sprint 42 release" in
...
let test_continuity_perfect () =
  ...
  let result = Checkpoint_validation.continuity_check
    ~full_context:full ~compressed_context:compressed in
...
let test_continuity_empty_context () =
  let result = Checkpoint_validation.continuity_check
    ~full_context:"" ~compressed_context:"anything" in
...
let test_overlap_ratio_identical () =
  let r = Checkpoint_validation.token_overlap_ratio
    ~source:"hello world test" ~target:"hello world test" in
  Alcotest.(check bool) "identical = 1.0" true (r >= 0.99)

let test_overlap_ratio_disjoint () =
  let r = Checkpoint_validation.token_overlap_ratio
    ~source:"alpha beta gamma" ~target:"delta epsilon zeta" in
  Alcotest.(check bool) "disjoint = 0.0" true (r < 0.01)

let test_contains_ci () =
  Alcotest.(check bool) "found" true
    (Checkpoint_validation.contains_substring_ci
      ~haystack:"Hello World" ~needle:"hello");
  Alcotest.(check bool) "not found" false
    (Checkpoint_validation.contains_substring_ci
      ~haystack:"Hello" ~needle:"xyz")

let () =
  Alcotest.run "Checkpoint Validation" [
    "validate_dna", [
