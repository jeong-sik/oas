...
let test_run_collect_ok () =
  ...
  @@ fun sw ->
  match
    Llm_provider.Cli_common_subprocess.run_collect
      ~sw
      ~mgr
      ~name:"sh"
      ~cwd:None
...
let test_run_collect_nonzero_exit_falls_back_to_stdout () =
  ...
  | Error (Llm_provider.Http_client.NetworkError { message; _ }) ->
    Alcotest.(check string)
      "stdout fallback uses last non-empty line"
      "sh exited with code 7: {\"error\":\"quota\"}"
      message
  | Error _ -> Alcotest.fail "expected NetworkError"
;;
...
let test_stream_cancel_sends_sigint () =
  ...
  | Error (Llm_provider.Http_client.NetworkError { message; _ }) ->
    Alcotest.(check bool) "non-empty error message" true (String.length message > 0)
  | Error _ -> Alcotest.fail "expected NetworkError"
;;
...
let test_scrub_env_removes_parent_var () =
  ...
  (match
   ...
   with
   | Ok { stdout; _ } ->
     Alcotest.(check string) "parent env inherited by default" "leaked\n" stdout
  ...
  with
  | Ok { stdout; _ } ->
    Alcotest.(check string) "scrub_env blocks named var from child" "UNSET\n" stdout
...
let test_cwd_sets_os_level_working_dir () =
  ...
  | Ok { stdout; _ } ->
    Alcotest.(check string)
      "child cwd follows ~cwd at OS level"
      expected
      (String.trim stdout)
  | Error _ -> Alcotest.fail "expected Ok"
;;
  ...
  Alcotest.run
    ...
    [ ( "run_collect"
      , [ Alcotest.test_case "ok" `Quick test_run_collect_ok
        ...
        ; Alcotest.test_case
            "exit falls back to stdout detail"
            `Quick
            test_run_collect_nonzero_exit_falls_back_to_stdout
        ; Alcotest.test_case
            "on_stderr_line forwards per-line"
            `Quick
            test_on_stderr_line_called_per_line
        ; Alcotest.test_case
            "scrub_env strips named parent var"
            `Quick
            test_scrub_env_removes_parent_var
        ; Alcotest.test_case
            "cwd sets OS-level working directory"
            `Quick
            test_cwd_sets_os_level_working_dir
        ; Alcotest.test_case
            "blank cwd treated as None"
            `Quick
            test_cwd_blank_treated_as_none
        ] )
    ; ( "run_stream_lines"
