  ...
  Alcotest.run
    ...
    [ ( "deny_by_default"
      , [ Alcotest.test_case "store denied without grant" `Quick (fun () ->
            ...
            match
              Memory_access.store acl ~agent:"alice" ~tier:Working "key" (`String "v")
            with
    ...
    ; ( "permission_levels"
      , [ Alcotest.test_case "read-only cannot write" `Quick (fun () ->
            ...
            Memory_access.grant
              acl
              { agent_name = "bob"; tier = Working; key_pattern = "*"; permission = Read };
            ...
            match
              Memory_access.store acl ~agent:"bob" ~tier:Working "key" (`String "v2")
            with
        ...
        ; Alcotest.test_case "write-only cannot read" `Quick (fun () ->
            ...
            (match
               Memory_access.store acl ~agent:"charlie" ~tier:Working "key" (`String "v")
             with
             | Ok () -> ()
    ...
    ; ( "backend_failure"
      , [ Alcotest.test_case "store propagates backend error" `Quick (fun () ->
            ...
            match
              Memory_access.store acl ~agent:"alice" ~tier:Long_term "k" (`String "v")
            with
