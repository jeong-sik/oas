...
let test_truncate_long () =
  with_env "OAS_MCP_OUTPUT_MAX_TOKENS" (Some "2") (fun () ->
    ...
    Alcotest.(check bool) "starts with 8 a's" true
      (String.length result > 8
       && String.sub result 0 8 = String.make 8 'a'))
...
let test_truncate_one_over () =
  with_env "OAS_MCP_OUTPUT_MAX_TOKENS" (Some "3") (fun () ->
    ...
    let result = Mcp.truncate_output text in
    Alcotest.(check bool) "truncated at 13" true
      (contains_substring ~sub:"...[oas mcp output truncated]" result))
...
let test_tool_json_no_name () =
  let json = `Assoc [
    ("description", `String "orphan");
  ] in
  Alcotest.(check bool) "None" true
    (Mcp.mcp_tool_of_json json = None)

let test_tool_json_name_is_int () =
  let json = `Assoc [
    ("name", `Int 42);
    ("description", `String "bad name type");
  ] in
  Alcotest.(check bool) "None for int name" true
    (Mcp.mcp_tool_of_json json = None)

let test_tool_json_not_assoc () =
  Alcotest.(check bool) "None for string" true
    (Mcp.mcp_tool_of_json (`String "hi") = None);
  Alcotest.(check bool) "None for list" true
    (Mcp.mcp_tool_of_json (`List []) = None);
  Alcotest.(check bool) "None for int" true
    (Mcp.mcp_tool_of_json (`Int 5) = None)

let test_tool_json_no_description () =
  let json = `Assoc [
    ("name", `String "bare_tool");
    ("inputSchema", `Assoc []);
  ] in
  match Mcp.mcp_tool_of_json json with
  | Some t ->
...
let test_tool_json_input_schema_underscore () =
  ...
  match Mcp.mcp_tool_of_json json with
  | Some t ->
    Alcotest.(check string) "name" "t" t.name
...
let test_tool_json_no_schema () =
  ...
  | Some t ->
    ...
    Alcotest.(check bool) "schema is assoc" true
      (t.input_schema = `Assoc [])
  | None -> Alcotest.fail "expected Some"
...
let () =
  Alcotest.run "MCP Deep" [
    ...
    "mcp_tool_of_json", [
      ...
      Alcotest.test_case "input_schema underscore" `Quick test_tool_json_input_schema_underscore;
