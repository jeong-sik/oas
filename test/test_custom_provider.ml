...
let make_test_impl ?(name = "test-provider") ?(request_path = "/v1/test") ()
...
let test_register_and_find () =
  ...
  | Some found -> Alcotest.(check string) "name matches" "crud-test" found.name
  | None -> Alcotest.fail "registered provider not found"
;;

let test_find_unregistered () =
  match Provider.find_provider "nonexistent-provider-xyz" with
  | None -> ()
  | Some _ -> Alcotest.fail "should not find unregistered provider"
;;
...
let test_register_overwrites () =
  ...
  | Some found -> Alcotest.(check string) "path updated" "/v1/new" found.request_path
  | None -> Alcotest.fail "provider should exist after overwrite"
;;
...
let test_resolve_unregistered () =
  ...
  | Ok _ -> Alcotest.fail "should fail for unregistered provider"
;;
  ...
  Alcotest.run
    ...
    ; ( "config"
      , [ Alcotest.test_case "custom_provider default" `Quick test_custom_provider_config
        ; Alcotest.test_case
            "custom_provider with overrides"
            `Quick
            test_custom_provider_config_with_overrides
        ] )
    ; ( "resolve"
      , [ Alcotest.test_case
            "resolve registered"
            `Quick
            (with_eio test_resolve_registered)
        ; Alcotest.test_case
            "resolve unregistered -> Error"
            `Quick
            (with_eio test_resolve_unregistered)
        ] )
    ; ( "request_kind"
      , [ Alcotest.test_case
            "registered kind"
            `Quick
            (with_eio test_request_kind_registered)
        ; Alcotest.test_case
            "unregistered kind -> Custom"
            `Quick
            (with_eio test_request_kind_unregistered)
        ] )
    ; ( "capabilities"
      , [ Alcotest.test_case
            "registered capabilities"
            `Quick
            (with_eio test_capabilities_registered)
        ; Alcotest.test_case
            "unregistered -> default"
            `Quick
            (with_eio test_capabilities_unregistered)
        ] )
    ; ( "model_spec"
      , [ Alcotest.test_case
            "model_spec registered"
            `Quick
            (with_eio test_model_spec_registered)
        ] )
    ; ( "lifecycle"
