...
let test_task_state_roundtrip () =
  let states = [
    A2a_task.Submitted; Working; Input_required;
...
let test_valid_transitions () =
  ...
  Alcotest.(check int) "completed empty" 0
    (List.length (A2a_task.valid_transitions Completed));
  Alcotest.(check int) "failed empty" 0
    (List.length (A2a_task.valid_transitions Failed))
...
let test_add_artifact () =
  let task = A2a_task.create (mk_message "q") in
  let art = { A2a_task.name = "result.json";
...
let test_well_known_agent_json () =
  ...
  let (status, body) = A2a_server.process_request server
...
let test_tasks_send () =
  ...
  let rpc_body = Yojson.Safe.to_string (`Assoc [
    ...
    ("params", `Assoc [
      ("message", A2a_task.task_message_to_yojson (mk_message "hello"))
    ]);
    ("id", `Int 1);
  ]) in
  let (status, body) = A2a_server.process_request server
...
let test_tasks_get () =
  ...
  let send_body = Yojson.Safe.to_string (`Assoc [
    ...
    ("method", `String "tasks/send");
    ("params", `Assoc [
      ("message", A2a_task.task_message_to_yojson (mk_message "q"))
    ]);
    ("id", `Int 1);
  ]) in
  ...
  let open Yojson.Safe.Util in
  let result = json |> member "result" in
  Alcotest.(check string) "id matches" task_id
    (result |> member "id" |> to_string)

let test_unknown_method () =
  let server = A2a_server.create {
    port = 8080;
  ...
  let (status, resp) = A2a_server.process_request server
...
let test_404_path () =
  ...
  let (status, _) = A2a_server.process_request server
    ~meth:"GET" ~path:"/unknown" ~body:"" in
...
let test_invalid_json () =
  ...
  let (status, _) = A2a_server.process_request server
    ~meth:"POST" ~path:"/a2a" ~body:"not json" in
...
let () =
  Alcotest.run "A2A" [
    ...
    "serialization", [
      Alcotest.test_case "task JSON" `Quick test_task_json_roundtrip;
    ];
    "store", [
