...
let test_no_tripwires_action_ok () =
  ...
  @@ fun _env ->
  let result =
    Guardrail_tripwire.guarded_action
      ~tripwires:[]
...
let test_tripwire_fires_immediate () =
  ...
  (match result with
   ...
   | Error (`Action e) -> Alcotest.failf "got Action error: %s" (Error.to_string e)
   | Ok _ -> Alcotest.fail "expected Tripped, got Ok");
...
let test_multiple_one_fires () =
  ...
  let result =
    Guardrail_tripwire.guarded_action
      ~tripwires:
        [ pass_tripwire "ok1"; fail_tripwire "bad" "violation"; pass_tripwire "ok2" ]
      ~messages:(make_messages [ "test" ])
      ~action:(fun () ->
        Eio.Time.sleep clock 0.05;
        Ok "slow")
...
let () =
  Alcotest.run
    ...
    ; "all_pass", [ Alcotest.test_case "pass" `Quick test_all_pass ]
    ; ( "tripwire_fires"
    ...
    ; "action_wins", [ Alcotest.test_case "fast_action" `Quick test_action_wins ]
    ; ( "error_propagation"
