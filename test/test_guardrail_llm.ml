...
let keyword_judge keyword : Guardrail_llm.judge =
  fun prompt ->
  if
    String.length prompt > 0
    &&
    try
      ignore (Str.search_forward (Str.regexp_string keyword) prompt 0);
      true
    with
    | Not_found -> false
...
let test_input_pass () =
  let v =
    Guardrail_llm.make_input_validator
      ~name:"safe_check"
      ~policy_prompt:"Check safety"
      ~judge:always_pass
  in
...
let test_input_judge_error () =
  ...
  | Error reason ->
    Alcotest.(check bool)
      "contains 'judge error'"
      true
      (try
         ignore (Str.search_forward (Str.regexp_string "judge error") reason 0);
         true
       with
       | Not_found -> false)
...
let test_input_keyword_detection () =
  let v =
    Guardrail_llm.make_input_validator
      ~name:"pii_check"
      ~policy_prompt:"Reject PII"
      ~judge:(keyword_judge "SSN")
  in
  let clean = make_messages [ "tell me about OCaml" ] in
  let dirty = make_messages [ "my SSN is 123-45-6789" ] in
  (match v.validate clean with
   | Ok () -> ()
   | Error r -> Alcotest.failf "clean: %s" r);
  match v.validate dirty with
  | Error _ -> ()
  | Ok () -> Alcotest.fail "dirty should fail"
;;
...
let test_output_pass () =
  let v =
    Guardrail_llm.make_output_validator
      ~name:"out_check"
      ~policy_prompt:"Check output"
      ~judge:always_pass
  in
...
let test_async_integration () =
  ...
  let iv =
    Guardrail_llm.make_input_validator
      ~name:"input_guard"
      ~policy_prompt:"Reject profanity"
      ~judge:(keyword_judge "bad_word")
  in
  let ov =
    Guardrail_llm.make_output_validator
      ~name:"output_guard"
  ...
  let config : Guardrails_async.t =
    { input_validators = [ iv ]; output_validators = [ ov ] }
  in
...
let () =
  Alcotest.run
    ...
    ; "integration", [ Alcotest.test_case "async" `Quick test_async_integration ]
    ]
;;
