...
let test_execution_mode_to_string () =
  Alcotest.(check string) "diagnose" "diagnose" (Execution_mode.to_string Diagnose);
  Alcotest.(check string) "draft" "draft" (Execution_mode.to_string Draft);
  Alcotest.(check string) "execute" "execute" (Execution_mode.to_string Execute)
;;

let test_execution_mode_of_string () =
  Alcotest.(check (result (of_pp Execution_mode.pp) string))
    "parse diagnose"
    (Ok Execution_mode.Diagnose)
    (Execution_mode.of_string "diagnose");
  Alcotest.(check (result (of_pp Execution_mode.pp) string))
    "parse execute"
    (Ok Execution_mode.Execute)
...
let test_risk_class_max_mode () =
  ...
  Alcotest.(check (option (of_pp Execution_mode.pp)))
    "high"
    (Some Execution_mode.Draft)
    (max_mode High);
  Alcotest.(check (option (of_pp Execution_mode.pp))) "critical" None (max_mode Critical)
;;
...
let test_event_bus_envelope_v2_from_legacy () =
  ...
  in
  let env =
    Event_bus.envelope_v2_of_envelope
      ~event_id:"evt-legacy"
      ~observed_at:(legacy.ts +. 1.0)
      ~seq:3
      legacy
  in
...
let test_runtime_health_roundtrip () =
  let probes =
    [ Runtime_health.make_probe
        ~name:Runtime_health.Provider
        ~status:Runtime_health.Status_ok
        ~checked_at:10.0
        ()
    ; Runtime_health.make_probe
        ~name:Runtime_health.Transport
        ~status:Runtime_health.Failed
...
let test_mode_resolver_downgrade_only () =
  ...
  | Ok d ->
    Alcotest.(check string)
      "stays diagnose"
      "diagnose"
      (Execution_mode.to_string d.effective_mode)
  | Error e -> Alcotest.fail e
;;
...
let test_cdal_proof_roundtrip () =
  ...
  | Ok decoded ->
    ...
    Alcotest.(check string)
      "result_status"
      "\"context_overflow\""
      (Yojson.Safe.to_string (Cdal_proof.result_status_to_yojson decoded.result_status))
...
let test_hooks_compose_outer_skip () =
  let outer = Hooks.{ empty with pre_tool_use = Some (fun _ -> Skip) } in
...
let test_proof_store_init_and_write () =
  let tmpdir = Filename.concat (Filename.get_temp_dir_name ()) "oas-test-proof-store" in
...
let test_proof_store_make_ref () =
  ...
  Alcotest.(check string) "ref format" "proof-store://r1/tool_traces/t.jsonl" ref_str
;;
...
let test_proof_store_resolve_ref () =
  ...
  | Ok resolved ->
    ...
    Alcotest.(check string)
      "path"
      "/tmp/oas-proof-store/proofs/r1/tool_traces/t.jsonl"
      resolved.path
  | Error e -> Alcotest.fail e
;;
...
let test_proof_store_read_json_and_jsonl () =
  let tmpdir =
    Filename.concat (Filename.get_temp_dir_name ()) "oas-test-proof-store-readers"
  in
  let config : Proof_store.config = { root = tmpdir } in
  let run_id = "test-run-readers-001" in
  Proof_store.init_run config ~run_id;
  Proof_store.write_evidence
    config
    ~run_id
    ~ref_id:"mode_violations"
    (`List [ `Assoc [ "tool_name", `String "bash" ] ]);
  Proof_store.append_tool_trace
    config
    ~run_id
    ~trace_id:"trace-0001"
    (`Assoc [ "tool_name", `String "read" ]);
  let json_ref = Proof_store.make_ref ~run_id ~subpath:"evidence/mode_violations.json" in
  let jsonl_ref = Proof_store.make_ref ~run_id ~subpath:"tool_traces/trace-0001.jsonl" in
  (match Proof_store.read_json config json_ref with
   | Ok (`List [ `Assoc fields ]) ->
     Alcotest.(check string)
       "tool_name"
       "bash"
       (match List.assoc "tool_name" fields with
        | `String s -> s
        | _ -> "")
   | Ok other ->
     Alcotest.fail
       (Printf.sprintf "unexpected json payload: %s" (Yojson.Safe.to_string other))
   | Error e -> Alcotest.fail e);
  (match Proof_store.read_jsonl config jsonl_ref with
   ...
   | Ok other ->
     Alcotest.fail
       (Printf.sprintf "unexpected jsonl payload count=%d" (List.length other))
   | Error e -> Alcotest.fail e);
  ignore (Sys.command (Printf.sprintf "rm -rf %s" tmpdir))
;;
...
let test_proof_store_load_manifest_and_contract () =
  let tmpdir =
    Filename.concat (Filename.get_temp_dir_name ()) "oas-test-proof-store-load"
  in
  let config : Proof_store.config = { root = tmpdir } in
...
let test_proof_store_list_runs () =
  let tmpdir =
    Filename.concat (Filename.get_temp_dir_name ()) "oas-test-proof-store-list"
  in
  let config : Proof_store.config = { root = tmpdir } in
...
let assert_json_string msg expected json =
  ...
  | other ->
    Alcotest.fail
      (Printf.sprintf "%s: expected string, got %s" msg (Yojson.Safe.to_string other))
;;
...
let test_result_status_json_lowercase () =
  ...
  assert_json_string "errored" "errored" (Cdal_proof.result_status_to_yojson Errored);
  assert_json_string
    "timed_out"
    "timed_out"
...
let test_proof_json_enum_fields () =
  let proof : Cdal_proof.t =
    ...
    ; mode_decision_source = "risk_class_downgrade"
    ; risk_class = Risk_class.High
    ; provider_snapshot = { provider_name = "test"; model_id = "m"; api_version = None }
    ; capability_snapshot = test_caps
    ; tool_trace_refs = []
    ; raw_evidence_refs = []
    ; checkpoint_ref = None
  ...
  in
  assert_json_string
    "requested mode in proof"
    "execute"
    (field "requested_execution_mode");
  assert_json_string "effective mode in proof" "draft" (field "effective_execution_mode");
  assert_json_string "risk class in proof" "high" (field "risk_class");
  assert_json_string "result status in proof" "completed" (field "result_status")
;;
...
let test_mode_decision : Mode_resolver.decision =
  { effective_mode = Execution_mode.Draft; source = "passthrough" }
;;
...
let make_test_store () =
  let tmpdir =
    Filename.concat
      ...
      (Printf.sprintf "oas-test-capture-%d" (Random.bits () land 0xFFFF))
  in
...
let test_proof_capture_lifecycle () =
  ...
  let _ = Hooks.invoke h.before_turn (BeforeTurn { turn = 1; messages = [] }) in
  let _ =
    Hooks.invoke
      h.pre_tool_use
      (PreToolUse
  ...
  let _ = Hooks.invoke h.after_turn (AfterTurn { turn = 1; response = resp }) in
  let _ = Hooks.invoke h.on_stop (OnStop { reason = Types.EndTurn; response = resp }) in
  ...
  (match Proof_store.read_jsonl store (List.hd proof.tool_trace_refs) with
   | Ok [ `Assoc fields ] ->
     Alcotest.(check string)
       "trace tool_use_id"
       "tu-bash"
       (match List.assoc "tool_use_id" fields with
        | `String value -> value
        | _ -> "");
     Alcotest.(check int)
       "trace planned_index"
       0
       (match List.assoc "planned_index" fields with
        | `Int value -> value
        | _ -> -1);
     Alcotest.(check string)
       "trace concurrency_class"
       "sequential_workspace"
       (match List.assoc "concurrency_class" fields with
        | `String value -> value
        | _ -> "")
   | Ok other ->
     Alcotest.fail
       (Printf.sprintf
          "unexpected tool trace payload: %s"
...
let test_proof_capture_multiple_tools () =
  ...
  let _ =
    Hooks.invoke
      ...
      (PostToolUse
         ...
         ; input = `String "b.ml"
         ; output =
             Error { Types.message = "conflict"; recoverable = true; error_class = None }
         ; result_bytes = 0
         ; duration_ms = 3.0
         ; schedule = default_schedule ~planned_index:2 ~batch_index:2 ()
         })
  ...
  (match Proof_store.read_jsonl store (List.hd proof.tool_trace_refs) with
   | Ok [ `Assoc fields ] ->
     Alcotest.(check string)
       "first trace concurrency_class"
       "parallel_read"
       (match List.assoc "concurrency_class" fields with
        | `String value -> value
        | _ -> "");
     Alcotest.(check int)
       "first trace planned_index"
       0
       (match List.assoc "planned_index" fields with
        | `Int value -> value
        | _ -> -1)
   | Ok other ->
     Alcotest.fail
       (Printf.sprintf
          "unexpected first trace payload: %s"
          (Yojson.Safe.to_string (`List other)))
   | Error e -> Alcotest.fail e);
  ignore (Sys.command (Printf.sprintf "rm -rf %s" tmpdir))
;;
...
let test_violation_records_details () =
  let st = diagnose_enforcer () in
  let h = Mode_enforcer.hooks st in
  let _ = Hooks.invoke h.pre_tool_use (make_enforcer_event "edit" (`String "file.ml")) in
  let vs = Mode_enforcer.violations st in
...
let test_evidence_violations_in_proof () =
  ...
  in
  let state =
    Proof_capture.create ~store ~contract ~mode_decision ~capability_snapshot:test_caps ()
  in
  let enforcer =
    Mode_enforcer.create ~contract ~effective_mode:Execution_mode.Diagnose ()
  in
  ...
  Alcotest.(check bool)
    ...
    (List.exists
       (fun r ->
          try
            ignore (Str.search_forward (Str.regexp_string "mode_violations") r 0);
            true
          with
          | Not_found -> false)
  ...
  (match effects_ref with
   ...
   | Some ref_ ->
     (match Proof_store.read_json store ref_ with
      ...
      | Ok other ->
        Alcotest.fail
          (Printf.sprintf "unexpected effects payload: %s" (Yojson.Safe.to_string other))
      | Error e -> Alcotest.fail e));
  ignore (Sys.command (Printf.sprintf "rm -rf %s" tmpdir))
;;
...
let test_evidence_token_usage () =
  ...
  in
  let enforcer =
    Mode_enforcer.create ~contract:test_contract ~effective_mode:Execution_mode.Draft ()
  in
  Proof_capture.set_enforcer state enforcer;
  let eh = Mode_enforcer.hooks enforcer in
  ...
  Alcotest.(check bool)
    ...
    (List.exists
       (fun r ->
          try
            ignore (Str.search_forward (Str.regexp_string "token_usage") r 0);
            true
          with
          | Not_found -> false)
       proof.raw_evidence_refs);
...
let test_evidence_review_warning () =
  ...
  Proof_capture.set_enforcer state enforcer;
  let eh = Mode_enforcer.hooks enforcer in
  let _ = Hooks.invoke eh.before_turn (Hooks.BeforeTurn { turn = 1; messages = [] }) in
  Alcotest.(check bool)
    "review warning set"
    true
  ...
  let proof = Proof_capture.finalize state ~result_status:Cdal_proof.Completed in
  Alcotest.(check bool)
    "has review_warning ref"
    true
    (List.exists
       (fun r ->
          try
            ignore (Str.search_forward (Str.regexp_string "review_warning") r 0);
            true
          with
          | Not_found -> false)
       proof.raw_evidence_refs);
...
let test_tool_effect_class_of_string () =
  ...
  Alcotest.(check bool)
    "workspace parses (compat)"
    true
    (Mode_enforcer.tool_effect_class_of_string "workspace"
     = Some Mode_enforcer.Local_mutation);
  Alcotest.(check bool)
    "workspace_mutating parses (compat)"
    true
    (Mode_enforcer.tool_effect_class_of_string "workspace_mutating"
     = Some Mode_enforcer.Local_mutation);
  Alcotest.(check bool)
    "local_mutation parses"
    true
    (Mode_enforcer.tool_effect_class_of_string "local_mutation"
     = Some Mode_enforcer.Local_mutation);
  Alcotest.(check bool)
    "external parses"
    true
    (Mode_enforcer.tool_effect_class_of_string "external"
     = Some Mode_enforcer.External_effect);
  Alcotest.(check bool)
    "external_effect parses"
    true
    (Mode_enforcer.tool_effect_class_of_string "external_effect"
     = Some Mode_enforcer.External_effect);
  Alcotest.(check bool)
    "shell_dynamic parses"
    true
    (Mode_enforcer.tool_effect_class_of_string "shell_dynamic"
     = Some Mode_enforcer.Shell_dynamic);
  Alcotest.(check bool)
...
let () =
  Alcotest.run
    ...
    ; "Risk_class", [ Alcotest.test_case "max_mode" `Quick test_risk_class_max_mode ]
    ; ( "Truth-layer primitives"
      , [ Alcotest.test_case
            "event envelope roundtrip"
            `Quick
            test_event_envelope_roundtrip
        ; Alcotest.test_case
            "event bus v2 from legacy"
            `Quick
            test_event_bus_envelope_v2_from_legacy
        ; Alcotest.test_case
            "effect evidence roundtrip"
            `Quick
            test_effect_evidence_roundtrip
        ; Alcotest.test_case
            "runtime health roundtrip"
            `Quick
    ...
    ; ( "Risk_contract"
      , [ Alcotest.test_case
            ...
            `Quick
            test_contract_id_deterministic
    ...
    ; ( "Hooks.compose"
      , [ Alcotest.test_case
            "outer skip bypasses inner"
            `Quick
            test_hooks_compose_outer_skip
    ...
    ; ( "Proof_store"
      , [ Alcotest.test_case "init and write" `Quick test_proof_store_init_and_write
        ...
        ; Alcotest.test_case
            "reject traversal ref"
            `Quick
            test_proof_store_rejects_traversal_ref
        ; Alcotest.test_case
            "reject traversal run_id"
            `Quick
            test_proof_store_rejects_traversal_run_id
        ; Alcotest.test_case
            "read json and jsonl"
            `Quick
            test_proof_store_read_json_and_jsonl
        ; Alcotest.test_case
            "load manifest and contract"
            `Quick
            test_proof_store_load_manifest_and_contract
    ...
    ; ( "Risk_contract edge"
      , [ Alcotest.test_case
            "empty eval_criteria"
            `Quick
            test_contract_id_empty_eval_criteria
        ] )
    ; ( "JSON schema conformance"
      , [ Alcotest.test_case
            "execution_mode lowercase"
            `Quick
            test_execution_mode_json_lowercase
        ; Alcotest.test_case "risk_class lowercase" `Quick test_risk_class_json_lowercase
        ; Alcotest.test_case
            "result_status lowercase"
            `Quick
            test_result_status_json_lowercase
    ...
    ; ( "Proof_capture"
      , [ Alcotest.test_case "full lifecycle" `Quick test_proof_capture_lifecycle
        ; Alcotest.test_case
            "no hooks fired (guard)"
            `Quick
            test_proof_capture_no_hooks_fired
    ...
    ; ( "Mode_enforcer"
      , [ Alcotest.test_case "diagnose blocks write" `Quick test_diagnose_blocks_write
        ...
        ; Alcotest.test_case
            "violation records details"
            `Quick
            test_violation_records_details
        ; Alcotest.test_case
            "violation JSON roundtrip"
            `Quick
            test_violation_json_roundtrip
        ; Alcotest.test_case
            "violation_kind roundtrip"
            `Quick
            test_violation_kind_roundtrip
        ] )
    ; ( "Mode_resolver capability"
      , [ Alcotest.test_case "read-only tools" `Quick test_resolver_read_only_tools
        ; Alcotest.test_case
            ...
            `Quick
            test_resolver_workspace_only_tools
    ...
    ; ( "Evidence enrichment"
      , [ Alcotest.test_case
            "violations in proof"
            `Quick
            test_evidence_violations_in_proof
    ...
    ; ( "Tool classification"
      , [ Alcotest.test_case "known tools" `Quick test_classify_known_tools
        ; Alcotest.test_case
            "registry override"
            `Quick
            test_classify_via_registry_override
        ; Alcotest.test_case
            "tool_effect_class_of_string"
            `Quick
            test_tool_effect_class_of_string
        ; Alcotest.test_case
            "shell pattern classification"
            `Quick
            test_shell_pattern_classification
        ] )
    ]
;;
