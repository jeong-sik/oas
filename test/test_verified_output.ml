  ...
  Alcotest.run
    ...
    [ ( "phantom_safety"
      , [ Alcotest.test_case "verified output gives content" `Quick (fun () ->
            let raw =
              Verified_output.of_response ~producer:"alice" (mock_response "hello")
            in
            match
              Verified_output.verify
                raw
                ~verifier:"bob"
                ~confidence:0.9
                ~evidence:"checked"
    ...
    *)
          Alcotest.test_case
            "unverified cannot access content (documented)"
            `Quick
            (fun () ->
               let _raw =
                 Verified_output.of_response ~producer:"alice" (mock_response "hello")
    ...
    ; ( "verification_flow"
      , [ Alcotest.test_case "low confidence fails verification" `Quick (fun () ->
            ...
            in
            match
              Verified_output.verify raw ~verifier:"bob" ~confidence:0.3 ~evidence:"weak"
            with
        ...
        ; Alcotest.test_case "verify_with_results quorum" `Quick (fun () ->
            ...
            let results =
              [ Verified_output.Verified
                  { verifier = "bob"; confidence = 0.8; evidence = "ok" }
              ; Verified_output.Disputed { verifier = "charlie"; reason = "disagree" }
              ; Verified_output.Verified
                  { verifier = "dave"; confidence = 0.9; evidence = "confirmed" }
              ]
            in
    ...
    ; ( "trust"
      , [ Alcotest.test_case "trust bypasses verification" `Quick (fun () ->
            let raw =
              Verified_output.of_response ~producer:"alice" (mock_response "trusted")
            in
            ...
            Alcotest.(check bool)
              "is_verified"
              true
              (Verified_output.is_verified verified))
        ] )
    ; ( "accessors"
      , [ Alcotest.test_case "producer works on any status" `Quick (fun () ->
            ...
            Alcotest.(check string)
              "producer unverified"
              "alice"
              (Verified_output.producer raw);
            let verified = Verified_output.trust raw ~reason:"test" in
            Alcotest.(check string)
              "producer verified"
              "alice"
              (Verified_output.producer verified))
