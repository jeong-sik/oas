...
let test_behavioral_tool_selected () =
  ...
  let verdict = Harness.Behavioral.evaluate obs
    (ToolSelected ["get_weather"]) in
  Alcotest.(check bool) "tool found" true verdict.passed;
  let verdict2 = Harness.Behavioral.evaluate obs
    (ToolSelected ["missing_tool"]) in
...
let test_behavioral_all () =
  ...
  let verdict = Harness.Behavioral.evaluate obs
    (All [ToolSelected ["search"]; CompletesWithin 5; ContainsText "Found"]) in
  Alcotest.(check bool) "all pass" true verdict.passed;
  let verdict2 = Harness.Behavioral.evaluate obs
    (All [ToolSelected ["search"]; CompletesWithin 1]) in
...
let test_performance_evaluate () =
  ...
  let exp = { Harness.Performance.default_expectation with
...
let test_regression_fuzzy_match () =
  ...
  let v = Harness.Regression.evaluate
    ~mode:(FuzzyMatch { threshold = 0.5 }) obs "hello earth" in
...
let test_swiss_cheese_all_pass () =
  ...
  Alcotest.(check (float 0.01)) "coverage 1.0" 1.0
    (Option.get verdict.score)
...
let () =
  Alcotest.run "harness" [
    ...
    ("composability", [
      ...
      Alcotest.test_case "context_propagated" `Quick test_composability_context_propagated;
    ]);
  ]
