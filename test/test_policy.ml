...
let test_all_verdict_types () =
  ...
  (match Policy.evaluate p1 dp with
   | Deny "blocked" -> ()
   | _ -> fail "expected Deny");
  (* Allow *)
  let p2 = Policy.create [ allow_tool_rule "t" ] in
  (match Policy.evaluate p2 dp with
   | Allow -> ()
...
let test_verdict_to_string () =
  ...
  check
    string
    "cond"
    "AllowWithCondition(cond)"
    (Policy.verdict_to_string (AllowWithCondition "cond"));
  check string "escalate" "Escalate(esc)" (Policy.verdict_to_string (Escalate "esc"))
;;

let test_decision_point_to_string () =
  let s1 =
    Policy.decision_point_to_string (BeforeToolCall { tool_name = "t"; agent_name = "a" })
  in
  check bool "contains BeforeToolCall" true (String.length s1 > 0);
  let s2 =
    Policy.decision_point_to_string (BeforeHandoff { from_agent = "x"; to_agent = "y" })
  in
  check bool "contains BeforeHandoff" true (String.length s2 > 0);
  let s3 =
    Policy.decision_point_to_string
      (BeforeResponse { agent_name = "a"; content_preview = "hi" })
  in
...
let test_first_matching_rule_wins () =
  ...
  match Policy.evaluate p dp with
  | Deny "first-match" -> ()
  | v -> fail (Printf.sprintf "expected first-match, got %s" (Policy.verdict_to_string v))
;;
...
let () =
  run
    ...
    ; ( "no-match"
      , [ test_case
            "no matching rule returns Allow"
            `Quick
            test_no_matching_rule_returns_allow
        ; test_case "empty policy returns Allow" `Quick test_empty_policy_returns_allow
        ] )
    ; "verdicts", [ test_case "all verdict types" `Quick test_all_verdict_types ]
    ; ( "management"
      , [ test_case "add rule" `Quick test_add_rule
        ; test_case "remove rule" `Quick test_remove_rule
        ; test_case
            "add maintains priority order"
            `Quick
            test_add_maintains_priority_order
        ] )
    ; ( "strings"
