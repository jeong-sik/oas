...
let test_remember_defaults_to_working () =
  ...
  let json =
    execute_ok_json tool
      (`Assoc
        [
          ("key", `String "count");
          ("value_json", `String "42");
        ])
  in
  check bool "ok" true
    (Yojson.Safe.Util.(json |> member "ok" |> to_bool));
  check string "tier" "working"
    (Yojson.Safe.Util.(json |> member "tier" |> to_string));
  check (option int) "stored value" (Some 42)
  ...
  ignore
    (execute_ok_json tool
       (`Assoc
         [
           ("key", `String "note");
           ("value_json", `String "hello world");
         ]));
  check string "raw string" "hello world"
...
let test_recall_found_and_missing () =
  ...
  check bool "found" true
    (Yojson.Safe.Util.(found |> member "found" |> to_bool));
  check bool "value.enabled" true
    (Yojson.Safe.Util.(found |> member "value" |> member "enabled" |> to_bool));
  let missing =
    execute_ok_json tool (`Assoc [ ("key", `String "missing") ])
  in
  check bool "missing" false
    (Yojson.Safe.Util.(missing |> member "found" |> to_bool))
...
let test_invalid_generic_tier_is_rejected () =
  ...
  in
  check bool "mentions generic tools" true
...
let test_remember_episode_generates_id () =
  ...
  check bool "generated prefix" true
    (Util.string_contains ~needle:"ep_" episode_id);
...
let test_remember_acl_denied_by_default () =
  ...
  in
  check bool "denied" true
    (Util.string_contains ~needle:"Access denied" message)
...
let test_all_acl_reuses_same_tool_surface () =
  ...
  in
  check (list string) "acl tool names"
    [
      "memory_remember";
      "memory_recall";
      "memory_remember_episode";
...
let () =
  run "Memory_tools"
    [
      ( "surface",
        [
          test_case "all exposes expected names" `Quick
            test_all_exposes_expected_names;
          test_case "all_acl reuses same tool names" `Quick
            test_all_acl_reuses_same_tool_surface;
        ] );
      ( "remember",
        [
          test_case "defaults to working" `Quick
            test_remember_defaults_to_working;
          test_case "raw string fallback" `Quick
            test_remember_falls_back_to_raw_string;
          test_case "invalid generic tier" `Quick
            test_invalid_generic_tier_is_rejected;
          test_case "acl denied by default" `Quick
            test_remember_acl_denied_by_default;
        ] );
      ( "recall",
        [
          test_case "found and missing" `Quick
            test_recall_found_and_missing;
        ] );
      ( "episodic",
        [
          test_case "generated id" `Quick
            test_remember_episode_generates_id;
        ] );
      ( "procedural",
        [
          test_case "threshold and touch" `Quick
