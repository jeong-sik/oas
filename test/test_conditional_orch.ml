...
let test_always () =
  Alcotest.(check bool) "always true" true
    (Orchestrator.eval_condition None Always)

let test_result_ok () =
  let ok_result = Some (make_result "t1" "agent-a") in
  let err_result = Some (make_result ~ok:false "t2" "agent-b") in
  Alcotest.(check bool) "ok result" true
    (Orchestrator.eval_condition ok_result ResultOk);
  Alcotest.(check bool) "error result" false
...
let test_loop_construction () =
  ...
  match plan with
  | Loop { max_iterations; _ } ->
    Alcotest.(check int) "max iterations" 3 max_iterations
  | _ -> Alcotest.fail "wrong plan type"
...
let () =
  Alcotest.run "conditional_orchestration" [
    ("route_condition", [
