...
let test_is_context_overflow_message () =
  ...
  check
    bool
    "raw non-overflow message"
    false
    (Retry.is_context_overflow_message "Invalid request: bad tool schema");
  check
    bool
    "json overflow message"
    true
    (Retry.is_context_overflow_message
       {|{"error":{"message":"This model's maximum context length is 128000 tokens. available context size (32768)"}}|});
  check
    bool
    "malformed json falls back to raw body"
    true
    (Retry.is_context_overflow_message {|{"error":"available context size (4096)"|});
  check
    bool
    "generic bad request"
    false
    (Retry.is_context_overflow_message {|{"error":{"message":"bad tool schema"}}|})
;;
...
let test_calculate_delay_cap () =
  let config : Retry.retry_config =
    { max_retries = 10; initial_delay = 1.0; max_delay = 5.0; backoff_factor = 2.0 }
  in
...
let fast_config : Retry.retry_config =
  { max_retries = 3; initial_delay = 0.001; max_delay = 0.005; backoff_factor = 2.0 }
;;
...
let test_with_retry_rate_limited_uses_retry_after () =
  ...
  let f () =
    incr attempt;
    if !attempt < 2
    then Error (Retry.RateLimited { retry_after = Some 0.001; message = "slow down" })
    else Ok "ok"
  in
  (match Retry.with_retry ~clock ~config:fast_config f with
...
let test_with_retry_map_error_succeeds_after_retries () =
  ...
  let f () =
    incr attempt;
    if !attempt < 3
    then Error (Retryable (Retry.ServerError { status = 500; message = "transient" }))
    else Ok "recovered"
  in
  (match
...
let test_provider_constructors () =
  ...
  let p = Provider.provider_a_sonnet () in
  check string "provider_a_sonnet" "agent_llm_a-sonnet-4-6" p.model_id;
  let p = Provider.provider_a_haiku () in
  ...
  let p = Provider.provider_o_router ~model_id:"google/gemini-2.5-pro" () in
  check string "provider_o_router override" "google/gemini-2.5-pro" p.model_id
;;
  ...
  run
    ...
    ; ( "with_retry"
      , [ test_case "succeeds first try" `Quick test_with_retry_succeeds_first_try
        ...
        ; test_case
            "rate limited uses retry_after"
            `Quick
            test_with_retry_rate_limited_uses_retry_after
        ; test_case
            "non-retryable stops immediately"
            `Quick
            test_with_retry_non_retryable_stops
        ; test_case
            "non-retryable during loop"
            `Quick
            test_with_retry_non_retryable_during_loop
        ; test_case
            "map_error retries classified errors"
            `Quick
            test_with_retry_map_error_succeeds_after_retries
        ; test_case
            "map_error preserves original errors"
            `Quick
            test_with_retry_map_error_preserves_original_error
