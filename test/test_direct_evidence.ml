...
let test_direct_evidence_materializes_bundle () =
  ...
  in
  Alcotest.(check bool) "snapshot completed" true (snapshot.status = Agent.Completed);
  Alcotest.(check bool) "snapshot accepted_at" true (Option.is_some snapshot.accepted_at);
  Alcotest.(check bool) "snapshot ready_at" true (Option.is_some snapshot.ready_at);
  Alcotest.(check bool)
    "snapshot first_progress_at"
    true
    (Option.is_some snapshot.first_progress_at);
  let worker =
    unwrap
      (Direct_evidence.get_worker_run ~agent ~raw_trace ~options:(direct_options root) ())
  in
  Alcotest.(check string) "worker name" "direct-worker" worker.agent_name;
  Alcotest.(check (option string)) "worker id" (Some "worker-direct-1") worker.worker_id;
  Alcotest.(check (option string))
    "runtime actor"
    (Some "direct-worker")
    worker.runtime_actor;
  Alcotest.(check (option string)) "worker role" (Some "implementer") worker.role;
  Alcotest.(check (list string))
    "worker aliases"
    [ "impl-1"; "worker-alpha" ]
    worker.aliases;
  Alcotest.(check (option string)) "primary alias" (Some "impl-1") worker.primary_alias;
  Alcotest.(check (option string))
    "resolved provider"
    (Some "openai-compat")
  ...
  Alcotest.(check (option string))
    "resolved model"
    (Some "direct-evidence-model")
    worker.resolved_model;
  let bundle =
    unwrap
      (Direct_evidence.get_proof_bundle
         ~agent
         ~raw_trace
         ~options:(direct_options root)
         ())
  in
  Alcotest.(check bool) "proof bundle enabled" true bundle.capabilities.proof_bundle;
  ...
  Alcotest.(check bool)
    "hook summary non-empty"
    true
    (List.length bundle.hook_summary >= 3);
  Alcotest.(check bool)
    "hook summary has before_turn"
    true
    (List.exists
       (fun (hook : Sessions.hook_summary) -> String.equal hook.hook_name "before_turn")
       bundle.hook_summary);
  Alcotest.(check bool)
    "hook summary has on_stop"
    true
    (List.exists
       (fun (hook : Sessions.hook_summary) -> String.equal hook.hook_name "on_stop")
       bundle.hook_summary);
  Alcotest.(check bool)
    "structured telemetry tracks tool catalog artifact"
    true
    (List.exists
  ...
  Alcotest.(check bool)
    ...
    true
    (List.exists
       (fun (file : Sessions.evidence_file) -> String.equal file.label "telemetry_json")
  ...
  Alcotest.(check bool)
    ...
    true
    (List.exists
       (fun (file : Sessions.evidence_file) -> String.equal file.label "raw_trace_json")
       bundle.evidence.files);
  Alcotest.(check bool)
    "evidence includes tool_catalog_json"
    true
    (List.exists
  ...
  let report =
    unwrap
      (Direct_evidence.get_conformance
         ~agent
         ~raw_trace
         ~options:(direct_options root)
         ())
  ...
  Alcotest.(check bool)
    "conformance has proof bundle check"
    true
    (List.exists
  ...
  Alcotest.(check (option string))
    "latest resolved provider"
    (Some "openai-compat")
    report.summary.latest_resolved_provider;
  Alcotest.(check bool)
    "hook event count positive"
    true
    (report.summary.hook_event_count >= 3);
  Alcotest.(check int) "tool catalog count one" 1 report.summary.tool_catalog_count
;;
...
let () =
  ...
  run
    ...
    [ ( "bundle"
      , [ test_case
            "materializes direct proof bundle"
            `Quick
