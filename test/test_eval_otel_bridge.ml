...
let () =
  ...
  let test_metric_list_count () =
    let rm = make_run_metrics
      ~verdicts:[make_verdict ~passed:true (); make_verdict ~passed:false ()] () in
    let snap = Eval_otel_bridge.extract rm in
  ...
  let test_json_export () =
    let rm = make_run_metrics
      ~verdicts:[make_verdict ~passed:true ()] () in
    let snap = Eval_otel_bridge.extract rm in
