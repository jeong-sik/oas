...
let diverse_summary =
  make_summary
    ...
    ~tool_execution_started_count:8 ~assistant_block_count:6

(* Worker C: completely different tools from B — high divergence *)
let divergent_summary =
  make_summary
    ~tool_names:
      [ "database_query"; "api_call"; "log_analysis"; "metric_check" ]
    ~tool_execution_started_count:7 ~assistant_block_count:5
...
let clone_of_diverse =
  make_summary
    ...
    ~tool_execution_started_count:9 ~assistant_block_count:7

(* Worker with no tool calls *)
let empty_summary =
  make_summary ~tool_names:[] ~tool_execution_started_count:0
...
let test_metric_diverse () =
  ...
  Alcotest.(check bool)
    ...
    (m.diversity >= 0.3);
  Alcotest.(check int)
    "unique_tool_count = 5" 5 m.unique_tool_count;
  Alcotest.(check int)
    "total_calls = 8" 8 m.total_calls
...
let test_metric_scripted () =
  ...
  Alcotest.(check bool)
    ...
    (m.diversity < 0.15);
  Alcotest.(check int)
    "unique_tool_count = 1" 1 m.unique_tool_count

let test_metric_empty () =
  let m = metric_of_summary empty_summary in
  Alcotest.(check (float 0.001))
    "diversity = 0.0 when no calls" 0.0 m.diversity;
  Alcotest.(check int)
    "unique_tool_count = 0" 0 m.unique_tool_count
...
let test_divergence_identical () =
  ...
  Alcotest.(check (float 0.001))
    "jaccard_distance = 0.0 for identical tool sets" 0.0 d.jaccard_distance;
  Alcotest.(check int)
    "no exclusive tools in a" 0
    (List.length d.exclusive_a);
  Alcotest.(check int)
    "no exclusive tools in b" 0
    (List.length d.exclusive_b)
...
let test_divergence_disjoint () =
  ...
  let d = divergence_of_pair a b in
  Alcotest.(check bool)
    "jaccard_distance > 0.5 for mostly-disjoint sets" true
    (d.jaccard_distance > 0.5);
  Alcotest.(check bool)
    "has exclusive tools in a" true
    (List.length d.exclusive_a > 0);
  Alcotest.(check bool)
    "has exclusive tools in b" true
    (List.length d.exclusive_b > 0)

let test_divergence_completely_disjoint () =
  let a =
    metric_of_summary
      (make_summary ~tool_names:[ "alpha"; "beta" ]
         ~tool_execution_started_count:4 ~assistant_block_count:2)
  in
  let b =
    metric_of_summary
      (make_summary ~tool_names:[ "gamma"; "delta" ]
         ~tool_execution_started_count:4 ~assistant_block_count:2)
  in
  let d = divergence_of_pair a b in
  Alcotest.(check (float 0.001))
    "jaccard_distance = 1.0 for completely disjoint sets" 1.0
    d.jaccard_distance
...
let test_divergence_empty_sets () =
  ...
  let d = divergence_of_pair a b in
  Alcotest.(check (float 0.001))
    "jaccard_distance = 0.0 for two empty sets" 0.0 d.jaccard_distance

let test_verdict_autonomous () =
  let v = analyze [ diverse_summary; divergent_summary ] in
  Alcotest.(check classification_testable)
    "classification = autonomous" Autonomous v.classification;
  Alcotest.(check bool)
    "confidence > 0.5" true
    (v.confidence > 0.5);
  Alcotest.(check bool)
    "mean_diversity > 0.3" true
    (v.mean_diversity > 0.3);
  Alcotest.(check bool)
    "mean_divergence > 0.0" true
    (v.mean_divergence > 0.0)

let test_verdict_scripted () =
  let v = analyze [ scripted_summary; scripted_summary ] in
  Alcotest.(check classification_testable)
    "classification = scripted" Scripted v.classification;
  Alcotest.(check bool)
    "mean_diversity < 0.15" true
    (v.mean_diversity < 0.15);
  Alcotest.(check (float 0.001))
    "mean_divergence = 0.0 (identical)" 0.0 v.mean_divergence
...
let test_verdict_scripted_identical_diverse () =
  ...
  Alcotest.(check classification_testable)
    "identical tool sets with divergence=0 → scripted" Scripted
    v.classification;
  Alcotest.(check (float 0.001))
    "zero divergence" 0.0 v.mean_divergence

let test_verdict_insufficient_empty () =
  let v = analyze [] in
  Alcotest.(check classification_testable)
    "empty list → insufficient" Insufficient_data v.classification

let test_verdict_single_worker_diverse () =
  let v = analyze [ diverse_summary ] in
  Alcotest.(check classification_testable)
    "single diverse worker" Autonomous v.classification;
  Alcotest.(check bool)
    "confidence <= 0.7 (lower for single)" true
    (v.confidence <= 0.7)
...
let test_verdict_random () =
  ...
  let w1 =
    make_summary ~tool_names:[ "random_a" ]
      ~tool_execution_started_count:10 ~assistant_block_count:2
  in
  let w2 =
    make_summary ~tool_names:[ "random_b" ]
      ~tool_execution_started_count:10 ~assistant_block_count:2
  in
...
let test_verdict_to_json () =
  ...
  Alcotest.(check bool)
    "JSON contains classification" true
    (Util.string_contains ~needle:"Autonomous" json_str);
  Alcotest.(check bool)
    "JSON contains evidence" true
...
let test_three_workers_partial_overlap () =
  ...
  let w1 =
    make_summary
      ...
      ~tool_execution_started_count:6 ~assistant_block_count:4
  in
  let w2 =
    make_summary
  ...
  Alcotest.(check classification_testable)
    "3 workers with partial overlap → autonomous" Autonomous
    v.classification;
  Alcotest.(check bool)
    "3 pairwise divergences" true
    (List.length v.pairwise_divergences = 3);
  Alcotest.(check bool)
    "all divergences > 0" true
    (List.for_all
...
let () =
  Alcotest.run "Autonomy Trace Analyzer"
    [
      ( "metric",
        [
      ...
      ( "divergence",
        [
          Alcotest.test_case "identical sets" `Quick
            test_divergence_identical;
          Alcotest.test_case "disjoint sets" `Quick
            test_divergence_disjoint;
          Alcotest.test_case "completely disjoint" `Quick
      ...
      ( "verdict",
        [
          ...
          Alcotest.test_case "scripted identical diverse" `Quick
            test_verdict_scripted_identical_diverse;
          Alcotest.test_case "insufficient empty" `Quick
            test_verdict_insufficient_empty;
          Alcotest.test_case "single worker diverse" `Quick
            test_verdict_single_worker_diverse;
          Alcotest.test_case "single worker scripted" `Quick
            test_verdict_single_worker_scripted;
          Alcotest.test_case "random" `Quick test_verdict_random;
          Alcotest.test_case "three workers" `Quick
            test_three_workers_partial_overlap;
