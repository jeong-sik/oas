...
let test_clone_state_divergence () =
  ...
  Agent.set_state agent { (Agent.state agent) with
    turn_count = 10;
    messages = [{ Types.role = User; content = [Text "original only"]; name = None; tool_call_id = None ; metadata = []}] };
...
let test_clone_from_resumed_agent () =
  ...
  Agent.set_state agent { (Agent.state agent) with
    turn_count = 3;
    messages = [
      { Types.role = User; content = [Text "msg1"]; name = None; tool_call_id = None ; metadata = []};
      { Types.role = Assistant; content = [Text "reply1"]; name = None; tool_call_id = None ; metadata = []};
    ] };
