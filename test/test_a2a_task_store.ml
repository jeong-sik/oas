    ...
    Fun.protect
      ~finally:(fun () ->
        ...
        try
          ...
          List.iter
            (fun name ->
               try Eio.Path.unlink Eio.Path.(path / name) with
               | _ -> ())
            entries;
...
let test_list_tasks () =
  ...
  (match A2a_task_store.store_task store t1 with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
  (match A2a_task_store.store_task store t2 with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
...
let test_gc_removes_old_terminal () =
  ...
  @@ fun store ->
  let old_task =
    { (make_task ~id:"old_done" ~state:Completed ()) with
      updated_at = Unix.gettimeofday () -. 200.0
    }
  in
  ...
  (match A2a_task_store.store_task store old_task with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
  (match A2a_task_store.store_task store recent_task with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
  ...
  | Ok removed ->
    ...
    check
      ...
      "old removed"
      true
      (Option.is_none (A2a_task_store.get_task store "old_done"));
    check
      bool
      "recent kept"
      true
      (Option.is_some (A2a_task_store.get_task store "recent"))
  | Error e -> fail (Error.to_string e)
;;
...
let test_gc_keeps_non_terminal () =
  ...
  @@ fun store ->
  let working_task =
    { (make_task ~id:"working" ~state:Working ()) with
      updated_at = Unix.gettimeofday () -. 200.0
    }
  in
  (match A2a_task_store.store_task store working_task with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
  match A2a_task_store.gc ~max_age_s:100.0 store with
  | Ok removed ->
    ...
    check
      ...
      "still there"
      true
      (Option.is_some (A2a_task_store.get_task store "working"))
  | Error e -> fail (Error.to_string e)
;;
...
let test_concurrent_writes () =
  ...
  (match A2a_task_store.store_task store task1 with
   | Ok () -> ()
   | Error e -> fail (Error.to_string e));
