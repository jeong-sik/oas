...
let make_telemetry ?(session_id = "sess-001") () : Sessions.telemetry =
  { session_id; generated_at = 1000.0; step_count = 0; event_counts = []; steps = [] }
;;
...
let make_worker_run
      ...
      ?(worker_id = Some "w-1")
      ?(agent_name = "agent-a")
      ?(runtime_actor = Some "actor-a")
      ?(role = None)
      ?(aliases = [ "alias-a" ])
      ?(primary_alias = Some "alias-a")
...
let make_raw_trace_summary ?(worker_run_id = "run-1") ?(agent_name = "agent-a") ()
  : Sessions.raw_trace_summary
  =
...
let make_raw_trace_validation
      ?(worker_run_id = "run-1")
      ?(agent_name = "agent-a")
      ?(ok = true)
      ()
  : Sessions.raw_trace_validation
  =
  { run_ref = make_run_ref ~worker_run_id ~agent_name ()
  ; ok
  ; checks = []
...
let make_bundle
      ...
      ?(raw_trace_run_count = 0)
      ?(validated_worker_run_count = 0)
      ?(trace_capabilities = [])
      ?(hook_summary = [])
      ?(tool_catalog = [])
      ?(proof_bundle_cap = true)
      ()
  : Sessions.proof_bundle
  =
...
let test_report_valid_bundle () =
  ...
  let sum1 = make_raw_trace_summary ~worker_run_id:"r1" ~agent_name:"tester" () in
  let val1 = make_raw_trace_validation ~worker_run_id:"r1" ~agent_name:"tester" () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
      ~raw_trace_runs:[ ref1 ]
  ...
  let failed = List.filter (fun (c : Conformance.check) -> not c.passed) report.checks in
...
let test_report_empty_bundle () =
  ...
  Alcotest.(check (list string)) "empty aliases" [] report.summary.latest_worker_aliases
;;
...
let test_report_summary_fields () =
  ...
  in
  let report = Conformance.report bundle in
  Alcotest.(check string) "session_id" "sess-001" report.summary.session_id;
  Alcotest.(check int) "worker_run_count" 1 report.summary.worker_run_count;
  Alcotest.(check (option string))
...
let test_report_broken_bundle () =
  let w =
    make_worker_run
      ...
      ()
  in
  let bundle = make_bundle ~worker_runs:[ w ] ~latest_worker_run:(Some w) () in
  let report = Conformance.report bundle in
...
let test_check_latest_worker_inconsistent () =
  ...
  let bundle = make_bundle ~worker_runs:[ w1; w2 ] ~latest_worker_run:(Some w1) () in
  let report = Conformance.report bundle in
...
let test_check_latest_accepted_consistent () =
  let w = make_worker_run ~worker_run_id:"r1" ~status:Sessions.Accepted () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
...
let test_check_latest_ready_inconsistent () =
  let w1 = make_worker_run ~worker_run_id:"r1" ~status:Sessions.Ready () in
  let w2 = make_worker_run ~worker_run_id:"r2" ~status:Sessions.Ready () in
  let bundle =
    make_bundle
      ~worker_runs:[ w1; w2 ]
...
let test_check_latest_running_consistent () =
  let w = make_worker_run ~worker_run_id:"r1" ~status:Sessions.Running () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
...
let test_check_latest_completed_consistent () =
  let w = make_worker_run ~worker_run_id:"r1" ~status:Sessions.Completed () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
      ~latest_completed_worker_run:(Some w)
...
let test_check_lifecycle_non_monotonic () =
  let w =
    make_worker_run
      ~started_at:(Some 300.0)
      ~last_progress_at:(Some 100.0)
      ~finished_at:(Some 400.0)
      ()
  in
  let bundle = make_bundle ~worker_runs:[ w ] ~latest_worker_run:(Some w) () in
  let report = Conformance.report bundle in
...
let test_check_validated_not_raw () =
  let w = make_worker_run ~validated:true ~trace_capability:Sessions.Summary_only () in
  let bundle =
    make_bundle
      ~worker_runs:[ w ]
      ~validated_worker_runs:[ w ]
...
let test_report_tool_catalog_count () =
  ...
  let report = Conformance.report bundle in
  Alcotest.(check int) "tool_catalog_count" 1 report.summary.tool_catalog_count
;;
...
let test_check_direct_evidence_missing_raw () =
  let w = make_worker_run ~worker_run_id:"r1" ~trace_capability:Sessions.Raw () in
  (* Worker is Raw but no matching raw_trace_run *)
  let bundle =
    make_bundle ~worker_runs:[ w ] ~raw_trace_runs:[] ~latest_worker_run:(Some w) ()
  in
  let report = Conformance.report bundle in
  ...
  Alcotest.run
    ...
    [ ( "report_basic"
      , [ Alcotest.test_case "valid_bundle" `Quick test_report_valid_bundle
        ...
        ; Alcotest.test_case "summary_fields" `Quick test_report_summary_fields
        ; Alcotest.test_case "broken_bundle" `Quick test_report_broken_bundle
        ] )
    ; ( "check_counts"
      , [ Alcotest.test_case
            "validated_count_mismatch"
            `Quick
            test_check_validated_count_mismatch
        ; Alcotest.test_case
            "raw_trace_shape_mismatch"
            `Quick
            test_check_raw_trace_shape_mismatch
        ] )
    ; ( "check_latest_workers"
      , [ Alcotest.test_case
            "latest_worker_inconsistent"
            `Quick
            test_check_latest_worker_inconsistent
        ; Alcotest.test_case
            "latest_accepted_consistent"
            `Quick
            test_check_latest_accepted_consistent
        ; Alcotest.test_case
            "latest_ready_inconsistent"
            `Quick
            test_check_latest_ready_inconsistent
        ; Alcotest.test_case
            "latest_running_consistent"
            `Quick
            test_check_latest_running_consistent
        ; Alcotest.test_case
            "latest_completed_consistent"
            `Quick
            test_check_latest_completed_consistent
        ; Alcotest.test_case
            "latest_validated_inconsistent"
            `Quick
            test_check_latest_validated_inconsistent
        ; Alcotest.test_case
            "latest_failed_consistent"
            `Quick
            test_check_latest_failed_consistent
        ] )
    ; ( "check_lifecycle"
      , [ Alcotest.test_case "non_monotonic" `Quick test_check_lifecycle_non_monotonic
        ; Alcotest.test_case "monotonic_ok" `Quick test_check_lifecycle_monotonic_ok
        ; Alcotest.test_case "none_values" `Quick test_check_lifecycle_none_values
        ] )
    ; ( "check_identity"
      , [ Alcotest.test_case "alias_mismatch" `Quick test_check_identity_alias_mismatch
        ; Alcotest.test_case "no_worker_id" `Quick test_check_identity_no_worker_id
        ; Alcotest.test_case "consistent" `Quick test_check_identity_consistent
        ] )
    ; ( "check_resolved"
      , [ Alcotest.test_case "missing" `Quick test_check_resolved_runtime_missing
        ; Alcotest.test_case "ok_for_planned" `Quick test_check_resolved_ok_for_planned
        ] )
    ; ( "check_validated_raw"
      , [ Alcotest.test_case "not_raw_capable" `Quick test_check_validated_not_raw ] )
    ; ( "check_trace_capabilities"
      , [ Alcotest.test_case "consistent" `Quick test_check_trace_capabilities_consistent
        ; Alcotest.test_case
            "inconsistent"
            `Quick
    ...
    ; ( "check_raw_trace"
      , [ Alcotest.test_case "not_addressable" `Quick test_check_raw_trace_not_addressable
        ] )
    ; ( "check_proof_bundle"
      , [ Alcotest.test_case "available" `Quick test_check_proof_bundle_available
        ; Alcotest.test_case "unavailable" `Quick test_check_proof_bundle_unavailable
        ] )
    ; ( "check_direct_evidence"
      , [ Alcotest.test_case "missing_raw" `Quick test_check_direct_evidence_missing_raw
        ; Alcotest.test_case
            "summary_only_ok"
            `Quick
    ...
    ; ( "report_summary"
      , [ Alcotest.test_case "hook_count" `Quick test_report_hook_summary_count
        ; Alcotest.test_case "tool_catalog_count" `Quick test_report_tool_catalog_count
        ; Alcotest.test_case "failed_worker" `Quick test_report_failed_worker
        ; Alcotest.test_case
            "failed_error_fallback"
            `Quick
