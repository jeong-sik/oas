...
let with_temp_dir f =
  let dir = Filename.concat (Filename.get_temp_dir_name ())
    (Printf.sprintf "oas-test-%d-%06x" (Unix.getpid ()) (Random.int 0xFFFFFF)) in
...
let test_save_load_roundtrip () =
  with_temp_dir (fun dir ->
    ...
    | Ok loaded ->
      Alcotest.(check string) "roundtrip" content loaded
    | Error e -> Alcotest.fail (Error.to_string e))
...
let test_save_load_empty () =
  with_temp_dir (fun dir ->
    ...
    match Runtime_store.load_text path with
    | Ok loaded ->
      Alcotest.(check string) "empty" "" loaded
    | Error e -> Alcotest.fail (Error.to_string e))

let test_save_load_unicode () =
  with_temp_dir (fun dir ->
    let path = Filename.concat dir "unicode.txt" in
    let content = "Korean: \xed\x95\x9c\xea\xb5\xad\xec\x96\xb4 Emoji: \xf0\x9f\x92\xbb" in
    (match Runtime_store.save_text path content with
     | Ok () -> ()
     | Error e -> Alcotest.fail (Error.to_string e));
    match Runtime_store.load_text path with
    | Ok loaded ->
      Alcotest.(check string) "unicode roundtrip" content loaded
    | Error e -> Alcotest.fail (Error.to_string e))
...
let test_store_create () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      Alcotest.(check bool) "sessions dir exists" true
...
let test_load_session_corrupt () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      let sess_dir = Filename.concat
        (Filename.concat root "sessions") "bad-sess" in
  ...
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      | Ok events ->
        ...
        List.iter (fun (ev : Runtime.event) ->
          Alcotest.(check bool) "seq > 3" true (ev.seq > 3)
        ) events)
...
let test_read_events_no_file () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      | Ok events ->
        Alcotest.(check int) "empty" 0 (List.length events)
      | Error e -> Alcotest.fail (Error.to_string e))
...
let test_snapshot_path_with_label () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      Alcotest.(check bool) "contains 0005" true
        (String.length base >= 4
         && String.sub base 0 4 = "0005");
...
let test_snapshot_path_slash_in_label () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      ...
      Alcotest.(check bool) "no slash in basename" true
        (not (String.contains base '/')))
...
let test_save_artifact_text () =
  with_temp_dir (fun dir ->
    ...
    | Error e -> Alcotest.fail (Error.to_string e)
    | Ok store ->
      match Runtime_store.save_artifact_text store "art-sess"
        ~name:"report" ~kind:"markdown" ~content:"# Title" with
      | Ok path ->
        Alcotest.(check bool) "path ends with .md" true
...
let test_save_artifact_text_empty_name () =
  with_temp_dir (fun dir ->
    ...
    | Ok store ->
      match Runtime_store.save_artifact_text store "art-sess"
        ~name:"" ~kind:"json" ~content:"{}" with
      | Ok path ->
        (* Empty name should default to "artifact" *)
        let base = Filename.basename path in
        Alcotest.(check bool) "starts with artifact" true
          (String.length base >= 8
           && String.sub base 0 8 = "artifact")
      | Error e -> Alcotest.fail (Error.to_string e))

(* ── Runner ───────────────────────────────────────────────────── *)

let () =
  Alcotest.run "Runtime Store Unit" [
    "save_load_text", [
