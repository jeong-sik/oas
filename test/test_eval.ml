...
let mk_metric name value =
  { Eval.name; value; unit_ = None; tags = [] }

let mk_run_metrics ?(run_id="r1") ?(agent_name="test") metrics =
  { Eval.run_id; agent_name; timestamp = 0.0;
...
let test_metric_value_to_float () =
  Alcotest.(check (option (float 0.001))) "int"
    (Some 42.0) (Eval.metric_value_to_float (Int_val 42));
  Alcotest.(check (option (float 0.001))) "float"
    (Some 3.14) (Eval.metric_value_to_float (Float_val 3.14));
  Alcotest.(check (option (float 0.001))) "bool true"
    (Some 1.0) (Eval.metric_value_to_float (Bool_val true));
  Alcotest.(check (option (float 0.001))) "string"
    None (Eval.metric_value_to_float (String_val "x"))

(* ── metric tests ─────────────────────────────────────────────── *)

let test_metric_yojson_roundtrip () =
  let m = { Eval.name = "latency"; value = Float_val 1.5;
            unit_ = Some "ms"; tags = [("env", "test")] } in
...
let test_collector_verdict () =
  let c = Eval.create_collector ~agent_name:"a" ~run_id:"r" in
  let v = { Harness.passed = true; score = Some 1.0;
            evidence = []; detail = None } in
...
let test_compare_regression () =
  ...
  let candidate = mk_run_metrics ~run_id:"r2"
    [mk_metric "latency" (Float_val 120.0)] in
...
let test_compare_improvement () =
  let baseline = mk_run_metrics [mk_metric "latency" (Float_val 100.0)] in
  let candidate = mk_run_metrics ~run_id:"r2"
    [mk_metric "latency" (Float_val 80.0)] in
  let cmp = Eval.compare ~baseline ~candidate in
...
let test_compare_unchanged () =
  ...
  let candidate = mk_run_metrics ~run_id:"r2"
    [mk_metric "score" (Float_val 0.96)] in
  let cmp = Eval.compare ~baseline ~candidate in
...
let test_threshold_pass () =
  ...
  let ths = [{ Eval.metric_name = "latency";
               max_value = Some (Float_val 100.0); min_value = None }] in
...
let test_threshold_fail_min () =
  ...
  let ths = [{ Eval.metric_name = "score";
               max_value = None; min_value = Some (Float_val 0.5) }] in
  let v = Eval.check_thresholds rm ths in
  Alcotest.(check bool) "failed" false v.passed
...
let test_find_metric () =
  let rm = mk_run_metrics [
    mk_metric "a" (Int_val 1);
    mk_metric "b" (Int_val 2);
  ] in
  (match Eval.find_metric rm "a" with
   | Some m -> Alcotest.(check string) "found" "a" m.name
   | None -> Alcotest.fail "not found");
  Alcotest.(check bool) "missing" true
    (Option.is_none (Eval.find_metric rm "c"))
...
let test_eval_collector_basic () =
  ...
  Event_bus.publish bus (TurnStarted { agent_name = "test"; turn = 0 });
  Event_bus.publish bus (TurnStarted { agent_name = "test"; turn = 1 });
  Event_bus.publish bus (ToolCalled { agent_name = "test"; tool_name = "t1"; input = `Null });
  Event_bus.publish bus (ToolCompleted { agent_name = "test"; tool_name = "t1";
...
let () =
  Alcotest.run "Eval" [
    ...
    "lookup", [
      Alcotest.test_case "find_metric" `Quick test_find_metric;
    ];
    "serialization", [
