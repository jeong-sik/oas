...
let tmpdir () =
  let dir = Filename.concat
    ...
    (Printf.sprintf "oas_raw_trace_test_%d_%d"
       (Unix.getpid ()) (Random.int 0xFFFF))
  in
...
let test_record_type_to_string () =
  ...
  List.iter (fun (rt, expected) ->
    Alcotest.(check string) (expected ^ " to_string")
      expected (Raw_trace.record_type_to_string rt)
  ) pairs
...
let test_record_type_of_string () =
  ...
  List.iter (fun s ->
    ...
    | Ok rt ->
      Alcotest.(check string) (s ^ " roundtrip")
        s (Raw_trace.record_type_to_string rt)
    | Error _ ->
      Alcotest.fail (Printf.sprintf "valid string %s rejected" s)
  ) valid
...
let test_record_to_json_roundtrip_run_started () =
  ...
  | Ok decoded ->
    ...
    Alcotest.(check bool) "record_type"
      true (decoded.record_type = Raw_trace.Run_started)
...
let test_record_to_json_roundtrip_hook () =
  ...
  | Ok decoded ->
    ...
    Alcotest.(check (option string)) "hook_detail" (Some "file access ok") decoded.hook_detail
...
let test_record_to_json_roundtrip_assistant_block () =
  ...
  | Ok decoded ->
    ...
    Alcotest.(check bool) "assistant_block present" true
      (decoded.assistant_block <> None)
...
let test_read_all_empty_file () =
  ...
  match Raw_trace.read_all ~path () with
  | Ok records ->
    Alcotest.(check int) "empty" 0 (List.length records)
...
let test_read_all_with_records () =
  ...
  let records = [
    mk_record ~seq:1 ();
    mk_record ~record_type:Raw_trace.Run_finished ~seq:2 ~prompt:None
...
let test_read_all_nonexistent_file () =
  ...
  | Ok records ->
    Alcotest.(check int) "empty for missing file" 0 (List.length records)
...
let test_read_all_blank_lines () =
  ...
  match Raw_trace.read_all ~path () with
  | Ok records ->
    Alcotest.(check int) "filters blank lines" 1 (List.length records)
...
let test_record_json_all_fields_populated () =
  ...
  | Ok decoded ->
    ...
    Alcotest.(check (option string)) "tool_result" (Some "result text") decoded.tool_result;
    ...
    Alcotest.(check (option string)) "hook_detail" (Some "failed validation") decoded.hook_detail;
...
let test_record_json_none_fields_omitted () =
  ...
  Alcotest.(check bool) "no tool_name key when None" true
    (not (Util.string_contains ~needle:"tool_name" json_str));
  Alcotest.(check bool) "no hook_name key when None" true
    (not (Util.string_contains ~needle:"hook_name" json_str));
  (* But prompt should appear *)
  Alcotest.(check bool) "prompt present" true
...
let test_summarize_run () =
  ...
  let records = [
    ...
    mk_record ~seq:2 ~record_type:Raw_trace.Assistant_block ~prompt:None
      ~block_index:(Some 0) ~block_kind:(Some "text") ();
    mk_record ~seq:3 ~record_type:Raw_trace.Tool_execution_started ~prompt:None
      ~tool_use_id:(Some "tu-1") ~tool_name:(Some "bash")
      ~tool_input:(Some (`Assoc [("cmd", `String "ls")])) ();
    mk_record ~seq:4 ~record_type:Raw_trace.Tool_execution_finished ~prompt:None
      ~tool_use_id:(Some "tu-1") ~tool_name:(Some "bash")
      ~tool_result:(Some "ok") ~tool_error:(Some false) ();
    mk_record ~seq:5 ~record_type:Raw_trace.Hook_invoked ~prompt:None
      ~hook_name:(Some "pre") ~hook_decision:(Some "allow") ();
    mk_record ~seq:6 ~record_type:Raw_trace.Run_finished ~prompt:None
      ~final_text:(Some "All done") ~stop_reason:(Some "end_turn") ();
  ] in
  write_jsonl path records;
  ...
  | Ok summary ->
    ...
    Alcotest.(check bool) "bash in tool_names" true
      (List.mem "bash" summary.tool_names);
    Alcotest.(check bool) "pre in hook_names" true
      (List.mem "pre" summary.hook_names)
  | Error err ->
...
let test_validate_run_pass () =
  ...
  let records = [
    ...
    mk_record ~seq:2 ~record_type:Raw_trace.Tool_execution_started ~prompt:None
      ~tool_use_id:(Some "tu-1") ~tool_name:(Some "read") ~tool_input:(Some `Null) ();
    mk_record ~seq:3 ~record_type:Raw_trace.Tool_execution_finished ~prompt:None
      ~tool_use_id:(Some "tu-1") ~tool_name:(Some "read")
      ~tool_result:(Some "content") ~tool_error:(Some false) ();
    mk_record ~seq:4 ~record_type:Raw_trace.Run_finished ~prompt:None
      ~final_text:(Some "OK") ~stop_reason:(Some "end_turn") ();
  ] in
  write_jsonl path records;
  let run_ref : Raw_trace.run_ref = {
    worker_run_id = "wr-test-0001";
  ...
  | Ok validation ->
    ...
    Alcotest.(check int) "paired_tool_result_count" 1
      validation.paired_tool_result_count;
    Alcotest.(check bool) "has_file_write" false validation.has_file_write;
    List.iter (fun (check : Raw_trace.validation_check) ->
      Alcotest.(check bool) (Printf.sprintf "check %s" check.name)
        true check.passed
    ) validation.checks
  | Error err ->
...
let test_validate_run_no_finished () =
  ...
  | Ok validation ->
    ...
    let run_finished_check =
      List.find (fun (c : Raw_trace.validation_check) -> c.name = "run_finished")
        validation.checks
    in
...
let test_validate_run_unmatched_tool_pairs () =
  ...
  let records = [
    ...
    mk_record ~seq:3 ~record_type:Raw_trace.Run_finished ~prompt:None
      ~final_text:(Some "partial") ~stop_reason:(Some "error") ();
  ] in
  write_jsonl path records;
  let run_ref : Raw_trace.run_ref = {
    worker_run_id = "wr-test-0001";
  ...
  | Ok validation ->
    ...
    let tool_pairs_check =
      List.find (fun (c : Raw_trace.validation_check) -> c.name = "tool_pairs")
        validation.checks
    in
...
let test_read_runs () =
  ...
  let records = [
    ...
    { (mk_record ~seq:2 ~record_type:Raw_trace.Run_finished ~prompt:None
         ~final_text:(Some "a") ()) with worker_run_id = "wr-A" };
    { (mk_record ~seq:3 ()) with worker_run_id = "wr-B" };
    { (mk_record ~seq:4 ~record_type:Raw_trace.Run_finished ~prompt:None
         ~final_text:(Some "b") ()) with worker_run_id = "wr-B" };
  ] in
  write_jsonl path records;
...
let () =
  Alcotest.run "Raw_trace_deep" [
    ...
    "safe_name", [
      Alcotest.test_case "sanitization" `Quick test_safe_name;
    ];
    "parse_json_via_read_all", [
      Alcotest.test_case "invalid json" `Quick test_read_all_invalid_json;
    ];
    "record_json", [
      Alcotest.test_case "run_started roundtrip" `Quick test_record_to_json_roundtrip_run_started;
      Alcotest.test_case "tool_exec roundtrip" `Quick test_record_to_json_roundtrip_tool_exec;
      Alcotest.test_case "hook roundtrip" `Quick test_record_to_json_roundtrip_hook;
      Alcotest.test_case "assistant_block roundtrip" `Quick test_record_to_json_roundtrip_assistant_block;
      Alcotest.test_case "run_finished roundtrip" `Quick test_record_to_json_roundtrip_run_finished;
      Alcotest.test_case "all None optionals" `Quick test_record_to_json_all_none_optionals;
    ...
    "record_json_edge", [
      Alcotest.test_case "all fields populated" `Quick test_record_json_all_fields_populated;
      Alcotest.test_case "None fields omitted" `Quick test_record_json_none_fields_omitted;
    ];
    "summarize_run", [
      Alcotest.test_case "full trace" `Quick test_summarize_run;
    ];
    "validate_run", [
      ...
      Alcotest.test_case "unmatched tool pairs" `Quick test_validate_run_unmatched_tool_pairs;
    ];
    "read_runs", [
      Alcotest.test_case "multi-run file" `Quick test_read_runs;
    ];
  ]
