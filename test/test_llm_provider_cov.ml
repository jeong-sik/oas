...
let test_parse_model_string_empty () =
  Alcotest.(check bool) "empty -> None" true (Cascade_config.parse_model_string "" = None)
;;
...
let test_text_of_response_text_concat () =
  ...
  Alcotest.(check string) "concat" "hello world" (Cascade_config.text_of_response resp)
;;
...
let test_load_profile_nonexistent () =
  Eio_main.run (fun _env ->
    let result =
      Cascade_config.load_profile ~config_path:"/nonexistent/path.json" ~name:"test"
    in
...
let test_load_profile_non_string_items () =
  with_cascade_temp
    {|{"mix_models": ["llama:qwen3.5", 42, true, "glm:auto"]}|}
    (fun path ->
       Eio_main.run (fun _env ->
...
let test_build_request_with_system_prompt () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~system_prompt:"be helpful" ()
  in
  let body_str = Backend_gemini.build_request ~config ~messages:[ user_msg "hello" ] () in
  let json = Yojson.Safe.from_string body_str in
  let open Yojson.Safe.Util in
...
let test_build_request_with_thinking_default_budget () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~enable_thinking:true ()
  in
  let body_str =
...
let test_build_request_json_mode () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~response_format_json:true ()
  in
  let body_str =
...
let test_build_request_tool_choice_auto () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~tool_choice:Auto ()
  in
  let body_str =
    Backend_gemini.build_request
      ~config
...
let test_build_request_tool_choice_any () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~tool_choice:Any ()
  in
  let body_str =
    Backend_gemini.build_request
      ~config
...
let test_build_request_tool_choice_none () =
  let config =
    make_config ~kind:Gemini ~model_id:"gemini-2.5-flash" ~tool_choice:None_ ()
  in
    ...
    ; ( "cascade_config.parse_model_string"
      , [ Alcotest.test_case "empty" `Quick test_parse_model_string_empty
        ...
        ; Alcotest.test_case
            "unknown provider"
            `Quick
            test_parse_model_string_unknown_provider
        ; Alcotest.test_case "llama" `Quick test_parse_model_string_llama
        ; Alcotest.test_case
            "custom with url"
            `Quick
            test_parse_model_string_custom_with_url
        ; Alcotest.test_case
            "custom empty model"
            `Quick
            test_parse_model_string_custom_empty_model
        ...
        ; Alcotest.test_case
            "case insensitive"
            `Quick
            test_parse_model_string_case_insensitive
        ] )
    ...
    ; ( "cascade_config.text_of_response"
      , [ Alcotest.test_case "empty" `Quick test_text_of_response_empty
        ...
        ; Alcotest.test_case
            "non-text ignored"
            `Quick
            test_text_of_response_non_text_ignored
        ] )
