...
let make_tool name =
  Tool.create ~name ~description:name ~parameters:[]
    (fun _ -> Ok { Types.content = name })
    ...
    [
      ...
      ( "prepare_tools integration",
        [
          ...
          test_case "channel overrides operator" `Quick test_channel_overrides_operator_policy;
          test_case "multiple updates compose" `Quick test_multiple_updates_compose_correctly;
