...
let test_trailing_comma_object () =
  ...
  let result = Lenient_json.parse input in
  check json "trailing comma object" (`Assoc [ "a", `Int 1; "b", `Int 2 ]) result
;;

let test_trailing_comma_array () =
  let input = "[1, 2, 3,]" in
  let result = Lenient_json.parse input in
  check json "trailing comma array" (`List [ `Int 1; `Int 2; `Int 3 ]) result
;;
...
let test_trailing_comma_nested () =
  ...
  check
    ...
    (`Assoc [ "items", `List [ `Int 1; `Int 2 ]; "x", `Int 3 ])
    result
;;
...
let test_trailing_comma_in_string () =
  ...
  let result = Lenient_json.parse input in
  check json "comma in string preserved" (`Assoc [ "msg", `String "a, b," ]) result
;;
...
let test_complete_true () =
  ...
  let result = Lenient_json.parse input in
  check json "complete true" (`Assoc [ "flag", `Bool true ]) result
;;

let test_complete_false () =
  let input = {|{"flag": fals|} in
  let result = Lenient_json.parse input in
  check json "complete false" (`Assoc [ "flag", `Bool false ]) result
;;

let test_complete_null () =
  let input = {|{"val": nul|} in
  let result = Lenient_json.parse input in
  check json "complete null" (`Assoc [ "val", `Null ]) result
;;
...
let test_unclosed_brace () =
  let input = {|{"room": "lobby"|} in
  let result = Lenient_json.parse input in
  check json "unclosed brace" (`Assoc [ "room", `String "lobby" ]) result
;;

let test_unclosed_bracket () =
  let input = {|[1, 2, 3|} in
  let result = Lenient_json.parse input in
  check json "unclosed bracket" (`List [ `Int 1; `Int 2; `Int 3 ]) result
;;

let test_unclosed_nested () =
  let input = {|{"items": [1, 2|} in
  let result = Lenient_json.parse input in
  check json "unclosed nested" (`Assoc [ "items", `List [ `Int 1; `Int 2 ] ]) result
;;

let test_close_brackets_unit () =
  check string "close brace" {|{"a": 1}|} (Lenient_json.close_brackets {|{"a": 1|})
;;
...
let test_fence_plus_trailing_comma () =
  let input = "```json\n{\"a\": 1,}\n```" in
  let result = Lenient_json.parse input in
  check json "fence + trailing comma" (`Assoc [ "a", `Int 1 ]) result
;;

let test_fence_plus_unclosed () =
  let input = "```json\n{\"room\": \"test\"\n```" in
  let result = Lenient_json.parse input in
  check json "fence + unclosed" (`Assoc [ "room", `String "test" ]) result
;;

let test_trailing_comma_plus_unclosed () =
  let input = {|{"a": 1, "b": 2,|} in
  let result = Lenient_json.parse input in
  check json "trailing comma + unclosed" (`Assoc [ "a", `Int 1; "b", `Int 2 ]) result
;;
...
let test_total_garbage () =
  ...
  let result = Lenient_json.parse input in
  check json "garbage fallback" (`Assoc [ "raw", `String input ]) result
;;

let test_empty_string () =
  let result = Lenient_json.parse "" in
  check json "empty fallback" (`Assoc [ "raw", `String "" ]) result
;;
...
let test_api_common_uses_lenient () =
  ...
  check json "api_common lenient" (`Assoc [ "key", `String "val" ]) result
;;
