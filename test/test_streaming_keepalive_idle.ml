...
let test_keepalive_lines_skipped_between_real_events () =
  ...
  check
    ...
    "events match (keepalives skipped)" expected calls

let test_keepalive_only_stream_produces_no_events () =
  let payload = ": ka1\n: ka2\n: ka3\n: ka4\n" in
  let calls = collect_sse_events payload in
  check int "keepalive-only stream produces no on_data calls" 0
    (List.length calls)
...
let () =
  run "SSE keepalive idle timer"
    [
      ( "keepalive_skip",
        [
          test_case "events delivered, keepalives skipped" `Quick
            test_keepalive_lines_skipped_between_real_events;
          test_case "keepalive-only stream produces no events" `Quick
            test_keepalive_only_stream_produces_no_events;
          test_case "bare colon line treated as keepalive" `Quick
