...
let test_empty_pipeline_approves () =
  ...
  let decision = Approval.evaluate pipeline
    ~tool_name:"test" ~input:(`Assoc [])
    ~agent_name:"agent" ~turn:1 in
  match decision with
  | Hooks.Approve -> ()
...
let test_auto_approve_known () =
  let pipeline = Approval.create [
    Approval.auto_approve_known_tools ["safe_tool"; "read_file"]
  ] in
  let d1 = Approval.evaluate pipeline
    ~tool_name:"safe_tool" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  (match d1 with Hooks.Approve -> () | _ -> fail "should approve known tool");
  (* Unknown tool passes through -> default Approve *)
  let d2 = Approval.evaluate pipeline
    ~tool_name:"unknown" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  match d2 with Hooks.Approve -> () | _ -> fail "should approve (pass-through)"

(* ── Reject dangerous patterns ──────────────────────────── *)

let test_reject_dangerous () =
  let pipeline = Approval.create [
    Approval.reject_dangerous_patterns [("delete", ""); ("exec", "rm")]
  ] in
  let d1 = Approval.evaluate pipeline
    ~tool_name:"delete_file" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  (match d1 with Hooks.Reject _ -> () | _ -> fail "should reject delete");
  let d2 = Approval.evaluate pipeline
    ~tool_name:"exec_cmd" ~input:(`Assoc [("cmd", `String "rm -rf /")])
    ~agent_name:"a" ~turn:0 in
  (match d2 with Hooks.Reject _ -> () | _ -> fail "should reject exec+rm");
  let d3 = Approval.evaluate pipeline
    ~tool_name:"exec_cmd" ~input:(`Assoc [("cmd", `String "ls")])
    ~agent_name:"a" ~turn:0 in
  match d3 with Hooks.Approve -> () | _ -> fail "should approve exec+ls"
...
let test_multi_stage () =
  ...
  let d1 = Approval.evaluate pipeline
    ~tool_name:"safe" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  (match d1 with Hooks.Approve -> () | _ -> fail "should approve safe");
  (* Stage 2 rejects dangerous *)
  let d2 = Approval.evaluate pipeline
    ~tool_name:"danger_tool" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  (match d2 with Hooks.Reject _ -> () | _ -> fail "should reject danger");
  (* Stage 3 catches everything else *)
  let d3 = Approval.evaluate pipeline
    ~tool_name:"other" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  match d3 with Hooks.Approve -> () | _ -> fail "should approve other"
...
let test_always_reject () =
  let pipeline = Approval.create [
    Approval.always_reject "locked down"
  ] in
  let d = Approval.evaluate pipeline
    ~tool_name:"anything" ~input:(`Assoc [])
    ~agent_name:"a" ~turn:0 in
  match d with
...
let test_as_callback () =
  let pipeline = Approval.create [
    Approval.auto_approve_known_tools ["ok"]
  ] in
...
let () =
  run "Approval_pipeline" [
    ...
    "compat", [
      test_case "as_callback" `Quick test_as_callback;
    ];
  ]
