...
let test_judgment_roundtrip () =
  ...
  | Ok j2 ->
    ...
    Alcotest.(check int)
      "evidence count"
      (List.length j.evidence)
      (List.length j2.evidence);
...
let test_risk_level_roundtrip () =
  ...
  List.iter
    (fun r ->
       ...
       | Ok r2 -> Alcotest.check risk_testable "risk level" r r2)
    levels
;;
...
let test_parse_risk_string_variants () =
  ...
  in
  List.iter
    (fun (json, expected_risk) ->
       match Judge.parse_judgment json with
       | Error e ->
         Alcotest.fail (Printf.sprintf "Expected Ok for %s, got Error: %s" json e)
       | Ok j ->
         Alcotest.check
           risk_testable
           (Printf.sprintf "risk for input")
           expected_risk
           j.risk)
    cases
;;
  ...
  Alcotest.run
    ...
    ; ( "serialization"
      , [ Alcotest.test_case "judgment roundtrip" `Quick test_judgment_roundtrip
        ; Alcotest.test_case
            "judgment roundtrip no action"
            `Quick
            test_judgment_roundtrip_no_action
