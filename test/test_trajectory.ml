...
let test_basic_trajectory () =
  let records = [
    ...
    make_record ~seq:3 ~ts:100.2 ~agent_name:"test-agent"
      ~record_type:Assistant_block ~block_kind:"text"
      ~assistant_block:(`Assoc [("text", `String "answer here")]) ();
    make_record ~seq:4 ~ts:100.3 ~agent_name:"test-agent"
      ~record_type:Run_finished ~final_text:"answer here" ();
  ] in
...
let test_tool_call_pairing () =
  let records = [
    make_record ~seq:1 ~ts:200.0 ~agent_name:"tool-agent"
      ~record_type:Run_started ~prompt:"use a tool" ();
    make_record ~seq:2 ~ts:200.1 ~agent_name:"tool-agent"
      ~record_type:Tool_execution_started
      ~tool_use_id:"tu-1" ~tool_name:"read_file"
      ~tool_input:(`Assoc [("path", `String "/foo")]) ();
    make_record ~seq:3 ~ts:200.5 ~agent_name:"tool-agent"
      ~record_type:Tool_execution_finished
      ~tool_use_id:"tu-1" ~tool_name:"read_file"
      ~tool_result:"file contents here" ~tool_error:false ();
    make_record ~seq:4 ~ts:200.6 ~agent_name:"tool-agent"
      ~record_type:Run_finished ~final_text:"done" ();
  ] in
  let traj = Trajectory.of_raw_trace_records records in
  ...
  let act_step = List.find (function Trajectory.Act _ -> true | _ -> false) traj.steps in
  (match act_step with
   | Trajectory.Act { tool_call; _ } ->
     ...
     Alcotest.(check (option string)) "tool result" (Some "file contents here")
...
let test_error_run () =
  let records = [
    make_record ~seq:1 ~ts:300.0 ~agent_name:"fail-agent"
      ~record_type:Run_started ~prompt:"do something" ();
    make_record ~seq:2 ~ts:300.5 ~agent_name:"fail-agent"
      ~record_type:Run_finished ~error:"something went wrong" ();
  ] in
  let traj = Trajectory.of_raw_trace_records records in
...
let test_orphan_tool_finish () =
  ...
  let records = [
    make_record ~seq:1 ~ts:400.0 ~agent_name:"orphan-agent"
      ~record_type:Run_started ~prompt:"test" ();
    make_record ~seq:2 ~ts:400.3 ~agent_name:"orphan-agent"
      ~record_type:Tool_execution_finished
      ~tool_use_id:"tu-orphan" ~tool_name:"bash"
      ~tool_result:"ok" ~tool_error:false ();
    make_record ~seq:3 ~ts:400.4 ~agent_name:"orphan-agent"
...
let test_unfinished_tool () =
  ...
  let records = [
    make_record ~seq:1 ~ts:500.0 ~agent_name:"pending-agent"
      ~record_type:Run_started ~prompt:"test" ();
    make_record ~seq:2 ~ts:500.1 ~agent_name:"pending-agent"
      ~record_type:Tool_execution_started
      ~tool_use_id:"tu-pending" ~tool_name:"long_op"
      ~tool_input:(`Assoc []) ();
    make_record ~seq:3 ~ts:500.5 ~agent_name:"pending-agent"
      ~record_type:Run_finished ();
  ] in
  let traj = Trajectory.of_raw_trace_records records in
  let (_think, act, _obs, _respond) = Trajectory.count_steps traj in
  ...
  (match act_step with
   | Trajectory.Act { tool_call; _ } ->
     ...
     Alcotest.(check bool) "no finished_at" true
       (Option.is_none tool_call.finished_at);
   | _ -> Alcotest.fail "expected Act step")
...
let test_json_roundtrip () =
  let records = [
    make_record ~seq:1 ~ts:600.0 ~agent_name:"json-agent"
      ~record_type:Run_started ~prompt:"roundtrip test" ();
    make_record ~seq:2 ~ts:600.1 ~agent_name:"json-agent"
      ~record_type:Assistant_block ~block_kind:"thinking"
      ~assistant_block:(`Assoc [("content", `String "hmm")]) ();
    make_record ~seq:3 ~ts:600.2 ~agent_name:"json-agent"
      ~record_type:Tool_execution_started
      ~tool_use_id:"tu-rt" ~tool_name:"search"
      ~tool_input:(`Assoc [("q", `String "test")]) ();
    make_record ~seq:4 ~ts:600.3 ~agent_name:"json-agent"
      ~record_type:Tool_execution_finished
      ~tool_use_id:"tu-rt" ~tool_name:"search"
      ~tool_result:"found it" ~tool_error:false ();
    make_record ~seq:5 ~ts:600.4 ~agent_name:"json-agent"
      ~record_type:Assistant_block ~block_kind:"text"
      ~assistant_block:(`Assoc [("text", `String "result is here")]) ();
    make_record ~seq:6 ~ts:600.5 ~agent_name:"json-agent"
      ~record_type:Run_finished ~final_text:"result is here" ();
  ] in
  let traj = Trajectory.of_raw_trace_records records in
...
let test_step_json_roundtrip () =
  ...
  List.iter (fun step ->
    ...
    | Ok step2 ->
      ...
      Alcotest.(check (float 0.001)) "ts roundtrip"
        (Trajectory.step_ts step) (Trajectory.step_ts step2)
  ) steps
...
let test_count_steps_empty () =
  let traj : Trajectory.trajectory = {
    agent_name = "empty"; model = "m"; prompt = "";
    steps = []; started_at = 0.0; finished_at = None;
    success = true; metrics = None; error = None;
  } in
...
let test_elapsed_s () =
  let traj : Trajectory.trajectory = {
    agent_name = "t"; model = "m"; prompt = "";
    steps = []; started_at = 100.0; finished_at = Some 105.5;
    success = true; metrics = None; error = None;
  } in
  Alcotest.(check (option (float 0.001))) "elapsed"
    (Some 5.5) (Trajectory.elapsed_s traj)

let test_elapsed_s_none () =
  let traj : Trajectory.trajectory = {
    agent_name = "t"; model = "m"; prompt = "";
    steps = []; started_at = 100.0; finished_at = None;
    success = true; metrics = None; error = None;
  } in
  Alcotest.(check (option (float 0.001))) "no elapsed"
    None (Trajectory.elapsed_s traj)
...
let test_sandbox_basic () =
  ...
  let result = Sandbox_runner.run
    ~config ~agent_name:"sandbox-test" ~model:"mock"
    ~prompt:"test prompt" ~run_fn in
  ...
  List.iter (fun (v : Harness.verdict) ->
    Alcotest.(check bool) "verdict passed" true v.passed
  ) result.verdicts
...
let test_sandbox_max_turns () =
  ...
  (* First call succeeds *)
  let result = Sandbox_runner.run
    ~config ~agent_name:"limit-test" ~model:"mock"
    ~prompt:"test" ~run_fn in
...
let test_sandbox_tool_counting () =
  let run_fn _prompt =
    Ok { Types.id = "msg-tc"; model = "mock"; stop_reason = EndTurn;
         content = [
  ...
  let result = Sandbox_runner.run
    ~config ~agent_name:"tool-count-test" ~model:"mock"
    ~prompt:"run tools" ~run_fn in
  (* 2 tool_use blocks counted *)
  let tool_metric = List.find_opt (fun (m : Eval.metric) ->
    m.name = "tool_calls"
  ) result.metrics.metrics in
  (match tool_metric with
   | Some { value = Int_val n; _ } ->
     Alcotest.(check int) "tool calls counted" 2 n
   | _ -> Alcotest.fail "tool_calls metric not found")

let test_sandbox_trajectory_capture () =
  let run_fn _prompt =
    Ok (mock_response "captured response")
  in
  let config : Sandbox_runner.sandbox_config = {
    timeout_s = 10.0;
    max_turns = 5;
  ...
  let result = Sandbox_runner.run
    ~config ~agent_name:"capture-test" ~model:"mock"
    ~prompt:"capture me" ~run_fn in
  ...
  Alcotest.(check string) "trajectory agent" "capture-test"
    result.trajectory.agent_name

let test_sandbox_no_capture () =
  let run_fn _prompt =
    Ok (mock_response "not captured")
  in
  let config : Sandbox_runner.sandbox_config = {
    timeout_s = 10.0;
    max_turns = 5;
  ...
  let result = Sandbox_runner.run
    ~config ~agent_name:"no-capture" ~model:"mock"
    ~prompt:"silent" ~run_fn in
...
let test_sandbox_error_run () =
  let run_fn _prompt =
    Error (Error.Internal "mock failure")
  in
  let config = Sandbox_runner.default_config in
  let result = Sandbox_runner.run
    ~config ~agent_name:"error-test" ~model:"mock"
    ~prompt:"fail" ~run_fn in
...
let () =
  Alcotest.run "trajectory" [
    ("trajectory", [
      Alcotest.test_case "basic trajectory from records" `Quick
        test_basic_trajectory;
      Alcotest.test_case "tool call pairing" `Quick
        test_tool_call_pairing;
      Alcotest.test_case "error run" `Quick
        test_error_run;
      Alcotest.test_case "orphan tool finish" `Quick
        test_orphan_tool_finish;
      Alcotest.test_case "unfinished tool" `Quick
        test_unfinished_tool;
    ]);
    ("json", [
      Alcotest.test_case "trajectory json roundtrip" `Quick
        test_json_roundtrip;
      Alcotest.test_case "step json roundtrip" `Quick
        test_step_json_roundtrip;
    ]);
    ("analysis", [
      Alcotest.test_case "count_steps empty" `Quick
        test_count_steps_empty;
      Alcotest.test_case "elapsed_s" `Quick
        test_elapsed_s;
      Alcotest.test_case "elapsed_s none" `Quick
        test_elapsed_s_none;
    ]);
    ("sandbox", [
      Alcotest.test_case "basic sandbox run" `Quick
        test_sandbox_basic;
      Alcotest.test_case "max turns" `Quick
        test_sandbox_max_turns;
      Alcotest.test_case "tool counting" `Quick
        test_sandbox_tool_counting;
      Alcotest.test_case "trajectory capture" `Quick
        test_sandbox_trajectory_capture;
      Alcotest.test_case "no capture" `Quick
        test_sandbox_no_capture;
      Alcotest.test_case "error run" `Quick
        test_sandbox_error_run;
    ]);
  ]
