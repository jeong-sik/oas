...
let test_discover_unreachable () =
  ...
  let reg = Agent_registry.create () in
  match Agent_registry.discover_and_register ~sw ~net reg
    ~name:"remote" ~url:"http://127.0.0.1:19999" with
  | Error _ -> ()  (* expected *)
...
let test_overwrite () =
  ...
  | Some entry ->
    check string "updated" "v2" (Agent_registry.card_of_entry entry).name
  | None -> fail "not found"
