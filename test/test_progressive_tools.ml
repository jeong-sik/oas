...
let test_phase_based_single_phase () =
  let strategy = Progressive_tools.Phase_based { phases = [ 0, [ "read"; "search" ] ] } in
  let tools = Progressive_tools.tools_for_turn strategy 0 () in
...
let test_phase_based_multi_phase () =
  let strategy =
    Progressive_tools.Phase_based
      { phases =
          [ 0, [ "read" ]; 3, [ "read"; "write" ]; 6, [ "read"; "write"; "deploy" ] ]
      }
  in
...
let test_phase_based_below_first_threshold () =
  let strategy = Progressive_tools.Phase_based { phases = [ 5, [ "a"; "b" ] ] } in
...
let test_gav_gather_phase () =
  let strategy =
    Progressive_tools.Gather_act_verify
...
let test_gav_act_phase () =
  let strategy =
    Progressive_tools.Gather_act_verify
      { gather_tools = [ "search" ]; act_tools = [ "write" ]; verify_tools = [ "test" ] }
  in
  let t3 = Progressive_tools.tools_for_turn strategy 3 () in
...
let test_gav_boundary_turns () =
  let strategy =
    Progressive_tools.Gather_act_verify
      { gather_tools = [ "g" ]; act_tools = [ "a" ]; verify_tools = [ "v" ] }
  in
  (* turn <= 2 = gather *)
...
let test_as_hook_returns_adjust_params () =
  let strategy = Progressive_tools.Phase_based { phases = [ 0, [ "tool_a" ] ] } in
...
let test_builder_wires_to_before_turn_params () =
  let strategy =
    Progressive_tools.Phase_based { phases = [ 0, [ "tool_a"; "tool_b" ] ] }
  in
  Eio_main.run
  @@ fun env ->
  ...
  check_bool "before_turn_params is Some" true (Option.is_some hooks.before_turn_params);
...
let test_builder_composes_with_existing_btp () =
  ...
  let existing_btp : Hooks.hook =
    fun event ->
    match event with
    | Hooks.BeforeTurnParams _ ->
      Hooks.AdjustParams { Hooks.default_turn_params with temperature = Some custom_temp }
    | _ -> Hooks.Continue
  in
  Eio_main.run
  @@ fun env ->
  ...
  Alcotest.run
    ...
    [ ( "phase_based"
      , [ Alcotest.test_case "empty phases" `Quick test_phase_based_empty_phases
        ...
        ; Alcotest.test_case
            "below threshold"
            `Quick
            test_phase_based_below_first_threshold
        ] )
    ; ( "gather_act_verify"
    ...
    ; ( "as_hook"
      , [ Alcotest.test_case
            ...
            `Quick
            test_as_hook_returns_adjust_params
        ; Alcotest.test_case
            "non-BeforeTurnParams"
            `Quick
            test_as_hook_non_before_turn_params
        ] )
    ; ( "builder_integration"
      , [ Alcotest.test_case
            "wires to before_turn_params"
            `Quick
            test_builder_wires_to_before_turn_params
        ; Alcotest.test_case
            "does not pollute pre_tool_use"
            `Quick
            test_builder_does_not_pollute_pre_tool_use
        ; Alcotest.test_case
            "composes with existing btp"
            `Quick
