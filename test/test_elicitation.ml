...
let test_elicitation_request_with_schema () =
  ...
  Alcotest.(check bool) "has timeout" true
    (match req.timeout_s with Some 30.0 -> true | _ -> false)

let test_elicitation_response_answer () =
  let resp = Hooks.Answer (`String "production") in
  (match resp with
   | Hooks.Answer (`String s) ->
     Alcotest.(check string) "answer" "production" s
...
let test_hook_decision_elicit_input () =
  ...
  (match decision with
   | Hooks.ElicitInput r ->
     Alcotest.(check string) "question" "Confirm?" r.question
...
let test_elicitation_callback_answer () =
  let cb : Hooks.elicitation_callback = fun req ->
    Hooks.Answer (`String (Printf.sprintf "answered: %s" req.question))
  in
  let req : Hooks.elicitation_request = {
...
let test_agent_options_elicitation_default () =
  ...
  Alcotest.(check bool) "default none" true
    (Option.is_none opts.elicitation)
