...
let make_policy_with_audit () =
  ...
  let budget_limit = Policy.{
    name = "budget-limit";
    priority = 60;
    applies_to = (fun dp ->
      match dp with ResourceRequest _ -> true | _ -> false);
    evaluate = (fun dp ->
      match dp with
...
let test_policy_decisions_recorded_in_audit () =
  ...
  (* Allowed tool call *)
  let v1 = evaluate_and_record policy audit ~agent_name:"alice" ~action:"search"
    (BeforeToolCall { tool_name = "search"; agent_name = "alice" }) in
  (match v1 with Allow -> () | v -> fail (Policy.verdict_to_string v));
  (* Denied tool call *)
  let v2 = evaluate_and_record policy audit ~agent_name:"bob" ~action:"delete"
    (BeforeToolCall { tool_name = "delete"; agent_name = "bob" }) in
  (match v2 with Deny _ -> () | v -> fail (Policy.verdict_to_string v));
  (* Audit has both *)
...
let test_all_verdict_types_in_audit () =
  let policy, audit = make_policy_with_audit () in
  (* Allow *)
  let _ = evaluate_and_record policy audit ~agent_name:"a" ~action:"read"
    (BeforeToolCall { tool_name = "read"; agent_name = "a" }) in
  (* Deny *)
  let _ = evaluate_and_record policy audit ~agent_name:"a" ~action:"del"
    (BeforeToolCall { tool_name = "delete"; agent_name = "a" }) in
  (* Escalate *)
  let _ = evaluate_and_record policy audit ~agent_name:"a" ~action:"handoff"
    (BeforeHandoff { from_agent = "a"; to_agent = "b" }) in
  (* AllowWithCondition *)
  let _ = evaluate_and_record policy audit ~agent_name:"a" ~action:"resource"
    (ResourceRequest { agent_name = "a"; resource = "gpu"; amount = 5.0 }) in
...
let test_audit_query_filters_denied_actions () =
  ...
  for i = 1 to 5 do
    ...
    let _ = evaluate_and_record policy audit
      ~agent_name:(Printf.sprintf "agent-%d" i)
      ~action:"tool_call"
      (BeforeToolCall { tool_name = tool; agent_name = Printf.sprintf "agent-%d" i }) in
    ()
  done;
  check int "5 total" 5 (Audit.count audit);
  (* Count denied *)
  let all = Audit.query audit () in
  let denied = List.filter (fun (e : Audit.entry) ->
    match e.verdict with Some (Deny _) -> true | _ -> false) all in
...
let test_durable_steps_audited () =
  ...
  let step1 : Durable.step = {
    ...
    execute = (fun input ->
      Audit.record audit {
        id = "dur-validate"; timestamp = Unix.gettimeofday ();
        agent_name = "pipeline"; action = "step_execute";
        decision_point = None; verdict = None;
  ...
  let step2 : Durable.step = {
    ...
    execute = (fun input ->
      Audit.record audit {
        id = "dur-transform"; timestamp = Unix.gettimeofday ();
        agent_name = "pipeline"; action = "step_execute";
        decision_point = None; verdict = None;
  ...
  let sm = Durable.create ~name:"audited-pipeline" ()
    |> fun sm -> Durable.add_step sm step1
    |> fun sm -> Durable.add_step sm step2
  in
...
let test_durable_failure_audited () =
  ...
  let good_step : Durable.step = {
    name = "good"; retry_limit = 1;
    execute = (fun input ->
      Audit.record audit {
        id = "good"; timestamp = Unix.gettimeofday ();
        agent_name = "pipeline"; action = "step_pass";
        decision_point = None; verdict = None; detail = `Null;
      };
      Ok input);
  } in
  let bad_step : Durable.step = {
    name = "bad"; retry_limit = 1;
    execute = (fun _ ->
      Audit.record audit {
        id = "bad"; timestamp = Unix.gettimeofday ();
        agent_name = "pipeline"; action = "step_fail";
        decision_point = None; verdict = None;
        detail = `String "about to fail";
      };
      Error "intentional failure");
  } in
  let sm = Durable.create ~name:"fail-pipeline" ()
    |> fun sm -> Durable.add_step sm good_step
    |> fun sm -> Durable.add_step sm bad_step
  in
...
let test_full_governance_flow () =
  ...
  let verdict = evaluate_and_record policy audit ~agent_name:"etl" ~action:"pre-check" dp in
  (match verdict with Allow -> () | v -> fail (Policy.verdict_to_string v));
  (* Step 2: Run durable pipeline *)
  let pipeline = Durable.create ~name:"etl-pipeline" ()
    |> fun sm -> Durable.add_step sm {
      name = "extract"; retry_limit = 1;
      execute = (fun _ -> Ok (`Assoc [("rows", `Int 100)]));
    }
    |> fun sm -> Durable.add_step sm {
      name = "transform"; retry_limit = 2;
      execute = (fun input ->
        let open Yojson.Safe.Util in
    ...
    |> fun sm -> Durable.add_step sm {
      name = "load"; retry_limit = 1;
      execute = (fun input -> Ok input);
    }
  in
  ...
  (match state with
   | Completed { journal; _ } ->
     Audit.record audit {
       id = "etl-done"; timestamp = Unix.gettimeofday ();
       agent_name = "etl"; action = "pipeline_complete";
       decision_point = None; verdict = None;
...
let test_policy_blocks_pipeline () =
  ...
  let verdict = evaluate_and_record policy audit ~agent_name:"cleaner" ~action:"pre-check" dp in
  (match verdict with
   | Deny _ ->
...
let test_durable_serialization_preserves_governance_state () =
  ...
  let sm = Durable.create ~name:"gov-pipeline" ()
    |> fun sm -> Durable.add_step sm {
      name = "step1"; retry_limit = 1;
      execute = (fun _ -> Ok (`Int 42));
    }
  in
...
let test_dynamic_rule_addition_reflected_in_audit () =
  ...
  Audit.record audit {
    id = "before-rule"; timestamp = 1.0;
    agent_name = "ci"; action = "deploy";
    decision_point = Some dp; verdict = Some v1; detail = `Null;
  };
  (match v1 with Allow -> () | _ -> fail "expected Allow before rule");
  (* Add deny rule dynamically *)
  let policy = Policy.add_rule policy Policy.{
    name = "freeze-deploys";
    priority = 100;
    applies_to = (fun dp ->
      match dp with
  ...
  Audit.record audit {
    id = "after-rule"; timestamp = 2.0;
    agent_name = "ci"; action = "deploy";
    decision_point = Some dp; verdict = Some v2; detail = `Null;
  };
  (match v2 with Deny _ -> () | _ -> fail "expected Deny after rule");
  (* Audit shows the transition *)
  check int "2 entries" 2 (Audit.count audit);
  let entries = Audit.query audit ~agent:"ci" () in
  let verdicts = List.map (fun (e : Audit.entry) ->
    match e.verdict with
    | Some v -> Policy.verdict_to_string v
    | None -> "none") entries in
  check bool "contains Allow" true (List.mem "Allow" verdicts);
  check bool "contains Deny" true
    (List.exists (fun s -> String.length s > 4 && String.sub s 0 4 = "Deny") verdicts)

(* ── Suite ────────────────────────────────────────── *)

let () =
  run "governance-integration" [
    "policy+audit", [
      test_case "decisions recorded in audit" `Quick
        test_policy_decisions_recorded_in_audit;
      test_case "all verdict types in audit" `Quick
        test_all_verdict_types_in_audit;
      test_case "query filters denied actions" `Quick
        test_audit_query_filters_denied_actions;
      test_case "dynamic rule reflected in audit" `Quick
    ...
    "full-governance", [
      test_case "policy gate -> pipeline -> audit" `Quick
        test_full_governance_flow;
      test_case "policy blocks pipeline" `Quick
        test_policy_blocks_pipeline;
      test_case "serialization preserves state" `Quick
