...
let test_dynamic_source () =
  let config : Append_instruction.config =
    { sources =
        [ Dynamic (fun turn -> if turn >= 3 then Some "Late-game instruction" else None) ]
    }
  in
  check
    bool
...
let test_from_context () =
  ...
  let config2 : Append_instruction.config = { sources = [ FromContext "missing.key" ] } in
  check
    bool
    "missing key = None"
    true
...
let test_from_file () =
  ...
  Fun.protect
    ~finally:(fun () ->
      ...
      | _ -> ())
    (fun () ->
       let config : Append_instruction.config = { sources = [ FromFile path ] } in
       match Append_instruction.render ~turn:0 config with
...
let test_empty_sources () =
  let config : Append_instruction.config = { sources = [] } in
  check
    bool
    "empty = None"
    true
...
let test_hook_appends_to_existing () =
  let config : Append_instruction.config = { sources = [ Static "New." ] } in
  let hook = Append_instruction.as_hook config in
  let event =
    Hooks.BeforeTurnParams
      { turn = 1
      ...
      ; last_tool_results = []
      ; current_params =
          { Hooks.default_turn_params with extra_system_context = Some "Existing." }
      ; reasoning = Hooks.empty_reasoning_summary
      }
  in
  match hook event with
...
let test_hook_continues_on_other_events () =
  let config : Append_instruction.config = { sources = [ Static "text" ] } in
  let hook = Append_instruction.as_hook config in
