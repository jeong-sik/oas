...
let make_instance ?(service_name = "test-service") () =
  Otel_tracer.create_instance
    ~config:{ Otel_tracer.service_name; endpoint = None } ()
...
let test_build_otlp_body_structure () =
  ...
  let service_attr = List.find (fun a ->
  ...
  Alcotest.(check string) "traceId" "aaaa1234aaaa1234aaaa1234aaaa1234"
    (span_json |> member "traceId" |> to_string);
  Alcotest.(check string) "name" "test_span"
    (span_json |> member "name" |> to_string)
...
let test_build_otlp_body_empty () =
  ...
  let open Yojson.Safe.Util in
  let scope_spans =
    json |> member "resourceSpans" |> to_list |> List.hd
    |> member "scopeSpans" |> to_list |> List.hd
    |> member "spans" |> to_list
  in
...
let test_split_batches_exact () =
  ...
  List.iter (fun b ->
    Alcotest.(check int) "batch size 2" 2 (List.length b)
  ) batches
...
let test_export_result_variants () =
  ...
  (match e with Exported { span_count } -> Alcotest.(check int) "exported" 5 span_count | _ -> ());
  (match p with Partial_failure { exported; dropped; _ } ->
    Alcotest.(check int) "partial exported" 3 exported;
    Alcotest.(check int) "partial dropped" 2 dropped
  | _ -> ());
  (match f with Failed { reason } ->
...
let test_instance_flush_clears_spans () =
  ...
  let attrs : Tracing.span_attrs = {
    name = "test_op"; agent_name = "test_agent"; turn = 1;
    kind = Tracing.Agent_run; extra = [];
  } in
...
let test_total_exported_via_daemon () =
  ...
  let instance = make_instance () in
  let config = { (default_export_config ~endpoint:"http://127.0.0.1:1/v1/traces")
                 with flush_interval_sec = 60.0 } in
...
let () =
  Alcotest.run "otel_export" [
    ...
    ("serialization", [
      ...
      Alcotest.test_case "otlp_body_multiple" `Quick test_build_otlp_body_multiple_spans;
    ]);
    ("batch", [
    ...
    ("result_types", [
      Alcotest.test_case "variants" `Quick test_export_result_variants;
    ]);
    ("instance", [
