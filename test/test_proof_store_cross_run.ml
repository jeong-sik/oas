...
let make_test_store () =
  let tmpdir = Filename.concat
      ...
      (Printf.sprintf "oas-test-cross-run-%d" (Random.bits () land 0xFFFFFF)) in
...
let test_list_runs_ordered_sorted () =
  ...
  (match result with
   | Ok (runs, _) ->
     ...
     Alcotest.(check (list string)) "sorted by ended_at"
       ["run-a"; "run-b"; "run-c"] ids
   | Error e -> Alcotest.fail e);
  cleanup tmpdir
...
let test_list_runs_ordered_tiebreak () =
  ...
  (match result with
   | Ok (runs, _) ->
     let ids = List.map (fun (r : Proof_store.run_info) -> r.run_id) runs in
     Alcotest.(check (list string)) "tie-break by run_id"
       ["run-a"; "run-m"; "run-z"] ids
   | Error e -> Alcotest.fail e);
  cleanup tmpdir
...
let test_list_runs_ordered_max_runs_exceeded () =
  ...
  for i = 1 to 5 do
    write_proof config
      (make_proof ~run_id:(Printf.sprintf "r%d" i)
         ~ended_at:(Float.of_int (i * 1000)) ())
  done;
...
let test_scope_json_roundtrip () =
  let proof = make_proof ~run_id:"scope-test" ~ended_at:1000.0
      ~scope:"test-scope" () in
  let json = Cdal_proof.to_json proof in
  (match Cdal_proof.of_json json with
   | Ok decoded ->
     Alcotest.(check (option string)) "scope preserved"
       (Some "test-scope") decoded.scope
   | Error e -> Alcotest.fail e)
...
let test_scope_none_json_roundtrip () =
  ...
  (match Cdal_proof.of_json json with
   | Ok decoded ->
     Alcotest.(check (option string)) "scope is None" None decoded.scope
   | Error e -> Alcotest.fail e)
...
let test_scope_missing_in_json () =
  ...
  let json = Cdal_proof.to_json proof in
  let json_without_scope = match json with
    | `Assoc fields ->
      `Assoc (List.filter (fun (k, _) -> k <> "scope") fields)
    | other -> other
  in
  (match Cdal_proof.of_json json_without_scope with
   | Ok decoded ->
     Alcotest.(check (option string)) "defaults to None" None decoded.scope
   | Error e -> Alcotest.fail e)
...
let test_run_info_metadata () =
  let config, tmpdir = make_test_store () in
  write_proof config
    (make_proof ~run_id:"meta-1" ~ended_at:5000.0 ~scope:"test-scope" ());
  let result = Proof_store.list_runs_ordered config () in
  (match result with
   | Ok (runs, _) ->
...
let () =
  Alcotest.run "Proof_store_cross_run"
    [
      ( "list_runs_ordered",
        [
          ...
          Alcotest.test_case "corrupted manifest" `Quick test_list_runs_ordered_corrupted_manifest;
          Alcotest.test_case "max_runs exceeded" `Quick test_list_runs_ordered_max_runs_exceeded;
        ] );
      ( "load_window",
        [
      ...
      ( "scope",
        [
          ...
          Alcotest.test_case "JSON roundtrip without scope" `Quick test_scope_none_json_roundtrip;
          Alcotest.test_case "missing scope in JSON defaults to None" `Quick test_scope_missing_in_json;
        ] );
      ( "run_info",
        [
