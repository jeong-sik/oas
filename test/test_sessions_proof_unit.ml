...
let test_participant_by_name_empty () =
  ...
  Alcotest.(check bool) "empty participants" true (Option.is_none result)
;;

(* --- resolved_provider_of_participant tests --- *)

let test_resolved_provider_from_resolved () =
  let p = make_participant ~resolved_provider:(Some "gcp") ~provider:(Some "aws") () in
  ...
  Alcotest.(check (option string)) "resolved_provider wins" (Some "gcp") result
;;

let test_resolved_provider_uses_participant_provider () =
  let p = make_participant ~resolved_provider:None ~provider:(Some "aws") () in
  let session = make_session ~provider:(Some "default-p") () in
  let result = Sessions.resolved_provider_of_participant session (Some p) in
  Alcotest.(check (option string)) "falls back to provider" (Some "aws") result
;;
...
let test_resolved_provider_uses_session_provider () =
  ...
  Alcotest.(check (option string)) "falls back to session" (Some "session-p") result
;;

(* --- resolved_model_of_participant tests --- *)

let test_resolved_model_from_resolved () =
  let p = make_participant ~resolved_model:(Some "gpt-4") ~model:(Some "gpt-3") () in
  let session = make_session ~model:(Some "default-m") () in
  let result = Sessions.resolved_model_of_participant session (Some p) in
  Alcotest.(check (option string)) "resolved_model wins" (Some "gpt-4") result
;;

let test_resolved_model_fallback_to_model () =
  let p = make_participant ~resolved_model:None ~model:(Some "gpt-3") () in
  let session = make_session ~model:(Some "default-m") () in
  let result = Sessions.resolved_model_of_participant session (Some p) in
  Alcotest.(check (option string)) "falls back to model" (Some "gpt-3") result
;;
...
let test_resolved_model_fallback_to_session () =
  ...
  Alcotest.(check (option string)) "falls back to session" (Some "session-m") result
;;
...
let test_worker_status_failed () =
  ...
  Alcotest.(check bool) "Failed_participant -> Failed" true (result = Sessions.Failed)
;;

let test_worker_status_done () =
  let p = make_participant ~state:Runtime.Done () in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Done -> Completed" true (result = Sessions.Completed)
;;
...
let test_worker_status_live_with_progress () =
  ...
  in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Live with progress -> Running" true (result = Sessions.Running)
;;

let test_worker_status_live_no_progress () =
  let p =
    make_participant ~state:Runtime.Live ~started_at:None ~last_progress_at:None ()
  in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Live no progress -> Ready" true (result = Sessions.Ready)
;;
...
let test_worker_status_idle_same_timestamps () =
  ...
  in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Idle same timestamps -> Ready" true (result = Sessions.Ready)
;;

let test_worker_status_starting () =
  let p = make_participant ~state:Runtime.Starting () in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Starting -> Accepted" true (result = Sessions.Accepted)
;;

let test_worker_status_planned () =
  let p = make_participant ~state:Runtime.Planned () in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Planned -> Planned" true (result = Sessions.Planned)
;;

let test_worker_status_detached () =
  let p = make_participant ~state:Runtime.Detached () in
  let result = Sessions.worker_status_of_participant (Some p) in
  Alcotest.(check bool) "Detached -> Planned" true (result = Sessions.Planned)
;;

let test_worker_status_none () =
  let result = Sessions.worker_status_of_participant None in
  Alcotest.(check bool) "None -> Completed" true (result = Sessions.Completed)
;;
...
let test_worker_order_ts_finished () =
  let w =
    make_worker_run
      ...
      ~last_progress_at:(Some 200.0)
      ~started_at:(Some 100.0)
      ()
  in
  let result = Sessions.worker_order_ts w in
  Alcotest.(check (option (float 0.01))) "finished_at wins" (Some 300.0) result
;;
...
let test_worker_order_ts_fallback_progress () =
  ...
  in
  let result = Sessions.worker_order_ts w in
  Alcotest.(check (option (float 0.01))) "last_progress_at fallback" (Some 200.0) result
;;

let test_worker_order_ts_fallback_started () =
  let w =
    make_worker_run ~finished_at:None ~last_progress_at:None ~started_at:(Some 100.0) ()
  in
  let result = Sessions.worker_order_ts w in
  Alcotest.(check (option (float 0.01))) "started_at fallback" (Some 100.0) result
;;

let test_worker_order_ts_all_none () =
  let w = make_worker_run ~finished_at:None ~last_progress_at:None ~started_at:None () in
  let result = Sessions.worker_order_ts w in
  Alcotest.(check (option (float 0.01))) "all None" None result
;;
...
let test_sort_worker_runs_by_finished () =
  ...
  let ids = List.map (fun (w : Sessions.worker_run) -> w.worker_run_id) sorted in
  Alcotest.(check (list string)) "sorted by finished_at asc" [ "r2"; "r3"; "r1" ] ids
;;

let test_sort_worker_runs_none_last () =
  let w1 = make_worker_run ~worker_run_id:"r1" ~finished_at:(Some 100.0) () in
  let w2 =
    make_worker_run
      ~worker_run_id:"r2"
      ~finished_at:None
  ...
  let ids = List.map (fun (w : Sessions.worker_run) -> w.worker_run_id) sorted in
  Alcotest.(check (list string)) "None timestamps go after Some" [ "r1"; "r2" ] ids
;;
...
let test_sort_worker_runs_both_none_by_id () =
  ...
  let ids = List.map (fun (w : Sessions.worker_run) -> w.worker_run_id) sorted in
  Alcotest.(check (list string)) "fallback to id sort" [ "alpha"; "beta" ] ids
;;
...
let test_sort_worker_runs_mixed_timestamps () =
  ...
  let sorted = Sessions.sort_worker_runs [ w1; w2; w3 ] in
  let ids = List.map (fun (w : Sessions.worker_run) -> w.worker_run_id) sorted in
  Alcotest.(check (list string)) "mixed ts sources" [ "r1"; "r3"; "r2" ] ids
;;
...
let test_latest_worker_by_all () =
  let w1 = make_worker_run ~worker_run_id:"r1" ~finished_at:(Some 100.0) () in
  let w2 = make_worker_run ~worker_run_id:"r2" ~finished_at:(Some 200.0) () in
  let result = Sessions.latest_worker_by (fun _ -> true) [ w1; w2 ] in
  Alcotest.(check string) "latest by ts" "r2" (Option.get result).worker_run_id
;;
...
let test_latest_worker_by_with_predicate () =
  ...
  let result =
    Sessions.latest_worker_by
      ...
      [ w1; w2; w3 ]
  in
  Alcotest.(check string) "latest completed" "r3" (Option.get result).worker_run_id
;;
...
let () =
  Alcotest.run
    ...
    ; ( "resolved_provider_of_participant"
      , [ Alcotest.test_case "resolved wins" `Quick test_resolved_provider_from_resolved
        ; Alcotest.test_case
    ...
    ; ( "resolved_model_of_participant"
      , [ Alcotest.test_case "resolved wins" `Quick test_resolved_model_from_resolved
        ; Alcotest.test_case
            "fallback to model"
            `Quick
    ...
    ; ( "worker_status_of_participant"
      , [ Alcotest.test_case "Failed_participant" `Quick test_worker_status_failed
        ; Alcotest.test_case "Done" `Quick test_worker_status_done
        ; Alcotest.test_case
            "Live with progress"
            `Quick
            test_worker_status_live_with_progress
        ; Alcotest.test_case "Live no progress" `Quick test_worker_status_live_no_progress
        ; Alcotest.test_case
            "Idle same timestamps"
            `Quick
    ...
    ; ( "sort_worker_runs"
      , [ Alcotest.test_case "by_finished" `Quick test_sort_worker_runs_by_finished
        ; Alcotest.test_case "none_last" `Quick test_sort_worker_runs_none_last
        ; Alcotest.test_case
            "both_none_by_id"
            `Quick
    ...
    ; ( "latest_worker_by"
      , [ Alcotest.test_case "all" `Quick test_latest_worker_by_all
        ; Alcotest.test_case "with_predicate" `Quick test_latest_worker_by_with_predicate
