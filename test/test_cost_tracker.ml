...
let test_check_budget_no_limit () =
  ...
  check bool "no limit = no error" true
...
let test_check_budget_under () =
  ...
  check bool "under budget = no error" true
    (Option.is_none (Cost_tracker.check_budget config usage))

let test_check_budget_at_limit () =
  let config = make_config ~max_cost_usd:1.0 () in
  let usage = make_usage ~cost:1.0 () in
  check bool "at limit = no error" true
    (Option.is_none (Cost_tracker.check_budget config usage))
...
let test_check_budget_zero_limit () =
  ...
  check bool "zero limit + any cost = exceeded" true
...
let test_report_to_string () =
  ...
  check bool "contains cost" true
    (String.length s > 0 &&
     try let _ = Str.search_forward (Str.regexp_string "0.123456") s 0 in true
...
let test_offload_to_context_string_offloaded () =
  let s = Context_offload.to_context_string
    (Offloaded { path = "/tmp/test.txt"; preview = "first..."; original_bytes = 1000 }) in
  check bool "contains path" true
    (try let _ = Str.search_forward (Str.regexp_string "/tmp/test.txt") s 0 in true
     with Not_found -> false);
  check bool "contains bytes" true
    (try let _ = Str.search_forward (Str.regexp_string "1000") s 0 in true
     with Not_found -> false)

let test_offload_convenience () =
  let config = { Context_offload.default_config with threshold_bytes = 5 } in
  let result = Context_offload.offload_tool_result
    ~config ~tool_name:"conv" "this is longer than 5" in
  check bool "contains Offloaded" true
    (try let _ = Str.search_forward (Str.regexp_string "Offloaded") result 0 in true
     with Not_found -> false)
...
let test_offload_special_chars_in_name () =
  ...
  | Context_offload.Offloaded { path; _ } ->
    check bool "no slash in filename" true
...
let () =
  run "cost_and_offload" [
    ...
    "context_offload", [
      ...
      test_case "to_context_string offloaded" `Quick test_offload_to_context_string_offloaded;
