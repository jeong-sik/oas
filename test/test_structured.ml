...
let test_retry_finds_tool_use_id () =
  let content =
    [ Text "some text"
    ; ToolUse { id = "tu_abc"; name = "extract_person"; input = `Assoc [ "bad", `Null ] }
    ]
  in
  let tool_use_id =
    List.find_map
      ...
      content
  in
  Alcotest.(check (option string)) "finds tool_use_id" (Some "tu_abc") tool_use_id
;;
  ...
  Alcotest.run
    ...
    ; ( "extract_with_retry"
      , [ Alcotest.test_case
            ...
            `Quick
            test_retry_message_construction
        ; Alcotest.test_case "finds tool_use_id" `Quick test_retry_finds_tool_use_id
        ; Alcotest.test_case
            "on_validation_error callback"
            `Quick
            test_retry_on_validation_error_callback
        ] )
    ]
;;
