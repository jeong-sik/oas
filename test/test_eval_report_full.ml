...
let make_metric name value : Eval.metric =
  ...
  run "Eval_report_full" [
    ...
    "generate_no_baseline", [
      test_case "no baseline yields NoBaseline" `Quick (fun () ->
        ...
        check bool "no comparison" true (Option.is_none report.comparison);
        check bool "summary contains NO BASELINE" true
          (String.length report.summary > 0
           && let s = report.summary in
              try let _ = String.index s 'N' in true with Not_found -> false));
      ...
      test_case "no baseline summary mentions agent name" `Quick (fun () ->
        ...
        check bool "contains agent name" true
          (let s = report.summary in
           ...
           try let _ = Str.search_forward re s 0 in true
    ...
    "generate_baseline_pass", [
      test_case "baseline pass with high pass_at_k" `Quick (fun () ->
        let baseline_rm = make_run
          [make_metric "accuracy" (Float_val 0.90)]
          [pass_verdict] in
        ...
        check bool "summary starts with PASS" true
          (String.length report.summary >= 4
           && String.sub report.summary 0 4 = "PASS"));

      test_case "baseline pass with comparison details" `Quick (fun () ->
        let baseline_rm = make_run
          [make_metric "latency" (Float_val 100.0)]
          [pass_verdict] in
        let baseline = Eval_baseline.create ~description:"base" baseline_rm in
        let runs = [make_run [make_metric "latency" (Float_val 99.0)] [pass_verdict]] in
        let report = Eval_report.generate ~baseline runs in
    ...
    "generate_baseline_fail", [
      test_case "baseline fail when comparison has regressions" `Quick (fun () ->
        ...
        let runs = [make_run [make_metric "accuracy" (Float_val 0.50)] [pass_verdict]] in
        let report = Eval_report.generate ~baseline runs in
        (match report.verdict with
         | `Fail -> ()
        ...
        check bool "summary starts with FAIL" true
          (String.length report.summary >= 4
           && String.sub report.summary 0 4 = "FAIL"));

      test_case "fail when pass_at_k below 0.5 with passing comparison" `Quick (fun () ->
    ...
    "to_json", [
      test_case "pass verdict in json" `Quick (fun () ->
        let baseline_rm = make_run
          [make_metric "score" (Float_val 0.90)]
          [pass_verdict] in
        let baseline = Eval_baseline.create ~description:"b" baseline_rm in
        let runs = [make_run [make_metric "score" (Float_val 0.91)] [pass_verdict]] in
        let report = Eval_report.generate ~baseline runs in
        let json = Eval_report.to_json report in
        let open Yojson.Safe.Util in
        check string "verdict" "pass" (json |> member "verdict" |> to_string);
        check string "agent_name" "test-agent" (json |> member "agent_name" |> to_string);
    ...
    "to_string", [
      test_case "contains agent name" `Quick (fun () ->
        ...
        check bool "contains foobot" true
          (try let _ = Str.search_forward re s 0 in true
           with Not_found -> false));
      ...
      test_case "to_string with comparison shows diffs" `Quick (fun () ->
        ...
        let runs = [make_run [make_metric "latency" (Float_val 50.0)] [pass_verdict]] in
        let report = Eval_report.generate ~baseline runs in
        let s = Eval_report.to_string report in
      ...
      test_case "to_string without comparison is single line" `Quick (fun () ->
        ...
        let report = Eval_report.generate runs in
        let s = Eval_report.to_string report in
        check bool "no newline (single line)" true
