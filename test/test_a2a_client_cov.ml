...
let rpc_error_json msg =
  Printf.sprintf {|{"jsonrpc": "2.0", "error": {"code": -32600, "message": "%s"}, "id": 1}|} msg
...
let test_discover_error () =
  ...
  try
    Eio.Switch.run @@ fun sw ->
    let url = start_error_mock ~sw ~net:env#net ~port:21202
  ...
  try
    ...
    (match A2a_client.discover ~sw ~net:env#net url with
     ...
     | Ok remote ->
       let msg : A2a_task.task_message = {
         role = TaskUser;
         parts = [A2a_task.Text_part "will error"];
         metadata = [];
       } in
       (match A2a_client.send_task ~sw ~net:env#net remote msg with
        | Ok _ -> fail "expected error"
...
let test_text_of_task_multi () =
  ...
  let msg2 : A2a_task.task_message = {
    role = TaskUser;
    parts = [A2a_task.Text_part "user msg"];
    metadata = [];
  } in
  let task : A2a_task.task = {
    id = "t-multi"; state = Completed;
  ...
  try
    ...
    (match A2a_client.discover ~sw ~net:env#net url with
     ...
     | Ok remote ->
       ...
       let msg : A2a_task.task_message = {
         role = TaskUser;
         parts = [A2a_task.Text_part "will fail"];
         metadata = [];
       } in
