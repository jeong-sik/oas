...
let mk_tool_call ?(id = "tc-1") ?(name = "bash") ?(is_error = false)
    ?(started = 1.0) ?finished ?(result = None) () : Trajectory.tool_call =
  { tool_use_id = id; tool_name = name;
    tool_input = `Assoc [("cmd", `String "ls")];
    tool_result = result; is_error; started_at = started;
    finished_at = finished }

let mk_trajectory ?(steps = []) ?(success = true) ?finished ?error () : Trajectory.trajectory =
  { agent_name = "test-agent"; model = "test-model"; prompt = "do stuff";
    steps; started_at = 100.0; finished_at = finished;
    success; metrics = None; error }
...
let test_show_tool_call () =
  ...
  Alcotest.(check bool) "contains name" true
    (try ignore (Str.search_forward (Str.regexp_string "read") s 0); true
...
let test_show_trajectory () =
  ...
  Alcotest.(check bool) "has agent name" true
    (try ignore (Str.search_forward (Str.regexp_string "test-agent") s 0); true
     with Not_found -> false)
...
let test_elapsed_finished () =
  ...
  Alcotest.(check (option (float 0.001))) "elapsed" (Some 10.0)
    (Trajectory.elapsed_s t)

let test_elapsed_not_finished () =
  let t = mk_trajectory () in
  Alcotest.(check (option (float 0.001))) "not finished" None
    (Trajectory.elapsed_s t)

(* ── JSON round-trip: tool_call ──────────────────────────────── *)

let test_tool_call_json_roundtrip () =
  let tc = mk_tool_call ~id:"rt-1" ~name:"write" ~is_error:false
      ~started:10.0 ~finished:12.0 ~result:(Some "ok") () in
...
let test_step_json_act () =
  ...
  | Ok (Act { tool_call = tc2; _ }) ->
    Alcotest.(check string) "id" "a1" tc2.tool_use_id
...
let test_step_json_respond () =
  ...
  | Ok (Respond { content; _ }) ->
    Alcotest.(check string) "content" "done" content
...
let test_trajectory_json_roundtrip () =
  let t = mk_trajectory
      ~steps:[
        ...
        Act { tool_call = mk_tool_call ~id:"t1" ~started:101.0
                ~finished:102.0 ~result:(Some "ok") (); ts = 101.0 };
...
let () =
  Alcotest.run "Trajectory unit" [
    ...
    ("count_steps", [
      tc "empty" test_count_steps_empty;
      tc "mixed" test_count_steps_mixed;
    ]);
    ("derived_metrics", [
