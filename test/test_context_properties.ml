...
let test_copy_independence =
  QCheck.Test.make ~count:200 ~name:"copy_independence"
...
let test_json_roundtrip =
  QCheck.Test.make ~count:200 ~name:"json_roundtrip"
...
let test_scope_no_leakage =
  QCheck.Test.make ~count:300 ~name:"scope_no_leakage"
    QCheck.(triple
      (make gen_kv_list)   (* parent initial keys *)
      (make gen_kv_list)   (* keys written in child scope *)
      (make gen_kv_list))  (* propagate_down/up lists *)
    (fun (parent_pairs, child_pairs, prop_pairs) ->
       ...
       let prop_up_keys =
         if child_pairs = [] then []
         else [fst (List.hd child_pairs)] in
       let scope = Context.create_scope
         ~parent ~propagate_down:prop_down ~propagate_up:prop_up_keys in
       ...
       let child_only_keys =
         ...
         |> List.filter (fun k ->
              ...
              not (List.exists (fun (pk, _) -> pk = k) parent_pairs)) in
       let no_leak = List.for_all (fun k ->
...
let test_propagate_down_complete =
  QCheck.Test.make ~count:200 ~name:"propagate_down_complete"
    (QCheck.make gen_kv_list)
    (fun pairs ->
       let parent = Context.create () in
       ...
       let scope = Context.create_scope
         ~parent ~propagate_down:all_keys ~propagate_up:[] in
...
let test_propagate_up_selective =
  QCheck.Test.make ~count:200 ~name:"propagate_up_selective"
    QCheck.(pair (make gen_kv_list) (make gen_kv_list))
    (fun (child_pairs, extra_pairs) ->
       let parent = Context.create () in
       (* Only first pair's key is in propagate_up (if any) *)
       let up_keys = match child_pairs with
         | (k, _) :: _ -> [k]
         | [] -> [] in
       let scope = Context.create_scope
         ~parent ~propagate_down:[] ~propagate_up:up_keys in
...
let test_diff_correct =
  QCheck.Test.make ~count:200 ~name:"diff_correct"
    QCheck.(pair (make gen_kv_list) (make gen_kv_list))
    (fun (before_pairs, after_pairs) ->
       ...
       let added_ok = List.for_all (fun (k, _) ->
         not (List.exists (fun (bk, _) -> bk = k) before_snap)
       ) d.added in
       (* removed: in before but not in after *)
       let removed_ok = List.for_all (fun k ->
         not (List.exists (fun (ak, _) -> ak = k) after_snap)
       ) d.removed in
       (* changed: in both, different value *)
       let changed_ok = List.for_all (fun (k, new_v) ->
...
let test_scope_isolation_parallel =
  QCheck.Test.make ~count:100 ~name:"parallel_scope_isolation"
    QCheck.(pair (make gen_kv_list) (make gen_kv_list))
    (fun (pairs_a, pairs_b) ->
       let parent = Context.create () in
       (* Two isolated scopes from same parent *)
       let scope_a = Context.create_scope
         ~parent ~propagate_down:[] ~propagate_up:["result_a"] in
       let scope_b = Context.create_scope
         ~parent ~propagate_down:[] ~propagate_up:["result_b"] in
...
let () =
  ...
  Alcotest.run "Context.t Properties" [
    "qcheck", suite;
  ]
