...
let test_legacy_search_dispatches_to_search_files_when_visible () =
  Eio_main.run
  @@ fun _env ->
  let context = Context.create () in
  let captured_input = ref `Null in
  ...
  let result =
    Agent_tools.find_and_execute_tool
      ...
      "tool-legacy-search"
  in
  check bool "legacy Grep succeeds" false result.is_error;
...
let test_legacy_execute_dispatches_simple_command_as_typed_argv () =
  Eio_main.run
  @@ fun _env ->
  let context = Context.create () in
  let captured_input = ref `Null in
  ...
  let result =
    Agent_tools.find_and_execute_tool
      ...
      "tool-legacy-execute"
  in
  check bool "legacy execute_command succeeds" false result.is_error;
  ...
  run
    ...
    ; ( "fields"
      , [ test_case "agent_started" `Quick test_agent_started_fields
        ...
        ; test_case
            "legacy Grep dispatches to SearchFiles when visible"
            `Quick
            test_legacy_search_dispatches_to_search_files_when_visible
        ; test_case
            "legacy execute_command dispatches to typed Execute when visible"
            `Quick
            test_legacy_execute_dispatches_simple_command_as_typed_argv
        ; test_case
