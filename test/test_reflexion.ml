...
let counting_evaluator ~fail_until =
  ...
  fun _response ->
    ...
    then
      Reflexion.Fail
        { diagnosis = Printf.sprintf "attempt %d failed" !counter
        ; critique = [ "try harder" ]
        }
    else Reflexion.Pass
;;
...
let test_format_reflection_fail () =
  let verdict =
    Reflexion.Fail
      { diagnosis = "wrong output"; critique = [ "missed edge case"; "bad formatting" ] }
  in
  let text = Reflexion.format_reflection ~attempt_number:2 verdict in
  check
    bool
    "contains attempt number"
    true
    (String.length text > 0 && contains_sub ~affix:"attempt 2" text);
  check bool "contains diagnosis" true (contains_sub ~affix:"wrong output" text);
  check bool "contains critique" true (contains_sub ~affix:"missed edge case" text)
;;

let test_format_reflection_no_critique () =
  let verdict = Reflexion.Fail { diagnosis = "wrong"; critique = [] } in
  let text = Reflexion.format_reflection ~attempt_number:1 verdict in
  check bool "contains diagnosis" true (contains_sub ~affix:"wrong" text);
  check bool "no critique section" true (not (contains_sub ~affix:"Critique" text))
;;
  ...
  | Ok r ->
    ...
    check
      ...
      (match first.verdict with
       | Reflexion.Fail _ -> true
       | _ -> false);
    check bool "first has reflection" true (Option.is_some first.reflection_text)
  | Error _ -> fail "unexpected error"
;;
...
let test_no_critique_in_reflections () =
  let evaluator = counting_evaluator ~fail_until:2 in
  let config = { (Reflexion.default_config ~evaluator) with include_critique = false } in
  ...
  | Ok r ->
    ...
    (match !reflections_received with
     | [ refl ] ->
       check bool "no critique section" true (not (contains_sub ~affix:"Critique:" refl))
     | _ -> ())
  | Error _ -> fail "unexpected error"
;;
