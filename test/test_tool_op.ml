...
let test_apply_keep_all () = check sl "identity" base (Tool_op.apply Keep_all base)
let test_apply_clear_all () = check sl "empty" [] (Tool_op.apply Clear_all base)

let test_apply_add () =
  check sl "union" [ "a"; "b"; "c"; "d" ] (Tool_op.apply (Add [ "d" ]) base)
;;

let test_apply_add_duplicate () =
  check sl "no dup" [ "a"; "b"; "c" ] (Tool_op.apply (Add [ "a" ]) base)
;;

let test_apply_add_empty () = check sl "identity" base (Tool_op.apply (Add []) base)

let test_apply_remove () =
  check sl "diff" [ "a"; "c" ] (Tool_op.apply (Remove [ "b" ]) base)
;;

let test_apply_remove_absent () =
  check sl "no change" base (Tool_op.apply (Remove [ "x" ]) base)
;;

let test_apply_remove_empty () = check sl "identity" base (Tool_op.apply (Remove []) base)

let test_apply_replace_with () =
  check sl "replaced" [ "x"; "y" ] (Tool_op.apply (Replace_with [ "x"; "y" ]) base)
;;

let test_apply_replace_with_empty () =
  check sl "empty" [] (Tool_op.apply (Replace_with []) base)
;;
...
let test_apply_intersect_empty () =
  check sl "empty" [] (Tool_op.apply (Intersect_with []) base)
;;

let test_apply_intersect_disjoint () =
  check sl "empty" [] (Tool_op.apply (Intersect_with [ "x"; "y" ]) base)
;;
...
let test_apply_seq_empty () = check sl "identity" base (Tool_op.apply (Seq []) base)

let test_apply_add_to_empty () =
  check sl "add to empty" [ "x" ] (Tool_op.apply (Add [ "x" ]) [])
;;

let test_apply_remove_from_empty () =
  check sl "remove from empty" [] (Tool_op.apply (Remove [ "x" ]) [])
;;

let test_apply_dedup_current () =
  check sl "deduped input" [ "a"; "b" ] (Tool_op.apply Keep_all [ "a"; "b"; "a" ])
;;

let test_apply_remove_dedup_current () =
  check sl "deduped" [ "a" ] (Tool_op.apply (Remove [ "b" ]) [ "a"; "b"; "a" ])
;;
...
let test_compose_empty () =
  check bool "Keep_all" true (Tool_op.equal (Tool_op.compose []) Keep_all)
;;

let test_compose_single () =
  let op = Add [ "x" ] in
  check bool "unwrap" true (Tool_op.equal (Tool_op.compose [ op ]) op)
;;
...
let test_to_tool_filter () =
  ...
  match filter with
  | Guardrails.AllowList names -> check_set_eq "filtered" [ "a"; "c" ] names
...
let test_to_tool_filter_identity () =
  ...
  match filter with
  | Guardrails.AllowList names -> check_set_eq "identity" [ "a"; "b" ] names
  | _ -> fail "expected AllowList"
;;
...
let test_equal_add_order () =
  check bool "normalized" true (Tool_op.equal (Add [ "a"; "b" ]) (Add [ "b"; "a" ]))
;;
...
let test_equal_keep_all () = check bool "same" true (Tool_op.equal Keep_all Keep_all)

let test_equal_different_variant () =
  check bool "different" false (Tool_op.equal (Add [ "a" ]) (Remove [ "a" ]))
;;
...
let test_is_identity_seq_empty () =
  check bool "Seq [] is identity" true (Tool_op.is_identity (Seq []))
;;

let test_is_identity_nested_empty_seq () =
  check bool "Seq [Seq []] is identity" true (Tool_op.is_identity (Seq [ Seq [] ]))
;;

(* ── QCheck properties ────────────────────────────────────── *)

let gen_name = QCheck.Gen.(map (fun i -> Printf.sprintf "tool_%d" i) (int_range 0 20))
let gen_names = QCheck.Gen.(list_size (int_range 0 5) gen_name)

let gen_tool_set =
  QCheck.Gen.(
    map
      (fun names -> List.sort_uniq String.compare names)
...
let arb_simple_op =
  QCheck.make gen_simple_op ~print:(fun op ->
    Yojson.Safe.to_string (Tool_op.to_yojson op))
;;

let arb_tool_set = QCheck.make gen_tool_set ~print:(fun xs -> String.concat ", " xs)

let prop_keep_all_identity =
  QCheck.Test.make ~name:"Keep_all is identity" ~count:100 arb_tool_set (fun xs ->
    Tool_op.apply Keep_all xs = List.sort_uniq String.compare xs
...
let prop_compose_homomorphism =
  QCheck.Test.make
    ~name:"compose [a;b] = apply b (apply a)"
    ~count:100
...
let prop_yojson_roundtrip =
  QCheck.Test.make
    ~name:"to_yojson >> of_yojson roundtrip"
    ~count:100
    arb_simple_op
    (fun op ->
...
let () =
  run
    ...
    ; ( "edge_cases"
      , [ test_case
            "Replace_with [] = Clear_all"
            `Quick
            test_replace_with_empty_eq_clear_all
