...
let make_run ?(name="test-agent") metrics verdicts : Eval.run_metrics =
  { run_id = "run-1"; agent_name = name;
    timestamp = 1000.0; metrics; harness_verdicts = verdicts;
    trace_summary = None }
...
let test_compare_no_regression () =
  let baseline_rm = make_run [
    ...
    make_metric "latency_ms" (Int_val 200);
  ] [] in
  let baseline = Eval_baseline.create ~description:"base" baseline_rm in
  let current = make_run [
    make_metric "accuracy" (Float_val 0.92);
    make_metric "latency_ms" (Int_val 190);
  ] [] in
...
let test_compare_with_regression () =
  ...
  let baseline = Eval_baseline.create ~description:"base" baseline_rm in
  let current = make_run [
    make_metric "accuracy" (Float_val 0.80);
  ] [] in
  let c = Eval_baseline.compare ~baseline ~current () in
...
let test_compare_added_metric () =
  ...
  let baseline = Eval_baseline.create ~description:"base" baseline_rm in
  let current = make_run [
    make_metric "a" (Float_val 1.0);
    make_metric "b" (Float_val 2.0);
  ] [] in
  let c = Eval_baseline.compare ~baseline ~current () in
  check bool "passed" true c.passed;
  let added = List.filter (function Eval_baseline.Added _ -> true | _ -> false) c.diffs in
...
let test_compare_removed_metric () =
  ...
  let c = Eval_baseline.compare ~baseline ~current () in
  let removed = List.filter (function Eval_baseline.Removed _ -> true | _ -> false) c.diffs in
...
let test_report_with_baseline () =
  let baseline_rm = make_run [make_metric "accuracy" (Float_val 0.90)] [pass_verdict] in
  let baseline = Eval_baseline.create ~description:"base" baseline_rm in
  ...
  (match report.verdict with `Pass -> () | _ -> fail "should pass")

let test_report_no_baseline () =
  let runs = [make_run [] [pass_verdict]] in
  let report = Eval_report.generate runs in
  (match report.verdict with `NoBaseline -> () | _ -> fail "should be no_baseline")
...
let test_show_diff () =
  let s = Eval_baseline.show_diff
    (Regressed { name = "acc"; baseline_val = 0.9; current_val = 0.8; delta_pct = (-11.1) }) in
...
let () =
  run "eval_baseline" [
    ...
    "display", [
      test_case "show_diff" `Quick test_show_diff;
    ];
  ]
