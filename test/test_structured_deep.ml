...
let test_property_ordering_preserved () =
  let schema : unit Structured.schema = {
    name = "ordered"; description = "Test ordering";
    params = [
  ...
  let prop_names = json |> member "input_schema" |> member "properties"
    |> to_assoc |> List.map fst in
  Alcotest.(check (list string)) "property order matches declaration"
    ["first"; "second"; "third"] prop_names

(** Required list preserves filter_map order (declaration order). *)
let test_required_ordering () =
  let schema : unit Structured.schema = {
    name = "req_order"; description = "Required ordering";
    params = [
  ...
  Alcotest.(check (list string)) "required in declaration order"
    ["z_last"; "a_first"] required
...
let test_complex_parse_success () =
  let schema : (string * float * bool) Structured.schema = {
    ...
    params = [
      ...
      { name = "active"; description = "Active"; param_type = Boolean; required = true };
    ];
    parse = (fun json ->
      let open Yojson.Safe.Util in
      try
...
let test_complex_parse_detailed_error () =
  ...
  let content_neg = [
    ToolUse { id = "tu_neg"; name = "detail_error"; input = `Assoc [("value", `Int (-5))] };
  ] in
  ...
  let content_str = [
    ToolUse { id = "tu_s"; name = "detail_error";
...
let test_extract_skips_unrelated_tools () =
  ...
  let content = [
    ...
    ToolUse { id = "tu_c"; name = "target_tool"; input = `Assoc [("data", `String "found")] };
    ToolUse { id = "tu_d"; name = "target_tool"; input = `Assoc [("data", `String "second")] };
  ] in
  match Structured.extract_tool_input ~schema content with
...
let test_schema_all_six_param_types () =
  let schema : unit Structured.schema = {
    name = "all_six"; description = "All 6 types";
    params = [
...
let test_schema_json_full_structure () =
  let schema : string Structured.schema = {
    ...
    params = [
      ...
      { name = "unit"; description = "Temperature unit"; param_type = String; required = false };
    ];
    parse = (fun json ->
      let open Yojson.Safe.Util in
  ...
  Alcotest.(check (list string)) "input_schema keys"
    ["type"; "properties"; "required"] is_keys;
  (* Property descriptions preserved *)
  let city_desc = json |> member "input_schema" |> member "properties"
    |> member "city" |> member "description" |> to_string in
...
let test_extract_deeply_nested_input () =
  ...
  let nested_input = `Assoc [
    ("level1", `Assoc [
      ("level2", `Assoc [
        ("value", `String "deep_value")
      ])
    ])
  ] in
  let content = [
...
let () =
  Alcotest.run "structured_deep" [
    "schema_ordering", [
      Alcotest.test_case "property ordering preserved" `Quick
        test_property_ordering_preserved;
      Alcotest.test_case "required ordering" `Quick
        test_required_ordering;
    ];
    "complex_parse", [
      Alcotest.test_case "nested record parse" `Quick
        test_complex_parse_success;
      Alcotest.test_case "detailed error paths" `Quick
        test_complex_parse_detailed_error;
    ];
    "extract_tool_input_deep", [
      Alcotest.test_case "skips unrelated tools" `Quick
        test_extract_skips_unrelated_tools;
      Alcotest.test_case "deeply nested input" `Quick
        test_extract_deeply_nested_input;
    ];
    "json_extractor_deep", [
      Alcotest.test_case "parse array" `Quick
        test_json_extractor_array;
      Alcotest.test_case "parse nested" `Quick
        test_json_extractor_nested;
      Alcotest.test_case "ignores tool_use" `Quick
        test_json_extractor_ignores_tool_use;
    ];
    "text_extractor_deep", [
      Alcotest.test_case "multiline" `Quick
        test_text_extractor_multiline;
      Alcotest.test_case "empty string" `Quick
        test_text_extractor_empty_string;
    ];
    "schema_structure", [
      Alcotest.test_case "all 6 param types" `Quick
        test_schema_all_six_param_types;
      Alcotest.test_case "full JSON structure" `Quick
        test_schema_json_full_structure;
    ];
    "retry_result", [
