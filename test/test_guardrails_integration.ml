...
let tracking_handler ~tool_use_count ~tools_seen_count _conn _req body =
  ...
  decr tool_use_count;
  let response_body =
    if n > 0 then tool_use_body ~tool_name:"echo"
    else text_body "finished"
  in
...
let make_tool name =
  Tool.create ~name ~description:("tool " ^ name) ~parameters:[]
    (fun _input -> Ok { Types.content = name ^ " result" })
  ...
  with_mock_server ~port:18201
    ...
    (fun ~sw ~net ~base_url ->
      ...
      let options = { Agent.default_options with
        base_url;
        guardrails = { Guardrails.default with
...
let test_custom_prefix_filter () =
  ...
  with_mock_server ~port:18203
    (tracking_handler ~tool_use_count:tool_calls ~tools_seen_count:tools_seen)
    (fun ~sw ~net ~base_url ->
      let tools = [make_tool "safe_read"; make_tool "safe_write";
                   make_tool "danger_delete"] in
      let options = { Agent.default_options with
        base_url;
        guardrails = { Guardrails.default with
          tool_filter = Guardrails.Custom (fun schema ->
            String.length schema.name >= 5 &&
            String.sub schema.name 0 5 = "safe_") } } in
      let agent = Agent.create ~net ~options ~tools () in
      (match Agent.run ~sw agent "test" with
       | Ok _ | Error _ -> ());
...
let test_limit_blocks_multi_tool_response () =
  let multi_tool_handler _conn _req body =
    let body_str =
      Eio.Buf_read.(of_flow ~max_size:(1024 * 1024) body |> take_all) in
    let json = Yojson.Safe.from_string body_str in
    ...
    let has_limit_msg =
      try
        ...
        (try ignore (Str.search_forward
                       (Str.regexp_string "limit exceeded") s 0); true
         with Not_found -> false)
      with _ -> false
    in
    let response_body =
      if has_limit_msg then text_body "understood"
      else
  ...
  with_mock_server ~port:18204 multi_tool_handler (fun ~sw ~net ~base_url ->
    ...
    let tool = Tool.create ~name:"echo" ~description:"echo" ~parameters:[]
        (fun _input -> incr tool_calls; Ok { content = "ok" }) in
    let options = { Agent.default_options with
      base_url;
      guardrails = { Guardrails.default with
        max_tool_calls_per_turn = Some 2 } } in
...
let test_empty_tools_no_crash () =
  with_mock_server ~port:18206 text_only_handler (fun ~sw ~net ~base_url ->
    let options = { Agent.default_options with
      base_url;
      guardrails = { Guardrails.default with
        tool_filter = Guardrails.DenyList ["nonexistent"] } } in
    let agent = Agent.create ~net ~options ~tools:[] () in
    match Agent.run ~sw agent "test" with
    | Ok _ -> ()
...
let () =
  ...
  run "Guardrails_Integration" [
    ...
    "max_tool_calls", [
      test_case "blocks multi-tool response" `Quick
    ...
    "edge_cases", [
      ...
      test_case "default guardrails work" `Quick
        test_guardrails_default_with_agent;
    ];
  ]
