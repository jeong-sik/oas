...
let list_files_tool =
  Tool.create ~name:"list_files" ~description:"List files in a directory"
    ~parameters:[
...
let delete_file_tool =
  Tool.create ~name:"delete_file" ~description:"Delete a file permanently"
    ~parameters:[
      { name = "path"; description = "File to delete";
        param_type = String; required = true };
    ]
    (fun args ->
       let path = Yojson.Safe.Util.(args |> member "path" |> to_string) in
...
let my_approval ~tool_name ~input =
  Printf.eprintf "[approval] %s called with %s — REJECTED\n%!"
    tool_name (Yojson.Safe.to_string input);
...
let () =
  ...
  let options = { Agent.default_options with
    ...
    guardrails = {
      tool_filter = DenyList ["delete_file"];  (* also blocked via guardrails *)
  ...
  let agent = Agent.create
      ~net:env#net ~config ~options
      ~tools:[list_files_tool; delete_file_tool] () in
