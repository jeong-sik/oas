...
let demo_minimal () =
  ...
  let agent =
    ...
    |> Builder.build
  in
  Printf.printf "  Created agent: %s (model=%s)\n"
...
let demo_full () =
  Printf.printf "\n--- 2. Full-featured Builder ---\n";
  Eio_main.run @@ fun env ->
  let echo_tool = Tool.create ~name:"echo" ~description:"Echo input"
      ~parameters:[] (fun _input -> Ok { content = "echoed" }) in

  let hooks = { Hooks.empty with
    before_turn = Some (fun _ -> Printf.eprintf "  [hook] turn\n%!"; Continue);
    on_stop = Some (fun _ -> Printf.eprintf "  [hook] stop\n%!"; Continue) } in
  ...
  | Ok agent ->
    ...
    Printf.printf "  Max cost: %s USD\n"
      (match st.config.max_cost_usd with
       | Some c -> Printf.sprintf "%.2f" c | None -> "unlimited");
...
let demo_validation_error () =
  ...
  match result with
  | Ok _ ->
    Printf.printf "  (no validation error for negative max_turns)\n"
  | Error e ->
