...
let session_root () =
  ...
  | _ ->
    Filename.concat
      ...
      (Printf.sprintf "oas-direct-%d-%06x" (Unix.getpid ()) (Random.int 0xFFFFFF))
;;
...
let () =
  ...
  ignore
    (unwrap_result
       (Agent.run_stream ~sw ~on_event:(fun _ -> ()) agent "Create direct evidence."));
  let bundle =
    unwrap_result
      (Direct_evidence.persist
         ~agent
         ~raw_trace
         ~options:
  ...
  let report =
    unwrap_result
      (Direct_evidence.run_conformance
         ~agent
         ~raw_trace
         ~options:
           { Direct_evidence.session_root = Some root
