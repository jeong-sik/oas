...
let print_event (event : Event_bus.event) =
  ...
  | AgentStarted { agent_name; task_id } ->
    Printf.printf "  %s %s agent=%s task=%s\n%!"
      (dim (Printf.sprintf "[%s]" corr)) (green name) agent_name task_id
  | AgentCompleted { agent_name; result; elapsed; _ } ->
    let status = match result with
      | Ok _ -> green "ok" | Error _ -> red "err" in
    Printf.printf "  %s %s agent=%s status=%s elapsed=%.3fs\n%!"
      (dim (Printf.sprintf "[%s]" corr)) name agent_name status elapsed
  | AgentFailed { agent_name; task_id; error; elapsed } ->
    Printf.printf "  %s %s agent=%s task=%s elapsed=%.3fs\n%!"
      (dim (Printf.sprintf "[%s]" corr)) (red name) agent_name task_id elapsed;
