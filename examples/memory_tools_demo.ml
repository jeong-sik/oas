...
let () =
  ...
  run_tool "recall" recall (`Assoc [ "key", `String "shared_goal" ]);
  run_tool
    "remember_episode"
    remember_episode
    (`Assoc
  ...
  run_tool
    ...
    find_procedure
    (`Assoc
        [ "pattern", `String "deploy"; "min_confidence", `Float 0.7; "touch", `Bool true ]);
  ...
  run_tool
    "alice remember"
    alice_remember
    (`Assoc [ "key", `String "shared_note"; "value_json", `String {|{"owner":"alice"}|} ]);
  run_tool
    "bob remember"
    bob_remember
    (`Assoc [ "key", `String "shared_note"; "value_json", `String {|{"owner":"bob"}|} ]);
  run_tool "bob recall" bob_recall (`Assoc [ "key", `String "shared_note" ])
;;
