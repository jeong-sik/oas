...
let execute_plan plan =
  let rec loop plan =
    ...
    | None ->
      ...
      Plan.finish plan
    | Some step ->
      if not (Plan.deps_satisfied plan step.id) then begin
        ...
        Printf.printf "         -> Done (progress: %.0f%%)\n%!"
...
let () =
  ...
  let plan = Plan.create ~goal:"deploy service v2" ~planner:"ci-agent" ()
    |> fun p -> Plan.add_step p ~id:"build" ~description:"build Docker image" ()
    |> fun p -> Plan.add_step p ~id:"test" ~description:"run integration tests"
      ~depends_on:["build"] ()
    |> fun p -> Plan.add_step p ~id:"stage" ~description:"deploy to staging"
      ~depends_on:["test"] ()
    |> fun p -> Plan.add_step p ~id:"prod" ~description:"deploy to production"
      ~depends_on:["stage"] ()
  in
  ...
  let plan2 = Plan.create ~goal:"data migration" ~planner:"etl-agent" ()
    |> fun p -> Plan.add_step p ~id:"backup" ~description:"backup existing data" ()
    |> fun p -> Plan.add_step p ~id:"migrate" ~description:"run migration scripts"
      ~depends_on:["backup"] ()
    |> fun p -> Plan.add_step p ~id:"verify" ~description:"verify data integrity"
      ~depends_on:["migrate"] ()
    |> Plan.start
  in
  (* Execute backup *)
  ...
  let new_steps : Plan.step list = [
    { id = "fix-schema"; description = "fix schema conflicts";
      status = Pending; result = None; depends_on = [] };
    { id = "retry-migrate"; description = "retry migration";
      status = Pending; result = None; depends_on = ["fix-schema"] };
    { id = "verify-v2"; description = "verify with updated checks";
      status = Pending; result = None; depends_on = ["retry-migrate"] };
  ] in
  ...
  (match Plan.of_json json with
   | Ok restored ->
     Printf.printf "  Restored: goal='%s', steps=%d, status=%s\n"
       (Plan.goal restored) (Plan.step_count restored)
  ...
  let dag = Plan.create ~goal:"ML pipeline" ~planner:"ml-agent" ()
    |> fun p -> Plan.add_step p ~id:"data" ~description:"fetch dataset" ()
    |> fun p -> Plan.add_step p ~id:"features" ~description:"extract features"
      ~depends_on:["data"] ()
    |> fun p -> Plan.add_step p ~id:"train" ~description:"train model"
      ~depends_on:["features"] ()
    |> fun p -> Plan.add_step p ~id:"eval" ~description:"evaluate model"
      ~depends_on:["train"] ()
    |> fun p -> Plan.add_step p ~id:"deploy-model" ~description:"deploy to API"
      ~depends_on:["eval"; "data"] ()
  in
  Printf.printf "  deploy-model deps_satisfied? %b (expected: false)\n"
  ...
  Printf.printf "  deploy-model deps_satisfied? %b (expected: true)\n"
    (Plan.deps_satisfied dag "deploy-model");
  ignore dag;
