...
let demo_diff_guard () =
  ...
  in
  let report =
    Autonomy_diff_guard.validate_patch ~allowed_paths:[ "lib/"; "test/" ] patch
  in
  Printf.printf
    "diff_guard.accepted=%b paths=[%s]\n"
    report.accepted
...
let demo_lesson_memory () =
  ...
  | Some text -> Printf.printf "lesson_context:\n%s\n" text
  | None -> Printf.printf "lesson_context: <none>\n"
;;
...
let demo_metric_exec () =
  ...
  in
  match result with
  | Error err -> Printf.printf "exec_error=%s\n" (Error.to_string err)
  | Ok output ->
    Printf.printf "exec_status=%s\n" (Autonomy_exec.status_to_string output.status);
    (match Metric_contract.parse output.stdout with
     | Ok metric -> Printf.printf "parsed_metric=%s=%.2f\n" metric.name metric.value
     | Error detail -> Printf.printf "metric_parse_error=%s\n" detail)
;;
