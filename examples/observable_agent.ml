...
let tool_use_body ~tool_name ~input_json =
  Printf.sprintf
    ...
    tool_name tool_name input_json

let mock_handler call_count _conn _req body =
  let _ = Eio.Buf_read.(of_flow ~max_size:(1024 * 1024) body |> take_all) in
  let n = !call_count in
  incr call_count;
  let response_body = match n with
    | 0 -> tool_use_body ~tool_name:"calculator"
              ~input_json:{|{"expression":"6 * 7"}|}
...
let start_mock_server ~env ~sw =
  ...
  let server = Cohttp_eio.Server.make
      ~callback:(mock_handler call_count) () in
