...
let make_pipeline () =
  let validate : Durable.step = {
    ...
    execute = (fun input ->
      ...
      try
        ...
        Ok input
      with _ -> Error "missing 'data' field");
  } in
  ...
  |> fun sm -> Durable.add_step sm validate
  |> fun sm -> Durable.add_step sm transform
  |> fun sm -> Durable.add_step sm persist

(* ── Main scenario ───────────────────────────────── *)

let () =
  let policy = Policy.create [deny_dangerous_tools; escalate_handoffs; resource_budget] in
  ...
  let safe_dp = Policy.BeforeToolCall { tool_name = "search"; agent_name = "researcher" } in
  ...
  Printf.printf "\n--- Scenario 3: Resource budgeting ---\n";
  let small_dp = Policy.ResourceRequest { agent_name = "worker"; resource = "gpu"; amount = 5.0 } in
  ...
  let big_dp = Policy.ResourceRequest { agent_name = "worker"; resource = "gpu"; amount = 20.0 } in
  ...
  Printf.printf "  Pipeline '%s' with %d steps: [%s]\n"
  ...
  (match Durable.execution_state_of_json json with
   | Ok (Suspended { reason; _ }) ->
     Printf.printf "  Round-trip OK: reason='%s'\n" reason
   | _ -> Printf.printf "  Round-trip failed\n");

  (* Audit summary *)
  Printf.printf "\n--- Audit Summary ---\n";
  Printf.printf "  Total entries: %d\n" (Audit.count audit);
  let denied = Audit.query audit () |> List.filter (fun (e : Audit.entry) ->
    match e.verdict with Some (Deny _) -> true | _ -> false) in
  Printf.printf "  Denied actions: %d\n" (List.length denied);
  Printf.printf "  JSON export: %s\n"
