...
let print_event (event : Event_bus.event) =
  ...
  | AgentCompleted { agent_name; elapsed; _ } ->
    Printf.printf
      "  %s agent.completed %s elapsed=%.3fs\n%!"
      (dim (Printf.sprintf "[%s]" corr))
      (cyan agent_name)
      elapsed
  | _ -> ()
...
let response_for_message body_str =
  ...
  | _ ->
    ...
    in
    if text = "Please research OCaml Eio."
    then
...
let () =
  ...
  try
    ...
    let config =
      { default_config with
        name = "root"
      ; system_prompt = Some "Delegate research to specialists."
      }
    in
    let agent = Agent.create ~net:env#net ~config ~options () in
    let result =
      Agent.run_with_handoffs
        ~sw
        agent
        ~targets:[ researcher ]
        "Please research OCaml Eio."
    in
