...
벤치마크 (2026-04-01/02). 도구 21개를 가진 외부 coordinator의 essential tool surface로 측정:
...
Available tools:
1. coord_heartbeat  - Send heartbeat to keep the session alive
2. coord_broadcast  - Broadcast a message to all agents in the room
3. coord_status     - Get current room status and agent list
...

Response: ["coord_broadcast", "coord_status"]
```
...
### 6.1 원칙: OAS는 어떤 specific consumer도 모른다

OAS에 특정 coordinator의 prefix(`masc_`, `xyz_`, …)나 특화 로직이 들어가면 안 된다.
Tool_selector는 범용 API만 제공하고, 외부 coordinator가 소비자로서 자신만의 전략을 구성한다.

### 6.2 소비 패턴 (예시)

다음 예시는 어떤 consumer가 자신의 ~21개 도구 중 일부만 매 턴 노출하고 싶을 때 OAS API를 어떻게 호출하는지 보여준다. tool 이름의 prefix(`coord_`, `kpr_`, 그 외)는 consumer가 정한다.

```ocaml
(* Consumer-side: their own tool list, their own naming. *)
let consumer_tools : Tool.t list = [
  (* coord_heartbeat, coord_done, coord_status, ... *)
]

let agent =
  Builder.create ()
  |> Builder.with_tools consumer_tools
  |> Builder.with_tool_selector
       (Tool_selector.TopK_bm25 {
         k = 5;
         always_include = ["coord_heartbeat"; "coord_done"];
       })
  |> Builder.build_safe
```

또는 Categorical 전략으로 자신의 도구를 그룹화:

```ocaml
let consumer_groups = [
  ("lifecycle",     ["coord_heartbeat"; "coord_heartbeat_start"; "coord_done"; "coord_leave"]);
  ("task",          ["coord_add_task"; "coord_claim"; "coord_claim_next"; "coord_transition"]);
  ("communication", ["coord_broadcast"; "coord_agents"; "coord_who"]);
  ("plan",          ["coord_plan_get"; "coord_plan_update"; "coord_plan_set_task"]);
  ("status",        ["coord_status"; "coord_check"; "coord_tasks"]);
]
...
let agent =
  ...
  |> Builder.with_tool_selector
       (Tool_selector.Categorical {
         ...
         classifier      = `Bm25;
         always_include  = ["coord_heartbeat"];
       })
  |> Builder.build_safe
```
...
D: TopK_llm (k=5)
E: Categorical (consumer-defined groups)
...
| Qwen3.5-35B-A3B | 3B active | Primary target agent profile |
...
### Phase 5: Downstream consumer integration (추정 1일, consumer 쪽)

1. Consumer가 자기 측에서 tool_selector 전략 설정
2. Consumer가 자기 도구를 Categorical groups로 정의
3. Consumer가 자기 세션에서 검증

이 작업은 OAS 레포 밖이며, 본 RFC 범위는 아니다.
...
2. Tool Selection Benchmark, 2026-04-01/02. Internal benchmark notes referenced by this RFC (consumer-side dataset, not in this repository).
