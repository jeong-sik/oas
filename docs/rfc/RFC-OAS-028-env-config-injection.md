...
| 5 | `Cli_common_env` alias surface (33 sites / 16 files) — H1 | **Resolved (option a, §9 Q1):** add `?getenv` seam to `Cli_common_env.get/.bool/.int/.float/.list` now; Phase 7 migrates the 33 sites to config-record fields; §10 lint enforces zero `Cli_common_env` calls in core. End-state identical for (a)/(b); (a) touches 1 module for the seam. **Type-interaction trap (discovered in Phase 0 (b), PR #2226):** adding `?getenv` to `get` breaks existing per-module `?(getenv = Cli_common_env.get)` defaults (#2213/#2214 sites: `discovery` ×5, `provider_config.default_reasoning_effort_value`) because `get`'s type becomes optional-bearing, so the default's type is inferred as a doubly-nested optional. Phase 0 (b) converts those to a partial-application wrapper `(fun n -> Cli_common_env.get n)`; `base/model_registry` uses raw `Sys.getenv_opt` (drops a `base→llm_provider` edge). **This is why Phase 0 (b) must touch the legacy per-module seam sites even though Phase 7 owns the full migration** — the seam cannot land in isolation. |
...
- **Phase 7 — `Cli_common_env` migration + lint (cut 5, 9).** **Depends on Phase 0 (b) seam (PR #2226).** Migrate the remaining 33 sites to the chosen seam (Q1). Enforce the pure-core definition (§4) with a CI lint (§10). Delete all retained `[@@deprecated]` snapshots. Gate G7: lint clean; zero snapshots retained.
