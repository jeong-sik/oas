...
| DashScope (Qwen) | JSON mode `json_object` only. The documented `response_format.type` enum is closed at `{text, json_object}` on both the OpenAI-compatible endpoint and the native DashScope API | JSON syntax only; the guide instructs callers to validate client-side | `validate_output_schema_request` applies the per-model capability check, and `dashscope_capabilities` declares `supports_structured_output = false`; structured output takes the tool strategy | Corrects the 2026-05-05 row, which claimed `json_schema` support. A 2026-07-22 re-read found zero occurrences of `json_schema` across the Chat Completions reference, the native DashScope reference, the structured-output guide, and the Responses-API compatibility page. The guide's "JSON Schema format" means prose in the prompt. `json_object` carries a documented prompt requirement: the request must instruct the model to output JSON or the API errors. Ref: https://www.alibabacloud.com/help/en/model-studio/qwen-structured-output — checked 2026-07-22. |
