...
as a deterministic, non-retryable tool error. Use `Continue` to execute the
call, `ApprovalRequired` to delegate the decision to the registered callback,
and `HookFailed` for hook infrastructure failure.
