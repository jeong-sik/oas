...
let load_records_from_session_like session_or_path =
  if Sys.file_exists session_or_path
  then (
    ...
    match
      Agent_sdk.Sessions_store.get_latest_raw_trace_run ~session_id:session_or_path ()
    with
    | Error _ as err -> err
    | Ok None ->
      Error
        (Agent_sdk.Error.Io
           (Agent_sdk.Error.ValidationFailed
              { detail =
                  Printf.sprintf "No raw trace run found for session '%s'" session_or_path
              }))
    | Ok (Some run_ref) ->
      (match
         Agent_sdk.Sessions_store.get_raw_trace_records
...
let case_id_of_trace_path path =
  Filename.basename path |> Filename.remove_extension |> Agent_sdk.Raw_trace.safe_name
;;
...
let exit_for_report ?eval_report (report : Agent_sdk.Harness_report.t) =
  Printf.printf
    "Evaluated %d cases: %d passed, %d failed, %d skipped\n"
    report.summary.total
    report.summary.passed
    report.summary.failed
    report.summary.skipped;
  Option.iter
    (fun (eval_report : Agent_sdk.Eval_report.t) ->
       Printf.printf "%s\n" eval_report.summary)
    eval_report;
  let eval_failed =
    match eval_report with
...
let select_report_case ?case_id ~purpose (report : Agent_sdk.Harness_report.t) =
  ...
  | Some case_id ->
    let selected, rest =
      List.partition
        (fun ((result, _) :
               Agent_sdk.Harness_report.case_result * Agent_sdk.Eval.run_metrics) ->
           String.equal result.case_id case_id)
        metric_results
    in
    (match selected with
     | [ selected ] -> Ok (selected, rest)
     | [] -> Error (Printf.sprintf "Case '%s' was not found while %s" case_id purpose)
     | _ ->
       Error
         (Printf.sprintf
            "Case '%s' matched multiple evaluated runs while %s"
            case_id
            purpose))
  | None ->
    (match metric_results with
     | [] ->
       Error
         (Printf.sprintf "Dataset did not produce any evaluated runs while %s" purpose)
     ...
     | _ ->
       ...
       Error
         (Printf.sprintf
            "Dataset produced multiple evaluated runs (%s); rerun with --case <id> while \
             %s"
            case_ids
            purpose))
;;
  ...
  | Ok (((result, metrics) as selected), _) ->
    ...
    (match result.status with
     ...
     | Agent_sdk.Harness_report.Fail | Agent_sdk.Harness_report.Skip ->
       Error
         (Printf.sprintf
            "Cannot save baseline from case '%s' because it did not pass"
            result.case_id))
;;
...
let run_harness_dataset ?trace_dir config_file dataset =
  ...
  | Some config_file ->
    (match Agent_sdk.Agent_config.load config_file with
     | Error e ->
       Error (Printf.sprintf "Error loading config: %s" (Agent_sdk.Error.to_string e))
...
let run_info =
  Cmd.info
    "run"
    ~doc:"Run an agent with a prompt"
    ~man:
      [ `S "DESCRIPTION"
      ; `P "Load an agent from a config file and run it with the given prompt."
      ]
;;
...
let init_cmd name =
  ...
  (try Sys.mkdir dir 0o755 with
   | Sys_error _ -> Printf.eprintf "Warning: directory '%s' already exists\n" dir);
  let config_content =
    Printf.sprintf
      {|{
  "name": "%s",
...
|}
  ...
  Out_channel.with_open_text config_path (fun oc -> output_string oc config_content);
...
let init_info =
  Cmd.info
    "init"
    ~doc:"Scaffold a new agent project"
...
let card_cmd config_file =
  ...
  | Ok cfg ->
    Oas_cli_support.with_runtime
    @@ fun ~env:_ ~sw:_ ~net ~mgr:_ ~clock:_ ->
...
let card_info =
  Cmd.info
    "card"
    ~doc:"Print agent card as JSON"
    ~man:
      [ `S "DESCRIPTION"
      ; `P "Load an agent config and output its agent card in JSON format."
      ]
;;
...
let eval_run_cmd config_file baseline_path case_id dataset_path out_dir =
  ...
  | Error e ->
    ...
    exit 1
  | Ok dataset ->
    if Option.is_some case_id && Option.is_none baseline_path
    then (
       ...
       let eval_report =
         ...
         | Some baseline_path ->
           (match Agent_sdk.Eval_baseline.load ~path:baseline_path with
            | Error msg -> Error (Printf.sprintf "Error loading baseline: %s" msg)
            | Ok baseline ->
...
let eval_run_term =
  let config =
    let doc =
      "Path to agent configuration file (JSON). Optional for offline trace-replay \
       datasets."
    in
  ...
  let case_id =
    let doc =
      "Case ID to compare against the baseline when the dataset has multiple evaluated \
       runs."
    in
...
let eval_run_info = Cmd.info "run" ~doc:"Run a harness dataset and write reports"
...
let eval_save_baseline_term =
  ...
  in
  Term.(
    const eval_save_baseline_cmd $ config $ case_id $ dataset $ out_path $ description)
;;

let eval_save_baseline_info =
  Cmd.info
    "save-baseline"
    ~doc:"Run a dataset and save a baseline from one evaluated case"
;;
...
let eval_record_trace_cmd session_or_path out_path =
  ...
  | Ok (records, source_trace_path) ->
    ...
    (match
       Agent_sdk.Harness_case.trace_replay_of_records
         ~id:case_id
         ~source_trace_path
         records
     with
     | Error e ->
       Printf.eprintf
         "Error creating trace replay case: %s\n"
         (Agent_sdk.Error.to_string e);
       exit 1
...
let eval_info = Cmd.info "eval" ~doc:"Harness dataset utilities"
...
let version_cmd () = Printf.printf "oas %s\n" version
let version_term = Term.(const version_cmd $ const ())
let version_info = Cmd.info "version" ~doc:"Print SDK version"

(* ── Main ────────────────────────────────────────────────── *)

let default_term = Term.(ret (const (fun () -> `Help (`Pager, None)) $ const ()))

let main_info =
  Cmd.info
    "oas"
    ~version
    ~doc:"OCaml Agent SDK CLI"
    ~man:
      [ `S "DESCRIPTION"
