...
let session_root () =
  ...
  | _ ->
      Filename.concat (Filename.get_temp_dir_name ())
        (Printf.sprintf "oas-proof-%d-%06x" (Unix.getpid ())
           (Random.int 0xFFFFFF))
  ...
  let rec loop acc remaining =
    ...
    let terminal =
      List.exists
        ...
        acc
    in
    if terminal || remaining <= 0 then acc
    else loop acc (remaining - 1)
  in
  loop [] 30
        ...
        |> List.filter_map (function
               | Client.Partial_message { participant_name; delta } ->
                   Some
                     (`Assoc
                        [
                          ("participant", `String participant_name);
                          ("delta", `String delta);
                        ])
               | _ -> None)
      in
      ...
      let output =
        `Assoc
          [
            ...
            ("proof", proof);
            ( "telemetry",
              artifact_json ~session_root:root ~session_id
                ~artifact_name:"runtime-telemetry-json" artifacts );
            ( "evidence",
              artifact_json ~session_root:root ~session_id
                ~artifact_name:"runtime-evidence" artifacts );
          ]
      in
